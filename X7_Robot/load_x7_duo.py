...
from isaaclab.app import AppLauncher   # 这一个可以在前面
...
def run(sim: sim_utils.SimulationContext, scene: InteractiveScene, client: websocket_client_policy.WebsocketClientPolicy):
    ...
    while simulation_app.is_running():
        ...
        for i in range(10):
            ...
            # 4）当前机器人状态
            cur_q = scene["robot"].data.joint_pos[0].clone()  # shape (13,) or (14,)
            
            # 5）更新目标动作：这里用「增量控制」更稳定
            cur_q[0:6]  += 0.05 * torch.tensor(left_arm,  device=cur_q.device)
            cur_q[6]     = 0
            cur_q[7:13] += 0.05 * torch.tensor(right_arm, device=cur_q.device)
            cur_q[13] = 0
...
def main():
    ...
    scene_cfg = X7SceneCfg(num_envs=args.num_envs, env_spacing=2.0)
