...
PRESS_INITIAL_Z = 1.80
PRESS_TARGET_Z = 0.44
PRESS_SETTLE_TIME = 2.0
PRESS_DURATION = 3.0
...
parser.add_argument(
    "--press_spheres",
    action="store_true",
    help="Compress sphere specimens with a descending kinematic plate after they settle.",
)
...
if args_cli.press_spheres and PRESET.geometry != "sphere_plate":
    parser.error("--press_spheres is only supported by sphere-based presets.")
...
PRESS_NAMES = tuple(name.replace("specimen", "press") for name in SPECIMEN_NAMES) if args_cli.press_spheres else ()
...
def create_scene_cfg():
    ...
    def press_cfg(index: int, center_x: float) -> RigidObjectCfg:
        return RigidObjectCfg(
            ...
            spawn=sim_utils.CuboidCfg(
                ...
                visual_material=sim_utils.PreviewSurfaceCfg(
                    diffuse_color=(0.72, 0.75, 0.80),
                    roughness=0.24,
                    metallic=0.35,
                ),
            ),
            init_state=RigidObjectCfg.InitialStateCfg(pos=(center_x, 0.0, PRESS_INITIAL_Z)),
        )
    ...
    if args_cli.variant_index is not None:
        ...
        class SingleMaterialTuningSceneCfg(InteractiveSceneCfg):
            ...
            press: RigidObjectCfg | None = press_cfg(variant_index, 0.0) if args_cli.press_spheres else None
    ...
    class MaterialTuningSceneCfg(InteractiveSceneCfg):
        ...
        press_0: RigidObjectCfg | None = press_cfg(0, float(centers_x[0])) if args_cli.press_spheres else None
        press_1: RigidObjectCfg | None = (
            press_cfg(1, float(centers_x[1])) if args_cli.press_spheres and len(PRESET.values) > 1 else None
        )
        press_2: RigidObjectCfg | None = (
            press_cfg(2, float(centers_x[2])) if args_cli.press_spheres and len(PRESET.values) > 2 else None
        )
...
def update_press(scene, simulation_time: float) -> None:
    """Move each optional sphere press along the shared smooth loading path."""
    if not PRESS_NAMES:
        return
    linear_progress = np.clip((simulation_time - PRESS_SETTLE_TIME) / PRESS_DURATION, 0.0, 1.0)
    smooth_progress = linear_progress * linear_progress * (3.0 - 2.0 * linear_progress)
    smooth_progress_rate = 6.0 * linear_progress * (1.0 - linear_progress) / PRESS_DURATION
    press_z = PRESS_INITIAL_Z + smooth_progress * (PRESS_TARGET_Z - PRESS_INITIAL_Z)
    press_velocity_z = smooth_progress_rate * (PRESS_TARGET_Z - PRESS_INITIAL_Z)
    for press_name in PRESS_NAMES:
        press = scene[press_name]
        press_pose = press.data.default_root_pose.torch.clone()
        press_pose[:, 2] = press_z
        press_velocity = press.data.default_root_vel.torch.clone()
        press_velocity[:, 2] = press_velocity_z
        press.write_root_link_pose_to_sim_index(root_pose=press_pose)
        press.write_root_link_velocity_to_sim_index(root_velocity=press_velocity)
