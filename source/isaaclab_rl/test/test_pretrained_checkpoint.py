...
@pytest.mark.parametrize(
    ("task_name", "physics_backend", "render_backend", "companion_filename"),
    [
        (
...
def test_get_published_pretrained_checkpoint_downloads_shadow_hand_feature_extractor(
    task_name: str,
    physics_backend: str,
    render_backend: str,
    ...
    """Test that Shadow Hand camera policies fetch their feature-extractor checkpoint."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(pretrained_checkpoint, "ISAACLAB_NUCLEUS_DIR", "omniverse://IsaacLab")
    retrieved = []
    ...
    stem = companion_filename.removesuffix("_feature_extractor.pth")
    download_dir = str(Path(".pretrained_checkpoints") / "rsl_rl" / stem)
    assert path == str(Path(download_dir) / f"{stem}.pt")
    assert retrieved == [
        (f"omniverse://IsaacLab/PretrainedCheckpoints/rsl_rl/{stem}.pt", download_dir),
        (f"omniverse://IsaacLab/PretrainedCheckpoints/rsl_rl/{companion_filename}", download_dir),
    ]


def test_get_published_pretrained_checkpoint_rejects_missing_shadow_hand_feature_extractor(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
):
    """Test that a Shadow Hand camera policy is unusable without its feature extractor."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(pretrained_checkpoint, "ISAACLAB_NUCLEUS_DIR", "omniverse://IsaacLab")

    def _retrieve_file_path(remote_path: str, download_dir: str) -> str:
        if remote_path.endswith("_feature_extractor.pth"):
    ...
    assert "feature-extractor checkpoint is currently unavailable" in output
    assert "Isaac-Reorient-Cube-Shadow-Camera_physx_rtx_rsl_rl_feature_extractor.pth" in output
