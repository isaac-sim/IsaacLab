...
"""skrl LEAPP export integration test."""
...
_EXPORT_MODULE_NAME = "_isaaclab_skrl_leapp_export"
...
def _export_dir(task_name: str) -> str:
    ...
    return os.path.join(_REPO_ROOT, ".pretrained_checkpoints", "skrl", train_task, task_name)
...
def _load_export_module():
    """Load the LEAPP skrl export script as an importable module."""
    module = sys.modules.get(_EXPORT_MODULE_NAME)
    if module is not None and hasattr(module, "export_skrl_agent"):
        return module
...
def _run_export_task(task_name: str, simulation_app, sim_utils, get_settings_manager, resolve_task_config) -> None:
    ...
    try:
        ...
        with _clean_hydra_argv():
            ...
            env_cfg, agent_cfg = resolve_task_config(task_name, agent_cfg_entry_point)
        exported = export_module.export_skrl_agent(args_cli, env_cfg, agent_cfg, simulation_app)
...
def test_skrl_export_flow():
    """Run skrl export.py and assert the expected artifacts are created."""
    try:
    ...
    if "Unfortunately a pre-trained checkpoint is currently unavailable" in result.stdout:
        pytest.skip("No pretrained skrl checkpoint available for test task")
