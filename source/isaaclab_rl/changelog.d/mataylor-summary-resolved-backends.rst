...
* Fixed the train and play startup summary reporting ``physics=physx`` and ``renderer=rtx`` by the
  ...
  summary now names the backend the run will actually use -- ``ovrtx`` for a kitless
  ``physics=ovphysx renderer=rtx`` run and ``isaacsim_rtx`` when the run needs Kit -- and lists the
  selector itself in the presets row.
