...
def test_public_joint_velocity_kernel_factory_requires_explicit_proxy_view() -> None:
    ...
    assert ProxyArray not in get_args(get_type_hints(write_joint_vel_data_kernel)["env_ids"])
    with pytest.raises(TypeError, match="torch.Tensor or wp.array"):
        write_joint_vel_data_kernel(env_ids, joint_ids)
    write_joint_vel_data_kernel(env_ids.warp, joint_ids.warp)
    write_joint_vel_data_kernel(env_ids.torch, joint_ids.torch)
