...
Carb path values align with Isaac Lab application rendering profiles (e.g. under ``apps/``) where applicable.
These baselines are used by :class:`~isaaclab.rendering_mode.RenderingModeCfg` and
``isaaclab.rendering_mode`` helpers for Kit viewport and Isaac RTX camera renderers.
"""
...
_BUILTIN_RTX_RENDERING_MODE_PRESETS: dict[str, dict[str, Any]] = {
    "performance": {
...
def get_builtin_rtx_rendering_mode_preset(preset_name: str) -> dict[str, Any]:
    """Return a deep copy of the requested built-in RTX rendering mode preset."""
    if preset_name not in {"performance", "balanced", "quality"}:
        raise ValueError(f"Unknown preset '{preset_name}'.")
    return deepcopy(_BUILTIN_RTX_RENDERING_MODE_PRESETS[preset_name])
