...
from isaaclab.sim.views.xform_prim_view import XformPrimView as _CoreXformPrimView
from isaaclab.utils.warp import fabric as fabric_utils

logger = logging.getLogger(__name__)


class XformPrimView(_CoreXformPrimView):
    ...
    .. warning::
        **Fabric requires CUDA**: Fabric is only supported on CUDA devices.
        ...
        Fabric with CPU device will automatically fall back to USD operations.
    """
    ...
    """
    Overrides -- Fabric-accelerated getters / setters.
    """
    ...
    def _set_world_poses_fabric(
        ...
        positions_wp = wp.from_torch(positions) if positions is not None else wp.zeros((0, 3), dtype=wp.float32).to(self._device)
        orientations_wp = wp.from_torch(orientations) if orientations is not None else wp.zeros((0, 4), dtype=wp.float32).to(self._device)
        ...
        wp.launch(
            ...
            dim=count,
            inputs=[
                self._fabric_world_matrices, positions_wp, orientations_wp, scales_wp,
                False, False, False, indices_wp, self._view_to_fabric,
            ],
    ...
    def _set_local_poses_fabric(
        ...
        """Set local poses using USD (matches Isaac Sim's design -- Fabric only accelerates world poses)."""
    ...
    def _get_world_poses_fabric(self, indices: Sequence[int] | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        wp.launch(
            ...
            inputs=[
                self._fabric_world_matrices, positions_wp, orientations_wp, scales_wp,
                indices_wp, self._view_to_fabric,
            ],
            device=self._fabric_device,
        )
    ...
    def _initialize_fabric(self) -> None:
        ...
        wp.launch(kernel=fabric_utils.arange_k, dim=self.count, inputs=[self._default_view_indices], device=self._device)
        wp.synchronize()
