...
class IsaacRtxRendererGlobalSettingsCfg:
    ...
    enable_translucency: bool | None = config_field(None)
    """Enable translucency for specular transmissive surfaces such as glass."""

    enable_reflections: bool | None = config_field(None)
    """Enable reflections."""

    enable_global_illumination: bool | None = config_field(None)
    """Enable diffuse global illumination."""

    antialiasing_mode: Literal["Off", "FXAA", "DLSS", "TAA", "DLAA"] | None = config_field(None)
    """Anti-aliasing mode selected through Replicator."""

    enable_dlssg: bool | None = config_field(None)
    """Enable DLSS frame generation."""

    enable_dl_denoiser: bool | None = config_field(None)
    """Enable DL denoiser."""

    dlss_mode: Literal[0, 1, 2, 3] | None = config_field(None)
    """DLSS performance/quality mode."""

    enable_direct_lighting: bool | None = config_field(None)
    """Enable direct light contributions from lights."""

    samples_per_pixel: int | None = config_field(None)
    """Direct lighting samples per pixel."""

    enable_shadows: bool | None = config_field(None)
    """Enable shadow rendering."""

    enable_ambient_occlusion: bool | None = config_field(None)
    """Enable ambient occlusion."""

    dome_light_upper_lower_strategy: Literal[0, 3, 4] | None = config_field(None)
    """Dome light sampling strategy."""

    max_bounces: int | None = config_field(None)
    """Maximum number of ray bounces for RTPT."""

    split_glass: bool | None = config_field(None)
    """Enable separate glass ray splitting."""

    split_clearcoat: bool | None = config_field(None)
    """Enable separate clearcoat ray splitting."""

    split_rough_reflection: bool | None = config_field(None)
    """Enable separate rough-reflection ray splitting."""

    ambient_light_intensity: float | None = config_field(None)
    """Scene ambient light intensity."""

    ambient_occlusion_denoiser_mode: Literal[0, 1] | None = config_field(None)
    """Ambient occlusion denoiser mode."""

    subpixel_mode: int | None = config_field(None)
    """RTX subpixel mode."""

    enable_cached_raytracing: bool | None = config_field(None)
    """Enable cached ray tracing."""

    max_samples_per_launch: int | None = config_field(None)
    """Path tracing maximum samples per launch."""

    view_tile_limit: int | None = config_field(None)
    """Maximum number of view tiles."""

    show_all_partitions_by_default: bool | None = config_field(None)
    ...
    carb_settings: dict[str, Any] | None = config_field(None)
...
class IsaacRtxRendererCfg(RendererCfg):
    ...
    class_type: type[IsaacRtxRenderer] | str = config_field("{DIR}.isaac_rtx_renderer:IsaacRtxRenderer")
    """Renderer implementation class."""

    renderer_type: str = config_field("isaac_rtx")
    """Type identifier for Isaac RTX renderer."""

    global_settings: IsaacRtxRendererGlobalSettingsCfg = config_field(IsaacRtxRendererGlobalSettingsCfg())
