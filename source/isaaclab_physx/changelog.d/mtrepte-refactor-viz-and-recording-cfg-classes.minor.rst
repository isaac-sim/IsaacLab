...
* **Breaking:** Removed ``eye`` and ``lookat`` fields from
  :class:`~isaaclab_physx.video_recording.IsaacsimKitPerspectiveVideoCfg`. The Kit perspective
  recorder no longer repositions ``/OmniverseKit_Persp``; camera placement is the sole
  responsibility of :class:`~isaaclab_visualizers.kit.KitVisualizer`.
