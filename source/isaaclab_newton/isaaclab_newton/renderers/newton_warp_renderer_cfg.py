...
class NewtonWarpRendererCfg(RendererCfg):
    ...
    """Whether to colorize the instance id segmentation image. Defaults to True.

    If True, :attr:`instance_id_segmentation_fast` is returned as an ``(N, H, W, 4) uint8`` RGBA image,
    else an ``(N, H, W, 1) int32`` id image. ``data.info["instance_id_segmentation_fast"]["idToLabels"]``
    maps the id/color to the leaf USD prim path.
    """
