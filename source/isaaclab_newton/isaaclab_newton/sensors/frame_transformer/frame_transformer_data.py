...
class FrameTransformerData(BaseFrameTransformerData):
    ...
    """Ordered target frame names, or ``None`` before initialisation."""
    ...
    def _create_buffers(self, num_envs: int, num_targets: int, device: str):
        ...
        self._source_pos_w: wp.array  # vec3f (num_envs,)
        self._source_quat_w: wp.array  # quatf (num_envs,)
        self._source_pos_w, self._source_quat_w = transform_to_vec_quat(self._source_transforms)

        self._target_pos_source: wp.array  # vec3f (num_envs, num_targets)
        self._target_quat_source: wp.array  # quatf (num_envs, num_targets)
        self._target_pos_source, self._target_quat_source = transform_to_vec_quat(self._target_transforms)

        self._target_pos_w: wp.array  # vec3f (num_envs, num_targets)
        self._target_quat_w: wp.array  # quatf (num_envs, num_targets)
