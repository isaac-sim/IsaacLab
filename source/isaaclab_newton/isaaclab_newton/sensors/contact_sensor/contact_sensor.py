...
class ContactSensor(BaseContactSensor):
    ...
    def reset(self, env_ids: Sequence[int] | None = None, env_mask: wp.array | None = None):
        ...
        if env_ids is None and env_mask is None:
            env_mask = wp.full(shape=self._num_envs, value=True, dtype=wp.bool, device=self._device)
        elif env_mask is None and env_ids is not None:
    ...
    def find_sensors(
        ...
        Raises:
            ValueError: If sensor_names is None (sensor not initialized).
        """
        if self.sensor_names is None:
            raise ValueError("Sensor names not available. Is the sensor initialized?")
    ...
    def _update_outdated_buffers(self):
        ...
        self._update_buffers_impl_mask(self._is_outdated)
    ...
    def _initialize_impl(self):
        ...
        if num_envs is None:
            raise RuntimeError("NewtonManager._num_envs is None. Is the simulation initialized?")
        self._num_envs = num_envs
        self._sim_physics_dt = wp.full(
            shape=self._num_envs, value=sim.get_physics_dt(), dtype=wp.float32, device=self._device
        )
        self._is_outdated = wp.full(shape=self._num_envs, value=True, dtype=wp.bool, device=self._device)
        self._timestamp = wp.zeros(shape=self._num_envs, dtype=wp.float32, device=self._device)
    ...
    def _update_buffers_impl_mask(self, env_mask: wp.array):
        """Fills the buffers of the sensor data (mask-based API for Newton).
    ...
    def find_bodies(
        ...
        """Find bodies (sensors) by name. Returns (indices, names) for compatibility with base class."""
        _, names, indices = self.find_sensors(name_keys, preserve_order)
