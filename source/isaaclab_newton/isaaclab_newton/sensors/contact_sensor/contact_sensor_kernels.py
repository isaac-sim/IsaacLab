...
@wp.kernel
def reset_sensor_state_kernel(
    env_mask: wp.array(dtype=wp.bool),
    is_outdated: wp.array(dtype=wp.bool),
    timestamp: wp.array(dtype=wp.float32),
    timestamp_last_update: wp.array(dtype=wp.float32),
):
    """Reset sensor state (timestamps and outdated flags).
...
def update_timestamp_kernel(
    ...
    timestamp: wp.array(dtype=wp.float32),
    timestamp_last_update: wp.array(dtype=wp.float32),
    sim_physics_dt: wp.array(dtype=wp.float32),
    update_period: wp.float32,
):
    """Update timestamps and mark environments as outdated if needed.

    Launch with dim=(num_envs,).
    """
    i = wp.tid()
    timestamp[i] = timestamp[i] + sim_physics_dt[i]
    if timestamp[i] - timestamp_last_update[i] >= update_period:
        is_outdated[i] = True
...
def update_outdated_envs_kernel(
    ...
    """Update timestamp_last_update and clear outdated flag for outdated environments.

    Launch with dim=(num_envs,).
    """
    i = wp.tid()
    if is_outdated[i]:
        timestamp_last_update[i] = timestamp[i]
        is_outdated[i] = False
