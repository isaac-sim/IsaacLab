...
class NewtonSiteFrameView(BaseFrameView):
    ...
    def get_world_scales(self, indices: wp.array | None = None) -> ProxyArray:
        """Get per-site world scales from the first collision shape on the same body.

        Newton's ``shape_scale`` is an absolute (world-space) quantity.
        """
        model = NewtonManager.get_model()
    ...
    def get_local_scales(self, indices: wp.array | None = None) -> ProxyArray:
        """Get per-site local scales.

        Newton does not distinguish local from world scale (``shape_scale`` is
        absolute).  Returns the same value as :meth:`get_world_scales`.
        """
        return self.get_world_scales(indices)

    def set_world_scales(self, scales: wp.array, indices: wp.array | None = None) -> None:
        """Set per-site world scales by writing to all collision shapes on the same body.
    ...
    def set_local_scales(self, scales: wp.array, indices: wp.array | None = None) -> None:
        """Set per-site local scales.

        Newton does not distinguish local from world scale (``shape_scale`` is
        absolute).  Delegates to :meth:`set_world_scales`.
        """
