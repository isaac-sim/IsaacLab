...
class NewtonMPMManager(NewtonManager):
    ...
    def reset_solver_state(
        ...
        if solver is None or NewtonManager._backend is None or not cls._implicit_mpm_solvers():
        ...
        candidates = (state,) if state is not None else (NewtonManager._backend.state_1, NewtonManager._backend.state_0)
        states: list[State] = []
        seen: set[int] = set()
