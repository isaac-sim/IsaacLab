...
class NewtonSceneDataBackend(SceneDataBackend):
    ...
    def __init__(self):
        ...
        self._transform_publication = SceneDataPublication(self._scene_data)

    @property
    def transform_publication(self) -> SceneDataPublication:
        """Publish the current native pointer, including physics state-buffer swaps."""
    ...
    def state(self) -> State:
        """Return the authoritative physics state without consulting renderer resources."""
        return NewtonManager._backend.state_0
...
class NewtonManager(PhysicsManager):
    ...
    Class-level manager that owns solver lifecycle, contacts, physics sensors,
    and stepping CUDA graphs. The simulation registry owns its native model and state.
    ...
    The simulation registry owns the native Newton resource. This manager retains
    solver configuration, lifecycle callbacks, and stepping orchestration.
    """
    ...
    _backend: NewtonReplicateContext | None = None
    ...
    def initialize(cls, sim_context: SimulationContext) -> None:
        ...
        NewtonManager._backend = sim_context.get_or_create_backend(NewtonReplicateContext, sim_context)
        if has_kit():
            NewtonManager._backend._capture_graph = cls._capture_relaxed_graph
    ...
    def _eval_fk_impl(cls, world_reset_mask: wp.array | None, fk_mask: wp.array | None) -> None:
        ...
        eval_fk(
            cls._backend.model,
            cls._backend.state_0.joint_q,
            cls._backend.state_0.joint_qd,
            cls._backend.state_0,
            fk_mask,
        )
    ...
    def forward(cls) -> None:
        ...
        if cls._transforms_may_change_on_graph_replay:
            # Captured authored writes can replay without a Python notification.
    ...
    def sync_cables_to_usd(cls) -> None:
        ...
        try:
            ...
            wp.copy(body_q, cls._backend.state_0.body_q)
            wp.launch(
                _sync_cable_points,
    ...
    def sync_particles_to_usd(cls) -> None:
        ...
        * ``UsdGeom.Points`` prims registered through
        ...
        last sync.
        """
        if not cls._particles_dirty or cls._backend.state_0 is None or cls._backend.state_0.particle_q is None:
            return
        try:
    ...
    def _sync_fabric_mesh_particles(cls) -> None:
        ...
        wp.launch(
            ...
            inputs=[
                ...
                cls._backend.state_0.particle_q,
            ],
    ...
    def _sync_particle_points_prims(cls) -> bool:
        ...
        if due:
            ...
            particle_q = cls._backend.state_0.particle_q.numpy()
        ...
        **Eager-actuator path** (fallback, some actuators not graph-safe):
        ...
        if cls._is_all_graphable():
            ...
            if cls._adapter is not None:
                cls._adapter.step(cls._backend.state_0, cls._backend.control, physics_dt)
    ...
    def invalidate_fk(
        ...
        if articulation_ids is not None and env_mask is not None:
            ...
            NewtonManager._world_reset_mask[: cls._backend.model.world_count].fill_(True)
    ...
    def start_simulation(cls) -> None:
        ...
        """
        logger.debug(f"Builder: {cls._backend.builder}")
        ...
        cls._register_builder_attributes(cls._backend.builder)
        ...
        cls._backend.builder.up_axis = Axis.from_string(cls._up_axis)
        ...
        if cls._pending_extended_state_attributes:
            cls._backend.builder.request_state_attributes(*cls._pending_extended_state_attributes)
            NewtonManager._active_extended_state_attributes |= cls._pending_extended_state_attributes
            NewtonManager._pending_extended_state_attributes = set()
        cls._prepare_builder_for_finalize(cls._backend.builder)
        with Timer(name="newton_finalize_builder", msg="Finalize builder took:", activity="Finalizing physics model"):
            cls._backend.finalize()
            cls._backend.model.set_gravity(cls._gravity_vector)
            if cls._num_envs is None:
                NewtonManager._num_envs = cls._backend.model.world_count
            cls._backend.model.num_envs = cls._num_envs

        if cls._pending_extended_contact_attributes:
            cls._backend.model.request_contact_attributes(*cls._pending_extended_contact_attributes)
            NewtonManager._pending_extended_contact_attributes = set()
        ...
        NewtonManager._world_reset_mask = wp.zeros(cls._backend.model.world_count + 1, dtype=wp.bool, device=device)
        NewtonManager._fk_reset_mask = wp.zeros(cls._backend.model.articulation_count, dtype=wp.bool, device=device)
    ...
    def _initialize_fabric_cable_prims(cls, stage, fabric_hierarchy, usdrt) -> None:
        ...
        NewtonManager._cable_sync_cpu_buffers = (
            ...
            cls._backend.model.shape_body.to("cpu"),
            wp.empty_like(cls._backend.state_0.body_q, device="cpu"),
            cls._backend.model.shape_transform.to("cpu"),
            cls._backend.model.shape_scale.to("cpu"),
        )
    ...
    def _initialize_contacts(cls) -> None:
        ...
        if cls._collision_pipeline is None:
            NewtonManager._collision_pipeline = CollisionPipeline(cls._backend.model, **pipeline_args)
    ...
    def initialize_solver(cls) -> None:
        ...
        with Timer(name="newton_initialize_solver", msg="Initialize solver took:", activity="Initializing solver"):
            ...
            cls._build_solver(cls._backend.model, cfg.solver_cfg)  # type: ignore[union-attr]
        ...
        if cls._use_single_state:
            for i in range(cls._num_substeps):
                for callback in cls._state_force_callbacks:
                    callback(cls._backend.state_0)
                cls._step_solver(
                    cls._backend.state_0, cls._backend.state_0, cls._backend.control, contacts, cls._solver_dt
                )
                cls._backend.state_0.clear_forces()
                if collide_mid_loop and (i + 1) % collide_every == 0 and i + 1 < cls._num_substeps:
                    cls._collision_pipeline.collide(cls._backend.state_0, contacts)
        else:
            cfg = PhysicsManager._cfg
            ...
            for i in range(cls._num_substeps):
                for callback in cls._state_force_callbacks:
                    callback(cls._backend.state_0)
                cls._step_solver(
                    cls._backend.state_0, cls._backend.state_1, cls._backend.control, contacts, cls._solver_dt
                )
                if need_copy_on_last and i == cls._num_substeps - 1:
                    cls._backend.state_0.assign(cls._backend.state_1)
                else:
                    NewtonManager._backend.state_0, NewtonManager._backend.state_1 = (
                        cls._backend.state_1,
                        cls._backend.state_0,
                    )
                cls._backend.state_0.clear_forces()
                if collide_mid_loop and (i + 1) % collide_every == 0 and i + 1 < cls._num_substeps:
    ...
    def _update_sensors(cls, contacts) -> None:
        ...
        if cls._newton_frame_transform_sensors:
            for sensor in cls._newton_frame_transform_sensors:
                sensor.update(cls._backend.state_0)
        ...
        if cls._report_contacts:
            ...
            cls._solver.update_contacts(eval_contacts, cls._backend.state_0)
            for sensor in cls._newton_contact_sensors.values():
                sensor.update(cls._backend.state_0, eval_contacts)
    ...
    def _simulate_full(cls) -> None:
        ...
        for _ in range(cls._decimation):
            if cls._needs_collision_pipeline:
                cls._collision_pipeline.collide(cls._backend.state_0, cls._contacts)
    ...
    def get_model(cls) -> Model:
        """Return the model built by the simulation's Newton clone context."""
        sim = SimulationContext.instance()
        return sim.get_or_create_backend(NewtonReplicateContext, sim).model

    @classmethod
    def get_state_0(cls) -> State:
        """Return the current native state."""
        sim = SimulationContext.instance()
        return sim.get_or_create_backend(NewtonReplicateContext, sim).state_0

    @classmethod
    def get_state(cls, scene_data_provider: SceneDataProvider | None = None) -> State:
        """Return the shared Newton state, with render transforms supplied by SDP."""
        sim = SimulationContext.instance()
        resource = sim.get_or_create_backend(NewtonReplicateContext, sim)
        if resource.physics_cfg is not None and cls._fk_reset_mask is not None:
            cls.forward()
        resource.update_transforms()
        return resource.state_0
    ...
    def _register_sensor_task(cls, name: str, update_fn: Callable[[], None], *, graph_capturable: bool = True) -> None:
        cls._backend._register_sensor_task(name, update_fn, graph_capturable=graph_capturable)

    @classmethod
    def _unregister_sensor_task(cls, name: str) -> None:
        cls._backend._unregister_sensor_task(name)

    @classmethod
    def _update_sensor_tasks(cls, *names: str) -> None:
        cls.forward()
        cls._backend._update_sensor_tasks(*names)

    @classmethod
    def _mark_sensor_state_dirty(cls) -> None:
        if cls._scene_data_backend is not None:
            cls._scene_data_backend.transform_publication.dirty = True
        if cls._backend is not None:
            cls._backend._mark_sensor_state_dirty()
    ...
    def get_state_1(cls) -> State:
        """Get the next state."""
        return cls._backend.state_1

    @classmethod
    def get_control(cls) -> Control:
        """Get the control object."""
        return cls._backend.control
    ...
    def activate_newton_actuator_path(cls) -> None:
        ...
        if cls._adapter is not None:
            return
        if cls._backend.model is None or not cls._backend.model.actuators:
            return
        from isaaclab.actuators.newton import NewtonActuatorAdapter  # noqa: PLC0415

        dofs_per_env = cls._backend.model.joint_dof_count // cls._num_envs
        NewtonManager._adapter = NewtonActuatorAdapter(
            actuators=list(cls._backend.model.actuators),
            num_envs=cls._num_envs,
            num_joints=dofs_per_env,
            dof_offset=0,
            device=PhysicsManager._device,
        )
        cls._adapter.finalize(cls._backend.control)
