...
class VBDSolverCfg(NewtonSolverCfg):
    ...
    class_type: type[NewtonManager] | str = config_field("{DIR}.vbd_manager:NewtonVBDManager")
    """Manager class for the VBD solver."""

    iterations: int = config_field(10)
    """Number of VBD iterations per substep."""

    integrate_with_external_rigid_solver: bool = config_field(False)
    """Whether an external solver integrates rigid bodies."""

    particle_enable_self_contact: bool = config_field(False)
    """Whether to enable particle self-contact."""

    particle_self_contact_radius: float = config_field(0.005)
    """Particle radius used for self-contact detection [m]."""

    particle_self_contact_margin: float = config_field(0.005)
    """Self-contact detection margin [m]."""

    particle_collision_detection_interval: int = config_field(-1)
    ...
    particle_vertex_contact_buffer_size: int = config_field(32)
    """Preallocation size for each vertex contact buffer."""

    particle_edge_contact_buffer_size: int = config_field(64)
    """Preallocation size for each edge contact buffer."""

    particle_topological_contact_filter_threshold: int = config_field(2)
    """Topological distance below which self-contacts are discarded."""

    particle_rest_shape_contact_exclusion_radius: float = config_field(0.0)
    """Rest-shape separation threshold for filtering contacts [m]."""

    rigid_contact_k_start: float = config_field(1.0e2)
    """Initial stiffness seed for rigid-body contacts [N/m]."""

    rigid_body_particle_contact_buffer_size: int = config_field(256)
