...
def test_newton_volume_anchor_applies_token_schemas_and_syncs_vis_mesh():
    ...
    NewtonManager.setup_deformable_body(body, "volume", sim_mesh, vis_mesh, stage)
    assert "PhysicsDeformableBodyAPI" in body.GetPrimTypeInfo().GetAppliedAPISchemas()
    assert "PhysicsVolumeDeformableSimAPI" in sim_mesh.GetPrimTypeInfo().GetAppliedAPISchemas()
    # vis mesh geometry overwritten from the tet surface
    assert len(vis_mesh.GetAttribute("points").Get()) == 4
    assert vis_mesh.GetAttribute("faceVertexCounts").Get() is not None
