...
def native_resource(monkeypatch):
    monkeypatch.setattr(NewtonManager, "_backend", SimpleNamespace(model=None, state_0=None, state_1=None))
...
def test_mpm_manager_explicit_reset_accepts_canonical_mask(cpu_mpm_solver_and_state, monkeypatch):
    ...
    monkeypatch.setattr(NewtonManager._backend, "model", solver.model)
...
def test_mpm_manager_explicit_reset_promotes_selected_single_world(monkeypatch):
    ...
    monkeypatch.setattr(NewtonManager, "_solver", solver)
    monkeypatch.setattr(NewtonManager._backend, "model", SimpleNamespace(world_count=1))
...
def test_mpm_manager_explicit_reset_deduplicates_manager_states(monkeypatch):
    ...
    solver = SimpleNamespace(reset=lambda *args, **kwargs: calls.append((args, kwargs)))
    monkeypatch.setattr(NewtonManager, "_solver", solver)
    monkeypatch.setattr(NewtonManager._backend, "model", SimpleNamespace(world_count=2))
    ...
    calls.clear()
    monkeypatch.setattr(NewtonManager._backend, "state_1", state_0)
