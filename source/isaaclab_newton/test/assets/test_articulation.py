...
"""Kitless real-solver integration tests for Newton articulations."""
...
def _newton_sim_context(*, device: str = "cpu", use_newton_actuators: bool = False):
...
def _author_two_link_articulations(*, actuators: dict | None = None) -> Articulation:
    """Author two local one-DOF articulations without files or remote assets."""
...
def test_articulation_real_newton_seams(monkeypatch) -> None:
    """Exercise the minimal real Newton articulation adapter with isolated partial writes."""
    with _newton_sim_context() as sim:
        ...
        target_root_pose[:, :3] += torch.tensor([0.2, -0.1, 0.3])
        target_joint_pos = torch.tensor([[0.25]], dtype=torch.float32)
        target_joint_vel = torch.tensor([[-0.5]], dtype=torch.float32)
        ...
        coms = torch.tensor([[[0.05, -0.02, 0.01]]])
        ...
        inertias = torch.diag_embed(torch.tensor([[[2.0, 3.0, 4.0]]])).reshape(1, 1, 9)
        ...
        articulation.permanent_wrench_composer.set_forces_and_torques_index(
            forces=torch.tensor([[[8.0, 0.0, 0.0]]]),
...
def test_articulation_cuda_jacobian_and_mass_access() -> None:
    """Smoke-test Newton's CUDA-backed Jacobian and mass-matrix adapter access."""
    with _newton_sim_context(device="cuda:0") as sim:
        articulation = _author_two_link_articulations()
        sim.reset()

        jacobians = articulation.data.body_link_jacobian_w.torch
        mass_matrix = articulation.data.mass_matrix.torch
        assert jacobians.device.type == "cuda"
        assert mass_matrix.device.type == "cuda"
        assert jacobians.shape == (2, 2, 6, 7)
        assert mass_matrix.shape == (2, 7, 7)
        assert torch.isfinite(jacobians).all()
        assert torch.isfinite(mass_matrix).all()
