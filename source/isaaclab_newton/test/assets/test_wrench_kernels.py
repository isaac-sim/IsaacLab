...
def _body_link_poses_w(quaternions: list[tuple[float, float, float, float]]) -> wp.array:
    """Create one-environment body-link poses with zero translations."""
    poses = np.asarray([[[0.0, 0.0, 0.0, *quaternion] for quaternion in quaternions]], dtype=np.float32)
...
def test_update_wrench_array_rotates_body_wrenches_to_world_frame() -> None:
    ...
    body_link_pose_w = _body_link_poses_w([_QUARTER_TURN_Z_QUAT])
...
def test_update_wrench_array_ordered_rotates_and_scatter_wrenches_to_backend_order() -> None:
    ...
    forces = wp.array(
        np.asarray([[[1.0, 0.0, 0.0], [3.0, 0.0, 0.0]]], dtype=np.float32), dtype=wp.vec3f, device="cpu"
    )
    torques = wp.array(
        np.asarray([[[2.0, 0.0, 0.0], [4.0, 0.0, 0.0]]], dtype=np.float32), dtype=wp.vec3f, device="cpu"
    )
    # Link poses are backend ordered: public body 1 is backend body 0.
    body_link_pose_w = _body_link_poses_w([_QUARTER_TURN_Z_QUAT, _IDENTITY_QUAT])
