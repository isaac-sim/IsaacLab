...
def _make_anymal_d_env(visualizer_kind: str | tuple[str, ...], backend_kind: str, *, tiled_camera: bool = False):
    ...
    preset_key = "newton_mjwarp" if backend_kind == "newton" else "isaacsim_physx"
    env_cfg.sim.physics = getattr(env_cfg.sim.physics, preset_key)
    env_cfg.scene.num_envs = (
...
def _make_cartpole_camera_env(
    ...
    # Pin the camera renderer: these goldens compare visualizer output, so the frames must not
    # shift when the MultiBackendRendererCfg default changes.
