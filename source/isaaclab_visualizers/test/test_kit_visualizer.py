...
"""Unit tests for Kit frame-boundary scene synchronization."""
...
def test_step_forwards_scene_before_app_update():
    visualizer = _make_visualizer()
    events = []
    sim = Mock()
    sim.render_context.scene_state_revision = 1
    ...
    assert events == ["forward", "update"]
    assert visualizer._app_pumped_this_step is True
    sim.render_context.mark_scene_state_rendered.assert_called_once_with(1)
    settings.set_bool.assert_has_calls([call(_PLAY_SIMULATIONS_SETTING, False), call(_PLAY_SIMULATIONS_SETTING, True)])


def test_render_rgb_array_forwards_scene_for_on_demand_update():
    visualizer = _make_visualizer()
...
def test_render_rgb_array_repumps_after_post_step_pose_write():
    ...
    events = []
    sim = Mock()
    sim.render_context.scene_state_is_rendered = True
    sim.render_context.scene_state_revision = 0
    sim.physics_manager.forward.side_effect = lambda: events.append("forward")
    app = Mock()
    app.update.side_effect = lambda: events.append("update")
    settings = Mock()
    ...
    with (
        ...
        sim.render_context.scene_state_is_rendered = False
        sim.render_context.scene_state_revision = 1
...
def test_kit_app_update_forwards_before_frame():
    visualizer = _make_visualizer()
    sim = Mock()
    sim.render_context.scene_state_revision = 0
    sim.render_context.scene_state_is_rendered = False
    ...
    sim.physics_manager.forward.assert_called_once_with()
    app.update.assert_called_once_with()
    sim.render_context.mark_scene_state_rendered.assert_called_once_with(0)


def test_camera_pose_sync_does_not_forward_scene_without_running_app():
    visualizer = _make_visualizer()
    sim = Mock()
    app = Mock()
...
def test_kit_app_update_restores_play_state_on_failure():
    ...
    sim.render_context.mark_scene_state_rendered.assert_not_called()
