...
def submit_jobs_to_clusters(jobs, clusters, test_mode):
    ...
    with ThreadPoolExecutor(max_workers=len(clusters)) as executor:
        for idx, cluster in enumerate(clusters):
            job_command = jobs[idx] if idx < len(jobs) else None  # Allow for running test mode without specific commands
...
if __name__ == "__main__":
    ...
    parser.add_argument("--jobs", nargs='*', required=False, help="Job commands to run on clusters, enclosed in quotes.")
