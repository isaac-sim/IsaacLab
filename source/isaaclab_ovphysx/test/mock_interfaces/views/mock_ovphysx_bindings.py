...
class MockTensorBinding:
    ...
    def read(self, tensor) -> None:
        """Copy internal data into the provided array."""
    ...
    def write(self, tensor, indices=None, mask=None) -> None:
        """Copy from the provided array into internal data."""
        np_src = np.asarray(tensor).astype(np.float32)
        if indices is not None:
            idx = np.asarray(indices)
            self._data[idx] = np_src
        elif mask is not None:
            np_mask = np.asarray(mask).astype(bool)
