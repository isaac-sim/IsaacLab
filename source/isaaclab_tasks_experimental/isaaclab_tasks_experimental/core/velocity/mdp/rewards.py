...
def _feet_air_time_kernel(
    ...
    """Accumulate per-foot (air time minus threshold) at first contact, gated by command magnitude."""
    i = wp.tid()
    s = float(0.0)
...
def _feet_air_time_positive_biped_kernel(
    ...
    """Compute the single-stance minimum foot mode time (air/contact) gated by command magnitude."""
    i = wp.tid()
...
def _feet_slide_kernel(
    ...
    """Accumulate each foot's planar velocity while it is in contact into the sliding penalty output."""
    i = wp.tid()
    s = float(0.0)
    for k in range(body_ids.shape[0]):
        b = body_ids[k]
...
def _track_lin_vel_xy_yaw_frame_exp_kernel(
    ...
    """Compute the exponential xy linear-velocity tracking reward in the yaw-aligned frame."""
    i = wp.tid()
    q = root_quat_w[i]
...
def _track_ang_vel_z_world_exp_kernel(
    ...
    """Compute the exponential yaw angular-velocity tracking reward in world frame."""
    i = wp.tid()
...
def _stand_still_joint_deviation_l1_kernel(
    ...
    """Accumulate L1 joint deviation from defaults, gated to when the command is below threshold."""
    i = wp.tid()
