...
def survival_success_rate(env: ManagerBasedRLEnv, out) -> None:
    """Write a zero reward for the stable Cartpole success-rate metric term.

    The stable term logs the episode survival rate during reset. That logging
    requires a host readback of the time-out mask and is not compatible with
    the Warp reward manager's CUDA-graph-captured reset. This twin preserves
    the term's zero reward without performing the diagnostic readback.
    """
    out.zero_()
