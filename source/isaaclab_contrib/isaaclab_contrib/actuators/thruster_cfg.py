...
class ThrusterCfg:
    ...
    and initial state specifications). Fields left as ``REQUIRED`` are required
    and must be provided by the user configuration.
    """

    class_type: type["Thruster"] | str = "isaaclab_contrib.actuators.thruster:Thruster"
    """Concrete Python class that consumes this config."""

    dt: float = REQUIRED
    """Simulation/integration timestep used by the thruster update [s]."""

    thrust_range: tuple[float, float] = REQUIRED
    ...
    thruster_names_expr: list[str] = REQUIRED
