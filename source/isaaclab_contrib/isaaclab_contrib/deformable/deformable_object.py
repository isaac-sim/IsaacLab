...
def read_deformable_entry(template_prim: Usd.Prim, prim_path: str) -> DeformableRegistryEntry:
    """Read the same authored deformable definition for spawning and deployment loading."""
...
def add_deformable_entry_to_builder(
    ...
    starts = {name: getattr(builder, name + "_count") for name in ("particle", "tri", "edge", "tet")}
...
def add_exported_deformables_to_builder(stage, builder) -> dict:
    ...
    for mesh in stage.Traverse():
        ...
        while root and not any("DeformableBodyAPI" in api for api in root.GetPrimTypeInfo().GetAppliedAPISchemas()):
            root = root.GetParent()
        if not root:
            raise ValueError(f"Missing deformable body for {mesh.GetPath()}.")
        entry = read_deformable_entry(root, str(root.GetPath()))
