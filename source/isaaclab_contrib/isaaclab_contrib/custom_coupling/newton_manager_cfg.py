...
class CoupledMJWarpVBDSolverCfg(NewtonSolverCfg):
    ...
    rigid_solver_cfg: MJWarpSolverCfg = field(default_factory=MJWarpSolverCfg)
    """MJWarp rigid-body solver configuration."""

    soft_solver_cfg: VBDSolverCfg = field(
        default_factory=lambda: VBDSolverCfg(integrate_with_external_rigid_solver=True)
    )
