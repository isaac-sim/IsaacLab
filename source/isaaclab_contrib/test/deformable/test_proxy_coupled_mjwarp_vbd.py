...
"""Pure-Python unit tests for proxy-coupled MJWarp+VBD partitioning logic.

These tests intentionally do NOT launch :class:`isaaclab.app.AppLauncher`.
:class:`NewtonProxyCoupledMJWarpVBDManager`'s body/joint/shape partitioning and
...
def test_partition_splits_bodies_joints_shapes():
    ...
    mjc_b, vbd_b, mjc_j, vbd_j, mjc_s, vbd_s = NewtonProxyCoupledMJWarpVBDManager._partition_model_by_entities(
        model,
        mjwarp_bodies=[SceneEntityCfg("robot", body_names=["panda_link0"])],
        vbd_bodies=[SceneEntityCfg("other", body_names=["panda_hand"])],
        scene_cfg=scene,
    )

    assert mjc_b == [0]
    assert vbd_b == [1]
    # Joint 0's child is body 1 (VBD partition) → the joint index lands in VBD.
    assert mjc_j == []
    assert vbd_j == [0]
    # Shape 0 → body 0 (MJC). Shape 1 → body 1 (VBD). Shape 2 → body -1 (static, → VBD).
    assert mjc_s == [0]
    assert vbd_s == [1, 2]
