...
def rendering_test_franka_cable_partition_visibility(physics_backend: str, renderer: str) -> None:
    """A deforming cable must stay visible after moving outside its initial extent.

    Regression guard for OMPE-105749: Kit RTX computes the bounding box of a ``UsdGeom.BasisCurves``
    prim once and never refreshes it as the curve deforms. A per-environment scene partition sized
    from that stale box culls the cable as soon as it animates outside the box, so the cable vanishes
    from the tiled camera images while still being in frame.

    The Franka cable env pins its partition to the whole workspace with two marker cubes
    (``partition_bounds_marker_min`` / ``_max``) so the cable cannot leave the partition bounds.

    Args:
        physics_backend: Physics backend label, as used by the golden AOV tests.
        renderer: Renderer preset name, e.g. ``"isaacsim_rtx_renderer"``.
    """
    from isaaclab.envs import ManagerBasedRLEnv
    ...
    try:
        ...
        for _ in range(2):
            env.step(zero_actions)
        settled_pixels = _cable_visible_pixel_count(camera)
        ...
        assert abs(_CABLE_CULL_PROBE_OFFSET_Y) > initial_span_y, (
            f"Probe offset {_CABLE_CULL_PROBE_OFFSET_Y} m no longer leaves the cable's initial y extent"
            f" of {initial_span_y} m, so it cannot trip the stale-bounding-box cull."
        )

        moved_pose = default_pose.clone()
        moved_pose[..., 1] += _CABLE_CULL_PROBE_OFFSET_Y
        ...
        moved_pixels = _cable_visible_pixel_count(camera)
        assert moved_pixels > 0, (
            f"Cable vanished after moving {_CABLE_CULL_PROBE_OFFSET_Y} m in y ({settled_pixels} px at"
            f" spawn, {moved_pixels} px after the move). The scene partition is culling the deforming"
            " curve - check that the partition-bounds markers are still spawned and still envelop the"
            " probe pose (OMPE-105749)."
        )
    finally:
        if env is not None:
            env.close()
