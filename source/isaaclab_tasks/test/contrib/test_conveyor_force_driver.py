...
"""Lifecycle tests for the Newton conveyor force driver."""
...
def _belt_spec(name: str = "Belt") -> ConveyorBeltSpec:
    ...
    return ConveyorBeltSpec(prim_path=f"{{ENV_REGEX_NS}}/{name}", velocity=0.35, friction_coefficient=0.5)
...
class _FakeBinding:
    ...
    def __init__(self, model, contacts, **kwargs) -> None:
        ...
        self._friction_host = np.full(2, 0.5, dtype=np.float32)
        self._threshold_host = np.full(2, 0.997, dtype=np.float32)
    ...
    def set_friction_coefficients(self, values) -> None:
        self._friction_host = np.asarray(values, dtype=np.float32).copy()

    def set_contact_processing_thresholds(self, values) -> None:
        self._threshold_host = np.asarray(values, dtype=np.float32).copy()
...
def test_driver_requests_force_and_rebinds_on_solver_reinitialization(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(driver_module.NewtonManager, "register_callback", classmethod(register_callback))
    monkeypatch.setattr(
        driver_module.NewtonManager,
    ...
    monkeypatch.setattr(driver_module, "_ConveyorForceBinding", _FakeBinding)

    driver = driver_module.ConveyorForceDriver(num_envs=2, belt_specs=(_belt_spec(),))

    assert driver.specs == (_belt_spec(),)
    assert driver.belts_per_env == 1
    assert driver.num_belts == 2
    assert driver.count == 2
    assert not driver.initialized

    assert [(event, name) for _, event, name in event_callbacks] == [
        (PhysicsEvent.MODEL_INIT, "conveyor_force_contact_attribute")
    ]
    ...
    first_binding.set_friction_coefficients([0.4, 0.6])
    first_binding.set_contact_processing_thresholds([0.98, 0.99])
    ...
    np.testing.assert_allclose(second_binding._friction_host, [0.4, 0.6])
    np.testing.assert_allclose(second_binding._threshold_host, [0.98, 0.99])
...
def test_unbound_driver_rejects_control_calls(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    driver = driver_module.ConveyorForceDriver(num_envs=1, belt_specs=(_belt_spec(),))
...
def test_driver_rejects_invalid_specs_before_registering_callbacks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match="At least one"):
        driver_module.ConveyorForceDriver(num_envs=1, belt_specs=())
    with pytest.raises(TypeError, match="ConveyorBeltSpec"):
        driver_module.ConveyorForceDriver(num_envs=1, belt_specs=(object(),))
    with pytest.raises(ValueError, match="unique"):
        driver_module.ConveyorForceDriver(num_envs=1, belt_specs=(_belt_spec(), _belt_spec()))
    with pytest.raises(ValueError, match="ancestors"):
        driver_module.ConveyorForceDriver(
            num_envs=1,
            belt_specs=(
                ConveyorBeltSpec(prim_path="{ENV_REGEX_NS}/Belt"),
                ConveyorBeltSpec(prim_path="{ENV_REGEX_NS}/Belt/Child"),
            ),
        )
    with pytest.raises(ValueError, match="explicit positive radius"):
        driver_module.ConveyorForceDriver(
            num_envs=1,
            belt_specs=(ConveyorBeltSpec(prim_path="{ENV_REGEX_NS}/Curve", curved=True),),
        )
    with pytest.raises(ValueError, match="Replicated conveyor environments"):
        driver_module.ConveyorForceDriver(
            num_envs=2,
            belt_specs=(ConveyorBeltSpec(prim_path="/World/Shared/Belt"),),
        )
    with pytest.raises(ValueError, match="env_path_format"):
        driver_module.ConveyorForceDriver(num_envs=1, belt_specs=(_belt_spec(),), env_path_format="/World/envs/env_.*")
...
def test_belt_paths_are_exact_and_environment_scoped() -> None:
    ...
    resolve = driver_module._resolve_belt_prim_path
    ...
    belongs = driver_module._shape_belongs_to_prim
...
def test_driver_cleans_up_model_callback_when_solver_registration_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="solver callback unavailable"):
        driver_module.ConveyorForceDriver(num_envs=1, belt_specs=(_belt_spec(),))
...
def test_binding_uses_deterministic_environment_major_belt_indices(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    binding = driver_module._ConveyorForceBinding(
        model=model,
        contacts=contacts,
        num_envs=2,
        belt_specs=specs,
        transported_body_count_per_env=1,
    )
    try:
        ...
        np.testing.assert_allclose(binding._friction_host, [0.4, 0.6, 0.4, 0.6])
        np.testing.assert_allclose(binding._threshold_host, [0.98, 0.99, 0.98, 0.99])
        np.testing.assert_array_equal(binding.get_enabled(indices=[3, 0]).numpy(), [0, 1])
        np.testing.assert_allclose(binding.get_friction_coefficients(indices=[1, 2]).numpy(), [0.6, 0.4])
        np.testing.assert_allclose(binding.get_contact_processing_thresholds(indices=[2, 1]).numpy(), [0.98, 0.99])
    finally:
        binding.close()
