...
def test_go2_rough_physics_presets_share_armature():
    """Go2 rough terrain keeps actuator conditioning identical across solvers."""
    from isaaclab_tasks.core.velocity.config.go2.rough_env_cfg import UnitreeGo2RoughEnvCfg

    armatures = []
    for preset_name in (None, "newton_mjwarp", "feather_pgs"):
        global_presets = [] if preset_name is None else [preset_name]
        env_cfg, _ = _apply(UnitreeGo2RoughEnvCfg(), global_presets=global_presets)
        armatures.append(env_cfg.scene.robot.actuators["base_legs"].armature)

    assert armatures == [armatures[0]] * len(armatures)
