...
from isaaclab_tasks.core.reach import mdp
...
def test_reach_uses_controller_neutral_franka_config_module():
    ...
    assert entry_point.endswith(".franka_env_cfg:FrankaReachEnvCfg")
...
def test_reach_action_presets_change_only_the_action_configuration():
    joint_pos_physx = _load_env_cfg("joint_pos")
    diffik_physx = _load_env_cfg("diffik")
...
def test_reach_success_requires_position_and_orientation():
    ...
    assert cfg.commands.ee_pose.position_success_threshold is None
    ...
    command = SimpleNamespace(robot=SimpleNamespace(data=robot_data), body_idx=0, command=command_values)
...
def test_reach_diffik_action_configuration_is_identical_across_backends():
    physx_action = _load_env_cfg("diffik").actions.arm_action
