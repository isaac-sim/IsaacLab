...
# =============================================================================
# Override order tests (Presets > Groups > Single field; depth; left > right)
# =============================================================================


def test_override_order_groups_override_global_presets(test_configs):
    """Path preset (Groups) overrides global preset (Presets) for the same path."""
    env_cfg, agent_cfg, presets = test_configs
    hydra_cfg = {"env": env_cfg.to_dict(), "agent": agent_cfg.to_dict()}

    # Global preset "noise_less" sets observations; path preset "fast" overrides for same path
    apply_overrides(
        env_cfg,
        agent_cfg,
        hydra_cfg,
        global_presets=["noise_less"],
        preset_sel=[("env", "observations", "fast")],
        preset_scalar=[],
        presets=presets,
    )

    # Path preset (fast) wins: concatenate_terms=False, not noise_less's True
    assert env_cfg.observations.concatenate_terms is False
    assert env_cfg.observations.enable_corruption is False


def test_override_order_single_field_overrides_preset(test_configs):
    """Scalar (Single field) overrides preset value."""
    env_cfg, agent_cfg, presets = test_configs
    hydra_cfg = {"env": env_cfg.to_dict(), "agent": agent_cfg.to_dict()}

    # RelativeJointPositionActionCfg has scale=0.2; scalar overrides to 9.0
    preset_sel = [("env", "actions.arm_action", "relative_joint_position")]
    preset_scalar = [("env.actions.arm_action.scale", "9.0")]
    apply_overrides(env_cfg, agent_cfg, hydra_cfg, [], preset_sel, preset_scalar, presets)

    assert env_cfg.actions.arm_action.scale == 9.0


def test_override_order_left_right_scalar_wins(test_configs):
    """For same target, rightmost scalar wins (left-to-right processing)."""
    _, _, presets = test_configs
    env_cfg = SampleEnvCfg()
    agent_cfg = SampleAgentCfg()
    hydra_cfg = {"env": env_cfg.to_dict(), "agent": agent_cfg.to_dict()}

    # Two scalars for same field - rightmost (2.0) should win
    preset_sel = [("env", "actions.arm_action", "relative_joint_position")]
    preset_scalar = [
        ("env.actions.arm_action.scale", "1.0"),
