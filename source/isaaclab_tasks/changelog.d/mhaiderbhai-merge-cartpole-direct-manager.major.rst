...
* **Breaking:** Renamed the cartpole Gym environment IDs so each spells out its workflow
  (``Direct`` or ``Manager``) and dropped the ``-v0`` version suffix. Update ``gym.make`` /
  ``--task`` calls:

  * ``Isaac-Cartpole-Direct-v0`` → ``Isaac-Cartpole-Direct``.
  * ``Isaac-Cartpole-Camera-Direct-v0`` → ``Isaac-Cartpole-Camera-Direct``.
  * ``Isaac-Cartpole-v0`` → ``Isaac-Cartpole-Manager``.
  * ``Isaac-Cartpole-Camera-v0`` → ``Isaac-Cartpole-Camera-Manager``.
