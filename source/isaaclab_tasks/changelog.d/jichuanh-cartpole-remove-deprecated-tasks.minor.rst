...
* **Breaking:** Removed 35 per-variant Cartpole task IDs (7 Direct camera,
  ...
  (``Isaac-Cartpole-Camera-Direct``, ``Isaac-Cartpole-Camera-Manager``,
  ...
  * ``Isaac-Cartpole-{RGB,Depth}-v0`` →
    ``--task=Isaac-Cartpole-Camera-Manager [presets=<rgb|depth>]``.
  * ``Isaac-Cartpole-RGB-{ResNet18,TheiaTiny}-v0`` →
    ``--task=Isaac-Cartpole-Camera-Manager --agent=rl_games_feature_cfg_entry_point presets=<resnet18|theia_tiny>``.
