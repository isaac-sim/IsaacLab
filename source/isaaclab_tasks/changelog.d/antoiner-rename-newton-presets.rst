...
* Renamed the Newton-backend solver presets to a ``newton_`` prefix so they
  group together in autocomplete and read distinctly from the Newton backend
  label, package, and visualizer. Migration: rename the field in any
...
* Deprecated the legacy ``newton`` and ``kamino`` preset names. They still
  ...
  :class:`FutureWarning` and will be removed in a future release. Update CLI
