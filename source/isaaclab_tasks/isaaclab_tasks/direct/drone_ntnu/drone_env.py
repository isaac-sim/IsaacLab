...
class DroneEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: DroneEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # drag
        self.body_vel_linear_damping_coefficient = torch.tensor(self.cfg.body_vel_linear_damping_coefficient).to(
            self.device
        )
        self.body_vel_quadratic_damping_coefficient = torch.tensor(self.cfg.body_vel_quadratic_damping_coefficient).to(
            self.device
        )
        self.angvel_linear_damping_coefficient = torch.tensor(self.cfg.angvel_linear_damping_coefficient).to(
            self.device
        )
        self.angvel_quadratic_damping_coefficient = torch.tensor(self.cfg.angvel_quadratic_damping_coefficient).to(
            self.device
        )
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        ref_motor_thrusts = torch.bmm(self.inv_force_torque_allocation_matrix, controller_output.unsqueeze(-1)).squeeze(
            -1
        )
        ...
        if self.cfg.enable_disturbance:
            max_disturb = torch.tensor(self.cfg.max_force_and_torque_disturbance, device=self.device).repeat(
                self.num_envs, 1
            )
            disturb_occurence = torch.bernoulli(
                self.cfg.disturbance_probability * torch.ones((self.num_envs), device=self.device)
            )
            self.external_wrench_b[:, 0] += torch_rand_float_tensor(
                -max_disturb, max_disturb
            ) * disturb_occurence.unsqueeze(1)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        self._reward_buf[:] = compute_reward(
            pos_error=math_utils.quat_apply_inverse(
                robot_vehicle_orientation, (self._commands_w - self._robot.data.root_pos_w)
            ),
