...
class AnymalCEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: DroneEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.external_wrench = torch.zeros((self.num_envs, 9, 3), device=self.device)
        ...
        # drag
        self.body_vel_linear_damping_coefficient = torch.tensor(self.body_vel_linear_damping_coefficient)
        self.body_vel_quadratic_damping_coefficient = torch.tensor(self.body_vel_quadratic_damping_coefficient)
        self.angvel_linear_damping_coefficient = torch.tensor(self.angvel_linear_damping_coefficient)
        self.angvel_quadratic_damping_coefficient = torch.tensor(self.angvel_quadratic_damping_coefficient)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        controller_output = self.sim_env.robot_manager.robot.controller(self._processed_actions)
        ...
        self.external_wrench[:, application_mask, :3] = motor_forces
        self.external_wrench[:, application_mask, 3:] = motor_torques
        
        drag_wrench = compute_drag_contributions(
            linvel=self._robot.data.root_lin_vel_w,
            angvel=self._robot.data.root_ang_vel_w,
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self._truncated[:] = torch.where(dist > 8.0, 1, self._truncated)
        time_out = self.episode_length_buf >= self.max_episode_length - 1
        return self._truncated.clone(), time_out
