...
class HumanoidSceneCfg(InteractiveSceneCfg):
    ...
    # robot
    robot: Any = field(default_factory=lambda: replace_config(HUMANOID_CFG, prim_path="{ENV_REGEX_NS}/Robot"))
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        base_ang_vel: Any = field(default_factory=lambda: ObsTerm(func=mdp.base_ang_vel, scale=0.25))
        ...
        joint_vel_rel: Any = field(default_factory=lambda: ObsTerm(func=mdp.joint_vel_rel, scale=0.1))
...
class RewardsCfg:
    ...
    # (2) Stay alive bonus
    alive: Any = field(default_factory=lambda: RewTerm(func=mdp.is_alive, weight=2.0))
    ...
    energy: Any = field(
        default_factory=lambda: RewTerm(func=mdp.power_consumption, weight=-0.005, params={"gear_ratio": JOINT_GEARS})
    )
    ...
    terminating: Any = field(default_factory=lambda: RewTerm(func=mdp.terminated_penalty, weight=-1.0))
...
class TerminationsCfg:
    ...
    torso_height: Any = field(
        default_factory=lambda: DoneTerm(func=mdp.root_height_below_minimum, params={"minimum_height": 0.8})
    )
...
class HumanoidEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # Scene settings
    scene: HumanoidSceneCfg = field(
        default_factory=lambda: HumanoidSceneCfg(num_envs=4096, env_spacing=5.0, clone_in_fabric=True)
    )
    # Basic settings
    observations: HumanoidObservationsCfg = field(default_factory=HumanoidObservationsCfg)
