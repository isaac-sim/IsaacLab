...
``ISAAC_LAB_ENABLE_ISAAC_RTX_PER_ENV_SCENE_PARTITION=1`` and, to preview the Kit
feature that keeps every partition visible to the spectator camera, pass
``--kit_args "--/rtx/scenePartitioning/showPartitionsInBackground=true"``.
"""
...
class AnymalDFlatCameraEnvCfg(AnymalDFlatEnvCfg):
    ...
    def __post_init__(self):
        ...
        # neighbours' as the base moves. Lower this (e.g. back to 2.5) to observe the
        # limitation when partitions overlap.
