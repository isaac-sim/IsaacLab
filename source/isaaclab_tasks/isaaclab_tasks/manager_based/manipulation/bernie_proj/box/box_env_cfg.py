...
class BoxSceneCfg(InteractiveSceneCfg):
    ...
    table = AssetBaseCfg(
        ...
        spawn=UsdFileCfg(usd_path=str(workspace) + "/assets/bernie_proj/table.usd"),
    )

    # Object on the table. In this case a Box
    box = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/box",
        init_state=RigidObjectCfg.InitialStateCfg(pos=[1, 0, 1], rot=[1, 0, 0, 0]),
        spawn=UsdFileCfg(
            usd_path=str(workspace) + "/assets/bernie_proj/box.usd",
...
@configclass
class BoxEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the MuJoCo-style Humanoid walking environment."""

    # Scene settings
    scene: BoxSceneCfg = BoxSceneCfg(num_envs=4096, env_spacing=5.0)
    # Basic settings
