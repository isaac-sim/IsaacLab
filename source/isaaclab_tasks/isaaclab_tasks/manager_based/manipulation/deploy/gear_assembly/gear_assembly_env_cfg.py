...
from isaaclab_tasks.manager_based.manipulation.deploy.gear_assembly.spawners import FixedGearBaseCfg, spawn_gear_base_fixed
...
class GearAssemblySceneCfg(InteractiveSceneCfg):
    ...
    factory_gear_base: ArticulationCfg = ArticulationCfg(
        prim_path="{ENV_REGEX_NS}/FactoryGearBase",
        spawn=FixedGearBaseCfg(
            func=spawn_gear_base_fixed,
        ...
        init_state=ArticulationCfg.InitialStateCfg(pos=(-1.0200, 0.2100, -0.1), rot=(0.0, 0.0, 0.70711, 0.70711)),
        actuators={},
    )
...
class GearAssemblyEnvCfg(ManagerBasedRLEnvCfg):
    # Scene settings
    scene: GearAssemblySceneCfg = GearAssemblySceneCfg(num_envs=1, env_spacing=2.5)  # 4096
    # Basic settings
    ...
    # Newton physics configuration (migrated from PhysxCfg)
