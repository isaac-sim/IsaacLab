...
def joint_position(
    ...
    return _articulation(env, asset_cfg).data.joint_pos
...
def joint_velocity(
    ...
    return _articulation(env, asset_cfg).data.joint_vel
...
def end_effector_pose_w(
    ...
    """Return live tool-tip pose ``[xyz, qw, qx, qy, qz]`` in world frame."""
    ...
    body_id = body_ids[0]
    return torch.cat((asset.data.body_pos_w[:, body_id], asset.data.body_quat_w[:, body_id]), dim=-1)
...
def needle_pose_w(
    ...
    """Return simulated needle pose ``[xyz, qw, qx, qy, qz]`` in world frame."""

    needle: RigidObject = env.scene[asset_cfg.name]
    return torch.cat((needle.data.root_pos_w, needle.data.root_quat_w), dim=-1)
...
def needle_velocity_w(
    ...
    needle: RigidObject = env.scene[asset_cfg.name]
    return torch.cat((needle.data.root_lin_vel_w, needle.data.root_ang_vel_w), dim=-1)
