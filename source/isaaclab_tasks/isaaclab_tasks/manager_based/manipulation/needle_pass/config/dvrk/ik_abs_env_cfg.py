...
from isaaclab.devices import DevicesCfg, DVRKOpenXRDeviceCfg
from isaaclab.devices.openxr.retargeters import DVRKPSMRetargeterCfg, DVRKPSMSideRetargeterCfg
...
PSM_ROOT_ROT_WXYZ = (1.0, 0.0, 0.0, 0.0)
...
# The two grasp poses below are unmodified ``needle_channel_*`` (``T_N_C``)
# rows emitted by Isaac Sim 5.1's native antipodal grasp generator. ``N`` is
# the scaled needle body and ``C`` is the generated channel frame, whose +Z
# axis is the jaw-gap axis.  These are not the mesh-local ``native_*`` fields:
# the asset's nested authored transform separates those frames by about 19 mm.
# Candidate selection never interpolated or manually permuted poses.  The
# donor was selected only after a fixed native candidate was physically closed,
# settled with gravity enabled, and retained through a fixed tool disturbance.
...
DVRK_HANDOFF_PHASE_CFG = HANDOFF_PHASE_CFG.replace(
    ...
    receiver_relative_orientation_target_wxyz=RECEIVER_NEEDLE_TARGET_ROT_WXYZ,
...
def _side_retargeter_cfg(
    ...
    if not all(lower <= home <= upper for lower, home, upper in zip(workspace_lower, home_position, workspace_upper)):
        raise ValueError("dVRK tool home must lie inside its world workspace")
...
class DVRKNeedlePassEnvCfg(NeedlePassEnvCfg):
    ...
    def __post_init__(self):
        ...
        retargeter_cfg = DVRKPSMRetargeterCfg(
            sim_device=self.sim.device,
            left=_side_retargeter_cfg(
