...
def _scene_cfg_from_parsed(parsed: dict, scene_cls: type, base: dict) -> object:
    ...
    # Omit renderer_type so task does not set it; users override via env.scene.base_camera.renderer_type=...
    kwargs = {k: v for k, v in parsed.items() if k != "renderer_type"}
    return scene_cls(**{**base, **kwargs})
...
class KukaAllegroSingleTiledCameraSceneCfg(kuka_allegro_dexsuite.KukaAllegroSceneCfg):
    ...
    def __repr__(self):
        ...
        for f in fields(self):
            if f.name in ("camera_type", "width", "height"):
                continue
            try:
    ...
    for (w, h) in _svk.RESOLUTIONS:
        for renderer_tag, camera_tag in _svk.RENDERER_CAMERA_COMBO:
            ...
            parsed = {
                "width": w,
                "height": h,
                "renderer_type": _svk.RENDERER_TAG_TO_TYPE[renderer_tag],  # stripped in _scene_cfg_from_parsed
