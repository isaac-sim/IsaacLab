...
import robot_lab.tasks.locomotion.velocity_spot.mdp as mdp
import robot_lab.tasks.locomotion.velocity_spot.spot_mdp as spot_mdp
from robot_lab.tasks.locomotion.velocity_spot.spot_mdp.commands.height_command import *
...
class ActionsCfg:
    ...
    joint_vel = mdp.JointVelocityActionCfg(
        asset_name="robot", joint_names=[".*"], scale=1.0, use_default_offset=False, clip=None, preserve_order=True
    )
    joint_eff = mdp.JointEffortActionCfg(
        asset_name="robot", joint_names=[".*"], scale=1.0, clip=None, preserve_order=True
    )
...
class EventCfg:
    ...
    #         "mass_distribution_params": (0.9, 1.1),  # (0.9, 1.1) (0.8, 1.2)
    #         "operation": "scale",
    #     },
    # )
    ...
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", body_names=".*"),
    #         "inertia_distribution_params": (0.8, 1.2),  # (0.8, 1.2) (0.9, 1.1)
    #         "operation": "scale",
    #     },
    # )
    ...
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", body_names="base"),
    #         "force_range": (-10.0, 10.0),   # (0.0, 0.0) (-10.0, 10.0),
    #         "torque_range": (-10.0, 10.0),  # (0.0, 0.0) (-10.0, 10.0),
    #     },
    # )
...
class RewardsCfg:
    ...
    joint_pos_hip = RewTerm(
        func=spot_mdp.joint_position_penalty_hip,
        weight=0.0, 
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=".*_hip_joint"),
            "stand_still_scale": 5.0,
    ...
    base_height_l2_exp = RewTerm(
        func=mdp.base_height_l2_from_command_exp,
        weight=0.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base"),
            "sensor_cfg": SceneEntityCfg("height_scanner"),
    ...
    action_smoothness_joint_pos = RewTerm(
        func=spot_mdp.action_smoothness_joint_pos_penalty, 
        weight=0.0  
    )
...
@configclass
class LocomotionVelocityHeightRoughEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        """Post initialization."""
        # general settings
        self.decimation = 10  # 4, 10
        self.episode_length_s = 20.0
        # simulation settings
        self.sim.dt = 0.002  # 0.005, 0.002
    ...
    # def desable_all_rewards(self):
    #     for attr in dir(self.rewards):
    #         if not attr.startswith("__"):
    #             reward_attr = getattr(self.rewards, attr)
    #             if not callable(reward_attr):
    #                 setattr(self.rewards, attr, None)
