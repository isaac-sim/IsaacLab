...
from isaaclab_tasks.manager_based.locomotion.velocity.velocity_env_cfg import EventsCfg, StartupEventsCfg
...
@configclass
class Go1NewtonEventsCfg(EventsCfg):
...
@configclass
class Go1PhysxEventsCfg(Go1NewtonEventsCfg, StartupEventsCfg):
    def __post_init__(self):
        super().__post_init__()
...
@configclass
class Go1EventsCfg(PresetCfg):
    default = Go1PhysxEventsCfg()
    newton = Go1NewtonEventsCfg()
    physx = default


@configclass
class UnitreeGo1RoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    events: Go1EventsCfg = Go1EventsCfg()
