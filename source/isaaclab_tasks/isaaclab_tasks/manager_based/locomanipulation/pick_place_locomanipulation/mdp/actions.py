...
class LowerBodyAction(ActionTerm):
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        obs_tensor = self._env.obs_buf["policy"]

        # Concatenate actions repeated by history length
        history_length = self._observation_cfg.policy.history_length
        repeated_actions = base_command.unsqueeze(1).repeat(1, history_length, 1).reshape(base_command.shape[0], -1)
        policy_input = torch.cat([obs_tensor, repeated_actions], dim=-1)
