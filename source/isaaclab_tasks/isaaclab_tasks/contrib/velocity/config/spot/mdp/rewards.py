...
def air_time_reward(
    ...
    body_vel = torch.linalg.norm(asset.data.root_lin_vel_b.torch[:, :2], dim=1).unsqueeze(dim=1).expand(-1, 4)
    reward = torch.where(
...
def base_angular_velocity_reward(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, std: float) -> torch.Tensor:
    ...
    ang_vel_error = torch.linalg.norm((target - asset.data.root_ang_vel_b.torch[:, 2]).unsqueeze(1), dim=1)
...
def base_linear_velocity_reward(
    ...
    # compute the error
    target = env.command_manager.get_command("base_velocity")[:, :2]
    lin_vel_error = torch.linalg.norm((target - asset.data.root_lin_vel_b.torch[:, :2]), dim=1)
...
class GaitReward(ManagerTermBase):
    ...
    def __call__(
        ...
        body_vel = torch.linalg.norm(self.asset.data.root_lin_vel_b.torch[:, :2], dim=1)
        return torch.where(
...
def base_motion_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    return 0.8 * torch.square(asset.data.root_lin_vel_b.torch[:, 2]) + 0.2 * torch.sum(
        torch.abs(asset.data.root_ang_vel_b.torch[:, :2]), dim=1
    )
...
def base_orientation_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    return torch.linalg.norm((asset.data.projected_gravity_b.torch[:, :2]), dim=1)
...
def joint_acceleration_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.linalg.norm((asset.data.joint_acc.torch), dim=1)
...
def joint_torques_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.linalg.norm(asset.actuators.applied_torque.torch, dim=1)
...
def joint_velocity_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.linalg.norm((asset.data.joint_vel.torch), dim=1)
