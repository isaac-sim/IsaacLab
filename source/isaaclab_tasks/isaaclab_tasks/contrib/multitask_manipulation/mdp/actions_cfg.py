...
class SelectedJointPositionActionCfg(ActionTermCfg):
    ...
    class_type: type[SelectedJointPositionAction] | str = (
        "isaaclab_tasks.contrib.multitask_manipulation.mdp.actions:SelectedJointPositionAction"
    )
...
class SelectedBinaryJointPositionActionCfg(ActionTermCfg):
    ...
    class_type: type[SelectedBinaryJointPositionAction] | str = (
        "isaaclab_tasks.contrib.multitask_manipulation.mdp.actions:SelectedBinaryJointPositionAction"
    )
    joint_names: list[str] = REQUIRED
    """Joint-name expressions resolved on the selected articulation."""
    open_command: float = REQUIRED
    """Open joint target [m or rad, depending on joint type]."""
    close_command: float = REQUIRED
