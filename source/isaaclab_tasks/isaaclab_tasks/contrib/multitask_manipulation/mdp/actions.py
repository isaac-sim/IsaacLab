...
class SelectedJointPositionAction(ActionTerm):
    """Apply a global action batch to one articulation's selected physics-view rows."""

    cfg: SelectedJointPositionActionCfg
    _asset: Articulation

    def __init__(self, cfg: SelectedJointPositionActionCfg, env: ManagerBasedEnv):
...
class SelectedBinaryJointPositionAction(ActionTerm):
    """Map one binary global action to joint targets in asset physics-view order."""

    cfg: SelectedBinaryJointPositionActionCfg
    _asset: Articulation

    def __init__(self, cfg: SelectedBinaryJointPositionActionCfg, env: ManagerBasedEnv):
        super().__init__(cfg, env)
        joint_ids, self._joint_names = self._asset.find_joints(cfg.joint_names, preserve_order=True, as_proxy=True)
        self._joint_ids = joint_ids.torch
        self._asset_cfg = SceneEntitySelectionCfg(cfg.asset_name)
        ...
        self._processed_actions = torch.zeros(len(self._asset_cfg.env_ids), len(joint_ids), device=self.device)
        self._open_command = torch.full((len(joint_ids),), cfg.open_command, device=self.device)
        self._close_command = torch.full((len(joint_ids),), cfg.close_command, device=self.device)
    ...
    @property
    def processed_actions(self) -> torch.Tensor:
        """Joint targets in asset physics-view order."""
        return self._processed_actions
