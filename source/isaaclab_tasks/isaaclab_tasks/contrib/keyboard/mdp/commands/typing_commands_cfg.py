...
class LetterTypingCommandCfg(CommandTermCfg):
    ...
    class ResetCfg:
        ...
        **oversampling** many candidate typing states, **grid-bucket-downsampling** them to uniform coverage over
        ...
        ik: DifferentialInverseKinematicsActionCfg | None = config_field(MISSING)
        ...
        ik_rpy_deg: tuple[float, float, float] = config_field(MISSING)
        ...
        ik_hover_height: float = config_field(MISSING)
        """Height [m] above the target key at which the jaw tip is placed on reset."""

        ik_iters: int | tuple[int, int] = config_field(MISSING)
        ...
        ik_seed_joint_noise: float = config_field(0.0)
        ...
        match_prob: float = config_field(0.5)
        ...
        oversample_factor: int = config_field(64)
        ...
        w_prefix_complete: float = config_field(1.0)
        ...
        w_remaining: float = config_field(1.0)
        ...
        buffer_size: int = config_field(4096)
        ...
        beta_target: float = config_field(0.5)
        ...
        beta_kappa: float = config_field(1.0)
        ...
        normal_weight: float = config_field(0.5)
        ...
        sample_eps: float = config_field(1e-4)
        ...
        history_len: int = config_field(20)
        """Sliding-window length of the per-snapshot success monitor."""

        reset_assets: tuple[str, ...] | None = config_field(None)
        ...
        pre_solve_reset: EventTermCfg | None = config_field(None)
    ...
    class_type: type = config_field(LetterTypingCommand)
    ...
    letter_length: tuple[int, int] = config_field((1, 5))
    """``(min, max)`` target sequence length sampled per episode."""

    max_len: int | None = config_field(None)
    ...
    command_mode: str = config_field("letter_full")
    """``letter_full`` -> ``[target, typed]`` (absolute); ``letter_left`` -> remaining target only."""

    actuation_fraction: float = config_field(KEY_ACTUATION_FRACTION)
    ...
    slot_labels: tuple[str, ...] = config_field(())
    ...
    viz_env_ids: tuple[int, ...] | None = config_field(None)
