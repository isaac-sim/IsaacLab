    ...
    def __init__(self, cfg: LetterTypingCommandCfg, env: ManagerBasedRLEnv):
        ...
        if cfg.reset.ik is not None:
            ...
            self._reset_ik = DifferentialIKController(
                ...
                device=self.device,
                num_joints=len(self._ik_joint_ids),
                joint_pos_limits=self.robot.data.soft_joint_pos_limits.torch[0, self._ik_joint_ids, :]
                if ik_cfg.controller.joint_limit_avoidance_gain > 0.0
                else None,
            )
