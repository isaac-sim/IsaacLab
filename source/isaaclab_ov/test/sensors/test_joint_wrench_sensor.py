...
# ---------------------------------------------------------------------------
# Release-branch wrench convention helper
# ---------------------------------------------------------------------------
...
def test_nested_articulation_root_resolution(sim, device):
    ...
    raw_wrench = wp.to_torch(robot.root_view.get_attribute(TT.LINK_INCOMING_JOINT_FORCE)).reshape(
        1, robot.num_bodies, 6
    )
    assert not torch.allclose(raw_wrench[:, 2:], raw_wrench[:, 1:2].expand_as(raw_wrench[:, 2:]))
    expected_force, expected_torque = _ovphysx_incoming_joint_wrench_in_joint_frame(sensor, raw_wrench)
    torch.testing.assert_close(sensor.data.force.torch, expected_force, rtol=1e-4, atol=1e-5)
    torch.testing.assert_close(sensor.data.torque.torch, expected_torque, rtol=1e-4, atol=1e-5)
