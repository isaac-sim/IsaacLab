...
@pytest.fixture()
def strategy(monkeypatch) -> _AsyncRenderStrategy:
    monkeypatch.delenv("OVRTX_NUM_BUFFERS", raising=False)
...
def test_every_frame_is_delivered_exactly_once(strategy, timeline):
    ...
    for ordinal in range(5):
        strategy.settle_before_scene_write()
        _render(strategy, renderer, ordinal, consumed)
    strategy.cleanup(object(), lambda render_data, products: consumed.append(-1))
