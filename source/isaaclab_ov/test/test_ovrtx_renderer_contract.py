...
def test_ovrtx_destroy_render_data_releases_only_the_given_render_data():
    """``destroy_render_data`` releases the render data's own buffers and leaves the renderer usable.

    The stage queries, tensor bindings and render products the renderer holds are shared with
    every other camera that resolved to it, so a single camera's teardown must not take them.
    """
    renderer = _make_ovrtx_renderer_without_backend()
    ...
    renderer.destroy_render_data(render_data)
...
def test_ovrtx_destroy_render_data_without_render_data_keeps_renderer_state():
    """``destroy_render_data(None)`` has nothing to release and must not disturb the renderer."""
    renderer = _make_ovrtx_renderer_without_backend()
    renderer._render_product_paths = ["/Render/RenderProduct_camera"]
    renderer._initialized_scene = True

    renderer.destroy_render_data(None)
