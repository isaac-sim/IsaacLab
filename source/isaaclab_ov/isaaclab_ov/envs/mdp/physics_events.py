...
class RandomizePhysicsSceneGravity:
    """Randomize the scene-wide OVPhysX gravity through its manager."""
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        if self._distribution not in ("uniform", "log_uniform", "gaussian"):
            raise NotImplementedError(f"Unknown distribution: {self._distribution!r} for gravity randomization.")
        if cfg.params["operation"] not in ("add", "scale", "abs"):
            raise NotImplementedError(f"Unknown operation: {cfg.params['operation']!r} for gravity randomization.")
    ...
    def __call__(
        ...
        bounds = tuple(torch.tensor(values, device="cpu") for values in gravity_distribution_params)
        gravity = randomize_prop_by_op(
            gravity, bounds, None, slice(None), operation=operation, distribution=self._distribution
        )
        self._manager.set_gravity(tuple(gravity[0].tolist()))
