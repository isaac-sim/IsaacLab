...
class OVRTXRenderer(BaseRenderer):
    ...
    def _register_camera(self, spec: CameraRenderSpec, render_data: OVRTXCameraRenderData) -> None:
        ...
        if self._use_ovstage:
            reference = ovstage.population.add_usd_reference_from_string(self._stage, usd, f"/{scope}")
            ...
            product_paths = self._stage_paths.create_path_list_from_strings([product_path])
            try:
                with self._stage.query_from_path_list(product_paths) as query:
                    # USD references drop external camera targets; author the relationship in Fabric.
                    self._stage.write_attribute(
                        query,
                        "camera",
                        ordinal=self._current_ordinal,
                        tensors=np.array([self._stage_paths.intern_path(p) for p in camera_paths], dtype=np.uint64),
                        is_array=True,
                        semantic=ovstage.AttributeSemantic.RELATIONSHIP_PATH_ID,
                    ).wait()
            finally:
                self._stage_paths.destroy_path_list(product_paths)
            camera_paths_list = self._stage_paths.create_path_list_from_strings(camera_paths)
            render_data.resources.callback(self._stage_paths.destroy_path_list, camera_paths_list)
            render_data.camera_xform_query = render_data.resources.enter_context(
                self._stage.query_from_path_list(camera_paths_list)
            )
            self._stage.write_attribute(
            ...
            self._stage.write_attribute(
                ...
                ordinal=self._current_ordinal,
                tensors=np.array(
                    [self._stage_paths.intern_token(f"env_{i}") for i in range(spec.num_instances)], dtype=np.uint64
                ),
                is_array=False,
                semantic=ovstage.AttributeSemantic.TOKEN_ID,
            ).wait()
        else:
            reference = self._renderer.add_usd_reference_from_string(usd, f"/{scope}")
            render_data.resources.callback(self._renderer.remove_usd, reference)
            self._renderer.write_array_attribute(
            ...
            render_data.camera_xform_binding = self._renderer.bind_attribute(
                prim_paths=camera_paths,
                attribute_name="omni:xform",
                semantic=Semantic.XFORM_MAT4x4,
                prim_mode=PrimMode.EXISTING_ONLY,
    ...
    def _remove_camera_reference(self, reference: int) -> None:
        ...
        ovstage.population.remove_usd(self._stage, reference)
        ovstage.population.apply_usd_changes(self._stage, ordinal=self._current_ordinal)
    ...
    def _close_ovstage(self) -> None:
        ...
        # _initialize_camera_render_data_from_spec_ovstage creates it.
