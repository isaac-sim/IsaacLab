...
def test_warp_environment_deprecates_and_preserves_stable_manager_profile() -> None:
    ...
    env.cfg = SimpleNamespace(rewards="warp_rewards", actions="warp_actions")
    env._manager_call_switch = SimpleNamespace(
        get_mode_for_manager=lambda name: (
            ManagerCallMode.STABLE if name == "RewardManager" else ManagerCallMode.WARP_NOT_CAPTURED
        ),
    )
    env._resolve_stable_cfg_counterpart = Mock(
    ...
    with pytest.warns(DeprecationWarning, match="STABLE managers"):
        env._apply_manager_term_cfg_profile()

    assert env.cfg.rewards == "stable_rewards"
    assert env.cfg.actions == "warp_actions"
...
def test_curriculum_uses_mask_before_scene_reset() -> None:
    ...
    env._reset_idx(env_mask=reset_mask, env_ids=reset_ids)
    ...
    assert stages[0][1]["env_ids"] is reset_ids
