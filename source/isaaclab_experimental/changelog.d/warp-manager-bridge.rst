...
* Changed ``--frontend warp`` to resolve a direct task's Warp env class by name
  from the mirrored experimental package (``<task>_direct_env:<Name>Env`` →
  ``<task>_warp_env:<Name>WarpEnv``), so stable direct registrations no longer
  need a ``warp_entry_point`` kwarg. An explicit ``warp_entry_point`` is still
  honored as an override for classes that cannot follow the convention.
