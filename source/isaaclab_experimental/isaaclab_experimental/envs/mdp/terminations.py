...
def time_out(env: ManagerBasedRLEnv, out) -> None:
    ...
    # TODO(warp-migration): env.episode_length_buf is a torch.Tensor (torch.long). Replace
    #  once ManagerBasedRLEnv provides a native warp property.
    episode_length_wp = wp.from_torch(env.episode_length_buf, dtype=wp.int64)
    wp.launch(
        kernel=_time_out_kernel,
        dim=env.num_envs,
        inputs=[episode_length_wp, env.max_episode_length, out],
        device=env.device,
    )
