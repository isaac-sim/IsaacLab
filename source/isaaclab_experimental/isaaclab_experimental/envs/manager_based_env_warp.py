...
class ManagerBasedEnvWarp:
    ...
    def env_origins_wp(self) -> wp.array(dtype=wp.vec3f):
        """Warp view of the scene-owned environment origins [m], shape ``(num_envs,)``.

        The scene hands out a :class:`~isaaclab.utils.warp.ProxyArray`; this boundary
        accessor resolves it once so Warp-native consumers see a plain Warp array.
        """
