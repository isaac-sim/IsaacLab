...
class CircularBuffer:
    ...
    def reset(
        self,
        batch_ids: Sequence[int] | torch.Tensor | wp.array | None = None,
        *,
        ...
        Args:
            ...
            env_mask: Boolean mask of shape (batch_size,) selecting elements to reset.
                If provided, it takes precedence over ``batch_ids``.
        """
        if env_mask is not None:
            ...
            if str(env_mask.device) != self._device:
                env_mask = env_mask.to(self._device)
            if env_mask.ndim != 1 or env_mask.shape[0] != self._batch_size:
                raise ValueError(f"Expected env_mask shape ({self._batch_size},), received {tuple(env_mask.shape)}.")
            batch_ids = env_mask
