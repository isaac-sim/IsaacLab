...
def _get_clip_kernel(source_ndims: int, lower_ndims: int, upper_ndims: int):
    if source_ndims == 1:
        ...
        def clip(x: wp.array(dtype=Any), lower: Any, upper: Any):
            i = wp.tid()
            x[i] = wp.clamp(x[i], wp.static(array_switch(lower_ndims))(lower, i), wp.static(array_switch(upper_ndims))(upper, i))
    elif source_ndims == 2:
        @wp.kernel
        def clip(x: wp.array2d(dtype=Any), lower: Any, upper: Any):
            i, j = wp.tid()
            x[i, j] = wp.clamp(x[i, j], wp.static(array_switch(lower_ndims))(lower, i, j), wp.static(array_switch(upper_ndims))(upper, i, j))
    elif source_ndims == 3:
        @wp.kernel
        def clip(x: wp.array3d(dtype=Any), lower: Any, upper: Any):
            i, j, k = wp.tid()
            x[i, j, k] = wp.clamp(x[i, j, k], wp.static(array_switch(lower_ndims))(lower, i, j, k), wp.static(array_switch(upper_ndims))(upper, i, j, k))
    elif source_ndims == 4:
        @wp.kernel
        def clip(x: wp.array4d(dtype=Any), lower: Any, upper: Any):
            i, j, k, l = wp.tid()
            x[i, j, k, l] = wp.clamp(x[i, j, k, l], wp.static(array_switch(lower_ndims))(lower, i, j, k, l), wp.static(array_switch(upper_ndims))(upper, i, j, k, l))
    return clip
...
def _get_mul_kernel(source_ndims: int, factor_ndims: int):
    if source_ndims == 1:
        ...
        def mul(x: wp.array4d(dtype=Any), factor: Any):
            i, j, k, l = wp.tid()
            x[i, j, k, l] = x[i, j, k, l] * wp.static(array_switch(ndims))(factor, i, j, k, l)
    return mul
