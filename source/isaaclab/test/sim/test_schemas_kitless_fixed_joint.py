...
"""Kitless regression tests for authoring a world fixed joint without ``omni.physx``.

These tests run on an in-memory USD stage and intentionally do NOT launch Isaac Sim /
Kit, so they exercise the same code path used by kitless backends (e.g. Newton) where
