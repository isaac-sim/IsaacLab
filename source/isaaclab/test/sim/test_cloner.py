...
from isaaclab.cloner import TemplateCloneCfg, clone_from_template, physx_replicate, sequential, usd_replicate
...
def test_usd_replicate_with_positions_and_mask(sim):
    ...
    prim_utils.create_prim("/World/template", "Xform")
    prim_utils.create_prim("/World/template/A", "Xform")
    prim_utils.create_prim("/World/template/B", "Xform")
    ...
    assert prim_utils.get_prim_at_path("/World/envs/env_0/Object/A").IsValid()
    assert not prim_utils.get_prim_at_path("/World/envs/env_0/Object/B").IsValid()
    assert prim_utils.get_prim_at_path("/World/envs/env_1/Object/B").IsValid()
    assert not prim_utils.get_prim_at_path("/World/envs/env_1/Object/A").IsValid()
    assert prim_utils.get_prim_at_path("/World/envs/env_2/Object/A").IsValid()

    # Check xformOp:translate authored for env_2/A
    prim = prim_utils.get_prim_at_path("/World/envs/env_2/Object/A")
...
def test_usd_replicate_depth_order_parent_child(sim):
    ...
    # Prepare sources
    prim_utils.create_prim("/World/template", "Xform")
    prim_utils.create_prim("/World/template/Parent", "Xform")
    prim_utils.create_prim("/World/template/Parent/Child", "Xform")

    # Destinations (single env)
    env_ids = torch.tensor([0, 1], dtype=torch.long)
    prim_utils.create_prim("/World/envs", "Xform")
    prim_utils.create_prim("/World/envs/env_0", "Xform")
    prim_utils.create_prim("/World/envs/env_1", "Xform")
    ...
    for i in range(2):
        assert prim_utils.get_prim_at_path(f"/World/envs/env_{i}/Parent").IsValid()
        assert prim_utils.get_prim_at_path(f"/World/envs/env_{i}/Parent/Child").IsValid()
