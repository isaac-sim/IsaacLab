...
def test_path_rebase():
    ...
    # rebase == dst_root + relative_to (L2)
    tail = cloner.path.relative_to("/World/envs/env_0/Robot", "/World/envs/env_0")
    assert tail is not None
    assert (
        cloner.path.rebase("/World/envs/env_0/Robot", "/World/envs/env_0", "/World/envs/env_9")
        == "/World/envs/env_9" + tail
    )
...
def test_iter_sources(sim):
    ...
    matches = list(cloner.query.iter_sources(plan, "/World/scenes/.*/Object/Body/Camera"))
...
def test_path_to_source_nested_templates_pick_most_specific(sim):
    ...
    resolved = cloner.query.path_to_source(plan=plan, path_expr="/World/envs/env_0/Robot/ee_link/palm_link/Camera")
    ...
    resolved = cloner.query.path_to_source(plan=plan, path_expr="/World/envs/env_0/Robot/base")

    assert resolved == ("/World/envs/env_0/Robot", "/World/envs/env_*/Robot", "/base")


def test_path_to_source_ambiguous_templates_raise(sim):
    ...
    with pytest.raises(ValueError, match="matches multiple destination templates"):
        cloner.query.path_to_source(plan=plan, path_expr="/World/envs/env_0/Robot")


def test_path_to_source_merges_same_template_rows(sim):
    ...
    # Both rows are active, so resolution reports the first one's source.
    resolved = cloner.query.path_to_source(plan=plan, path_expr="/World/envs/env_.*/Object/Body/Camera")

    assert resolved == ("/World/envs/env_0/Object", "/World/envs/env_*/Object", "/Body/Camera")


def test_path_to_source_partial_coverage_returns(sim):
...
def test_path_to_source_inactive_rows_return_none(sim):
    """A template whose every matching row populates no env resolves to ``None``."""
    plan = ClonePlan(
        sources=("/World/envs/env_0/Object",),
        destinations=("/World/envs/env_{}/Object",),
        clone_mask=torch.zeros((1, 4), dtype=torch.bool, device=sim.cfg.device),
    )

    assert cloner.query.path_to_source(plan=plan, path_expr="/World/envs/env_.*/Object/Body") is None


def _robot_plan(mask_row=(True, True, True, True)):
    """A single-asset (Robot) clone plan over 4 envs with the given mask row."""
    return ClonePlan(
        sources=("/World/envs/env_0/Robot",),
        destinations=("/World/envs/env_{}/Robot",),
        clone_mask=torch.tensor([list(mask_row)], dtype=torch.bool),
    )


def test_source_idx():
    """source_idx returns the owning row (a prototype index) in source and clone space."""
    plan = _robot_plan()
    assert cloner.query.source_idx(plan, "/World/envs/env_0/Robot/base", space="source") == 0
    assert cloner.query.source_idx(plan, "/World/envs/env_.*/Robot/base", space="clone") == 0
    assert cloner.query.source_idx(plan, "/World/table", space="source") is None
    with pytest.raises(ValueError, match="space must be"):
        cloner.query.source_idx(plan, "/World/envs/env_0/Robot", space="bogus")


def test_path_env_ids():
    """path_env_ids returns the env fiber of a source-space path (grows with num_clones)."""
...
def test_path_maps_to_env():
    """path_maps_to_env reports incidence membership for a source path and env."""
    plan = _robot_plan((True, True, False, True))
    assert cloner.query.path_maps_to_env(plan, "/World/envs/env_0/Robot/base", 1)
    assert not cloner.query.path_maps_to_env(plan, "/World/envs/env_0/Robot/base", 2)
    assert not cloner.query.path_maps_to_env(plan, "/World/ground", 0)
...
def test_path_to_clones():
    """path_to_clones returns a lazy (clone_template, env_ids), or None when unowned."""
    result = cloner.query.path_to_clones(_robot_plan(), "/World/envs/env_0/Robot/base")
    assert result == ("/World/envs/env_{}/Robot/base", (0, 1, 2, 3))
    assert cloner.query.path_to_clones(_robot_plan(), "/World/ground") is None
...
def test_path_to_clone_heterogeneous_selects_env_owning_row():
    ...
    plan = ClonePlan(
        sources=("/World/envs/env_0/Object", "/World/envs/env_2/Object"),
        destinations=("/World/envs/env_{}/Object", "/World/envs/env_{}/Object"),
        clone_mask=torch.tensor([[True, True, False, True], [False, False, True, False]], dtype=torch.bool),
    )
...
def test_algebraic_laws():
    """The path/plan operators satisfy the factorization, fan-out, coverage, and round-trip laws."""
    plan = _robot_plan()
    p = "/World/envs/env_0/Robot/base"
    num_envs = plan.clone_mask.shape[1]
    r = cloner.query.source_idx(plan, p, space="source")
    assert r is not None
    fiber = cloner.query.path_env_ids(plan, p)
    tail = cloner.path.relative_to(p, plan.sources[r])
    assert tail is not None

    for e in range(num_envs):
        # L5 coverage equivalence
        maps = cloner.query.path_maps_to_env(plan, p, e)
        clone = cloner.query.path_to_clone(plan, p, e)
        assert maps == (e in fiber) == (clone is not None)
        if clone is not None:
            # L1 factorization
            assert clone == plan.destinations[r].format(e) + tail
            # L6 round-trip (pull o push)
            resolved = cloner.query.path_to_source(plan, clone)
            assert resolved is not None
            source, _glob, resolved_tail = resolved
            assert source + resolved_tail == p

    # L4 fan-out = fibered push
    clone_template, env_ids = cloner.query.path_to_clones(plan, p)
    assert env_ids == fiber
    assert [clone_template.format(e) for e in env_ids] == [cloner.query.path_to_clone(plan, p, e) for e in env_ids]
