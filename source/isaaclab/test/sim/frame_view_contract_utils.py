...
def test_return_types_are_torcharray(device, view_factory):
    ...
    try:
        ...
        assert isinstance(scales_full, wp.array), (
            f"get_scales() must be wp.array (deprecated legacy return type), got {type(scales_full).__name__}"
        )
        scales_idx = bundle.view.get_scales(indices)
        assert isinstance(scales_idx, wp.array), (
            f"get_scales(indices) must be wp.array (deprecated legacy return type), got {type(scales_idx).__name__}"
        )
    finally:
        bundle.teardown()
