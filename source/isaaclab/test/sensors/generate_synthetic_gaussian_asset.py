...
def _author_animated_tracks(
    ...
    scales_attr = deformable_prim.GetAttribute("scalesh")
    for (frame, phase), positions in zip(frames, deformable_positions_by_frame, strict=True):
        ...
        scales_attr.Set(
            Vt.Vec3hArray([Gf.Vec3h(*(scale * axis for axis in animation.particle_scale))] * len(positions)),
            Usd.TimeCode(frame),
        )
    ...
    _author_extent(
        ...
        padding=1.2 * max(animation.particle_scale),
    )
