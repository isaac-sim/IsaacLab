...
class TestWarpCamera(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.camera_cfg = MultiMeshRayCasterCameraCfg(
    ...
    def test_camera_init(self):
        ...
        # Create camera
        camera = MultiMeshRayCasterCamera(cfg=self.camera_cfg)
        # Play sim
        self.sim.reset()
    ...
    def test_camera_init_offset(self):
        ...
        cam_cfg_offset_ros.offset = MultiMeshRayCasterCameraCfg.OffsetCfg(
            pos=POSITION,
            rot=QUAT_ROS,
            convention="ros",
        )
        ...
        camera_ros = MultiMeshRayCasterCamera(cam_cfg_offset_ros)
        # -- OpenGL convention
        cam_cfg_offset_opengl = copy.deepcopy(self.camera_cfg)
        cam_cfg_offset_opengl.offset = MultiMeshRayCasterCameraCfg.OffsetCfg(
            pos=POSITION,
            rot=QUAT_OPENGL,
            convention="opengl",
        )
        ...
        camera_opengl = MultiMeshRayCasterCamera(cam_cfg_offset_opengl)
        # -- World convention
        cam_cfg_offset_world = copy.deepcopy(self.camera_cfg)
        cam_cfg_offset_world.offset = MultiMeshRayCasterCameraCfg.OffsetCfg(
            pos=POSITION,
            rot=QUAT_WORLD,
            convention="world",
        )
        ...
        camera_world = MultiMeshRayCasterCamera(cam_cfg_offset_world)
        ...
        del camera_ros, camera_opengl, camera_world

    def test_camera_init_intrinsic_matrix(self):
        """Test camera initialization from intrinsic matrix."""
        # get the first camera
        camera_1 = MultiMeshRayCasterCamera(cfg=self.camera_cfg)
    ...
    def test_camera_set_world_poses(self):
        ...
        camera = MultiMeshRayCasterCamera(self.camera_cfg)
        # play sim
        self.sim.reset()
