...
# The multi-tile coverage below is also the regression guard for the PPISP demo's
# ``sortingModeHint`` workaround for OMPE-108417: the asset authors ``zDepth`` (as NuRec exports
# do), and RTX drops every splat in every tile with that sort mode once a RenderProduct carries
# more than one camera. Measured against a control render whose
