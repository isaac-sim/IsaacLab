...
def test_first_frame_is_textured_camera(setup_sim, device):
    ...
    sim.step()
    camera.update(dt)
    first_frame = wp.to_torch(camera.data.output["rgb"])[0].clone().to(dtype=torch.float32)

    # Let the renderer stabilise, then capture the reference frame
    for _ in range(STABILISATION_STEPS):
        sim.step()
    camera.update(dt)
    stable_frame = wp.to_torch(camera.data.output["rgb"])[0].clone().to(dtype=torch.float32)
