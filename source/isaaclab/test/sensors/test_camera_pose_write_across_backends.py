...
def test_camera_pose_write_moves_pose_and_render(physics_cfg):
    """``camera.data.pos_w`` and the rendered depth both follow a ``set_world_poses`` write on every backend.

    A write that never reaches the camera prim the RTX renderer reads leaves the image at the old pose,
    collapsing the far/close depth ratio to ~1.
    """
    ...
    assert ratio > 1.5, (
        f"Far depth ({depth_far_m:.2f} m) should be > 1.5x close depth ({depth_close_m:.2f} m); got {ratio:.2f}x."
        " The camera pose write is not reaching the renderer."
    )
