...
def test_visualizer_cfg_validates_background_color():
    assert VisualizerCfg(background_mode="sky").background_mode == "sky"
    assert VisualizerCfg(background_color=None).background_mode == "sky"
    assert VisualizerCfg(background_color=[0, 0.5, 1]).background_color == (0.0, 0.5, 1.0)
    with pytest.raises(ValueError, match="Invalid background_mode"):
        VisualizerCfg(background_mode="gradient")
