...
:class:`~isaaclab.utils.warp.ProxyArray` or ``wp.array``. Legacy conversion or
tensor-method callsites on these properties should migrate to explicit
``.torch`` / ``.warp`` access or ``wp.to_torch`` for raw Warp arrays. These
