# Copyright (c) 2022-2026, The Isaac Lab Project Developers (https://github.com/isaac-sim/IsaacLab/blob/main/CONTRIBUTORS.md).
# All rights reserved.
#
# SPDX-License-Identifier: BSD-3-Clause

"""Unit tests for the fused uint8 Warp path of :func:`isaaclab.utils.images.normalize_rgb`."""

from __future__ import annotations

import pytest
import torch
import warp as wp

from isaaclab.test.utils import test_devices

pytestmark = pytest.mark.unit

wp.config.quiet = True
wp.init()


def _pytorch_reference(src: torch.Tensor, channel_dim: int = -1) -> torch.Tensor:
    """Reference normalize: ``(x / 255 - per-image-channel mean)`` in pure PyTorch."""
    x = src.float() / 255.0
    resolved = channel_dim + x.ndim if channel_dim < 0 else channel_dim
    spatial_dims = tuple(d for d in range(1, x.ndim) if d != resolved)
    return x - torch.mean(x, dim=spatial_dims, keepdim=True)


@pytest.fixture(params=test_devices())
def device(request):
    """Parametrize kernel tests across devices; input validation runs on CPU only."""
    return request.param


cpu_only = pytest.mark.parametrize("device", ["cpu"])
"""Run a Python-side validation check once on CPU instead of on every device."""


class TestNormalizeRgbUint8:
    """Tests for the Warp-backed fused uint8 normalize."""

    @pytest.mark.parametrize("channel_dim", [-1, 3])
    def test_matches_pytorch_reference_random_input(self, device, channel_dim):
        """Output must match the pure-PyTorch reference on randomized BHWC input, per channel."""
        from isaaclab.utils.images import normalize_rgb

        torch.manual_seed(0)
        src = torch.randint(0, 255, (3, 16, 16, 6), dtype=torch.uint8, device=device)
        out = normalize_rgb(src, channel_dim=channel_dim)
        expected = _pytorch_reference(src)
        torch.testing.assert_close(out, expected, atol=1e-5, rtol=1e-5)

    @pytest.mark.parametrize("channel_dim", [-1, 1], ids=["bhwc", "bchw"])
    def test_preallocated_output_reused(self, device, channel_dim):
        """When ``out`` is passed in, the wrapper writes into it and returns the same object."""
        from isaaclab.utils.images import normalize_rgb

        src = torch.randint(0, 255, (2, 6, 8, 8), dtype=torch.uint8, device=device)
        out = torch.empty(src.shape, dtype=torch.float32, device=device)
        ptr_before = out.data_ptr()
        result = normalize_rgb(src, channel_dim=channel_dim, out=out)
        assert result is out
        assert result.data_ptr() == ptr_before
        torch.testing.assert_close(result, _pytorch_reference(src, channel_dim), atol=1e-5, rtol=1e-5)

        src2 = torch.randint(0, 255, (2, 6, 8, 8), dtype=torch.uint8, device=device)
        result2 = normalize_rgb(src2, channel_dim=channel_dim, out=out)
        assert result2 is out
        assert result2.data_ptr() == ptr_before
        torch.testing.assert_close(result2, _pytorch_reference(src2, channel_dim), atol=1e-5, rtol=1e-5)

    def test_constant_mean_with_layout_conversion(self, device):
        """A constant ``mean`` replaces the per-image mean and composes with the layout conversion."""
        from isaaclab.utils.images import normalize_rgb

        src = torch.randint(0, 255, (2, 9, 7, 4), dtype=torch.uint8, device=device)[..., :3]
        out = normalize_rgb(src, output_channel_dim=1, mean=0.5)
        torch.testing.assert_close(out, (src.float() / 255.0 - 0.5).permute(0, 3, 1, 2), atol=1e-6, rtol=1e-6)

    @cpu_only
    def test_deprecated_warp_op_forwards(self, device):
        """``isaaclab.utils.warp.ops.normalize_image_uint8`` warns, forwards, and still rejects non-uint8 input."""
        from isaaclab.utils.warp.ops import normalize_image_uint8

        src = torch.randint(0, 255, (2, 4, 4, 3), dtype=torch.uint8, device=device)
        with pytest.deprecated_call():
            out = normalize_image_uint8(src)
        torch.testing.assert_close(out, _pytorch_reference(src), atol=1e-5, rtol=1e-5)
        with pytest.deprecated_call(), pytest.raises(ValueError, match="4D uint8"):
            normalize_image_uint8(src.float())

    def test_strided_rgb_view_with_layout_conversion(self, device):
        """The RGB view of an RGBA camera buffer normalizes directly into a contiguous BCHW output."""
        from isaaclab.utils.images import normalize_rgb

        rgba = torch.randint(0, 255, (2, 9, 7, 4), dtype=torch.uint8, device=device)
        src = rgba[..., :3]
        assert not src.is_contiguous()
        out = normalize_rgb(src, output_channel_dim=1)

        expected = _pytorch_reference(src).permute(0, 3, 1, 2)
        torch.testing.assert_close(out, expected, atol=1e-5, rtol=1e-5)
        assert out.is_contiguous()

    @cpu_only
    @pytest.mark.parametrize(
        "out_shape, out_dtype",
        [((2, 4, 4, 3), torch.float32), ((2, 4, 4, 6), torch.float16)],
        ids=["shape", "dtype"],
    )
    def test_rejects_out_mismatch(self, device, out_shape, out_dtype):
        """A pre-allocated ``out`` of the wrong shape or dtype must raise."""
        from isaaclab.utils.images import normalize_rgb

        src = torch.zeros((2, 4, 4, 6), dtype=torch.uint8, device=device)
        bad_out = torch.empty(out_shape, dtype=out_dtype, device=device)
        with pytest.raises(ValueError, match="out shape/dtype/device"):
            normalize_rgb(src, out=bad_out)

    @pytest.mark.parametrize("channel_dim", [1, -3])
    def test_bchw_matches_pytorch_reference(self, device, channel_dim):
        """``channel_dim=1`` or ``-3`` (BCHW) must match a BCHW-layout PyTorch reference."""
        from isaaclab.utils.images import normalize_rgb

        torch.manual_seed(2)
        src = torch.randint(0, 255, (3, 6, 16, 16), dtype=torch.uint8, device=device)
        out = normalize_rgb(src, channel_dim=channel_dim)
        torch.testing.assert_close(out, _pytorch_reference(src, channel_dim=1), atol=1e-5, rtol=1e-5)

    @cpu_only
    @pytest.mark.parametrize("bad_dim", [0, 2, 4, -2, -4, -5])
    def test_rejects_invalid_channel_dim(self, device, bad_dim):
        """Only ``channel_dim`` resolving to 1 or 3 is accepted for 4D input."""
        from isaaclab.utils.images import normalize_rgb

        src = torch.zeros((2, 4, 4, 3), dtype=torch.uint8, device=device)
        with pytest.raises(ValueError, match="channel_dim must resolve to 1 .BCHW. or 3 .BHWC."):
            normalize_rgb(src, channel_dim=bad_dim)

    def test_non_divisible_h_last_tile_clamps(self, device):
        """H not divisible by tile_size: last tile's row range must clamp via ``wp.min``."""
        from isaaclab.utils.images import _UINT8_SUM_TILE_HW, normalize_rgb

        # H = 2*TILE + 1 forces a 3rd tile with a single row.
        h = 2 * _UINT8_SUM_TILE_HW + 1
        torch.manual_seed(3)
        src = torch.randint(0, 255, (2, h, 8, 6), dtype=torch.uint8, device=device)
        out = normalize_rgb(src)
        torch.testing.assert_close(out, _pytorch_reference(src), atol=1e-5, rtol=1e-5)

    def test_bchw_multi_tile_h_axis(self, device):
        """H > tile_size on BCHW (H at axis 2) must reduce correctly across multiple tiles."""
        from isaaclab.utils.images import _UINT8_SUM_TILE_HW, normalize_rgb

        h = 2 * _UINT8_SUM_TILE_HW + 1
        torch.manual_seed(6)
        src = torch.randint(0, 255, (2, 6, h, 8), dtype=torch.uint8, device=device)
        out = normalize_rgb(src, channel_dim=1)
        torch.testing.assert_close(out, _pytorch_reference(src, channel_dim=1), atol=1e-5, rtol=1e-5)

    def test_partials_cache_keyed_by_channel_dim(self, device):
        """BCHW and BHWC inputs of identical shape must land in separate cache slots."""
        import isaaclab.utils.images as images_utils

        shape = (2, 6, 8, 8)  # ambiguous shape: works as both BHWC (B=2,H=6,W=8,C=8) and BCHW (B=2,C=6,H=8,W=8)
        src_bhwc = torch.randint(0, 255, shape, dtype=torch.uint8, device=device)
        src_bchw = torch.randint(0, 255, shape, dtype=torch.uint8, device=device)

        images_utils._uint8_sum_partials_cache.clear()

        images_utils.normalize_rgb(src_bhwc)  # channel_dim defaults to -1 -> resolves to 3
        images_utils.normalize_rgb(src_bchw, channel_dim=1)

        # Both calls had shape ``shape`` but different channel_dim, so the cache key
        # (shape, device, channel_dim) must distinguish them: two entries, not one.
        assert (shape, device, 3) in images_utils._uint8_sum_partials_cache
        assert (shape, device, 1) in images_utils._uint8_sum_partials_cache
        assert len(images_utils._uint8_sum_partials_cache) == 2

    def test_partials_cache_reuses_scratch_across_calls(self, device):
        """Repeat calls with the same shape must hit the partials cache, not grow it."""
        import isaaclab.utils.images as images_utils

        shape = (2, images_utils._UINT8_SUM_TILE_HW + 4, 8, 6)
        src_a = torch.randint(0, 255, shape, dtype=torch.uint8, device=device)
        src_b = torch.randint(0, 255, shape, dtype=torch.uint8, device=device)

        # Clear the cache to isolate this test from prior tests' state.
        images_utils._uint8_sum_partials_cache.clear()

        images_utils.normalize_rgb(src_a)
        size_after_first = len(images_utils._uint8_sum_partials_cache)
        first_scratch_ptr = images_utils._uint8_sum_partials_cache[(shape, device, 3)].data_ptr()

        images_utils.normalize_rgb(src_b)
        size_after_second = len(images_utils._uint8_sum_partials_cache)
        second_scratch_ptr = images_utils._uint8_sum_partials_cache[(shape, device, 3)].data_ptr()

        assert size_after_first == 1
        assert size_after_second == size_after_first
        assert first_scratch_ptr == second_scratch_ptr
