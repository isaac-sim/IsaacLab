...
class TestWarpLaunchCache(unittest.TestCase):
    ...
    def test_fixed_device_cache_launches_from_its_owner(self):
        """An owner should retain and use one fixed-device launcher."""
        ...
        owner._warp_launch.launch(_increment, dim=4, inputs=[output])
        owner._warp_launch.launch(_increment, dim=4, inputs=[output])
    ...
    def test_sites_distinguish_persistent_argument_sets(self):
        ...
        self.cache.launch(_increment, dim=4, inputs=[output_a], site=output_a)
        self.cache.launch(_increment, dim=4, inputs=[output_b], site=output_b)
        self.cache.launch(_increment, dim=4, inputs=[output_a], site=output_a)
        self.cache.launch(_increment, dim=4, inputs=[output_b], site=output_b)
    ...
    def test_dim_change_updates_recorded_bounds(self):
        ...
        self.cache.launch(_write_index, dim=4, inputs=[output])
        self.cache.launch(_write_index, dim=8, inputs=[output])
    ...
    def test_eager_environment_mode_uses_current_arguments(self):
        ...
        with patch.dict(os.environ, {"ISAACLAB_WARP_LAUNCH_MODE": "eager"}):
            ...
            cache.launch(_affine, dim=4, inputs=[source, 2.0], outputs=[output])
            cache.launch(_affine, dim=4, inputs=[source, 5.0], outputs=[output])

        self.assertFalse(cache._entries)
        np.testing.assert_array_equal(output.numpy(), np.full(4, 10.0, dtype=np.float32))

    def test_explicit_eager_mode_uses_current_arguments(self):
        """Explicit eager mode should use the current launch arguments."""
        cache = WarpLaunchCache(device=self.device, mode="eager")
        source = wp.full(4, value=2.0, dtype=wp.float32, device=self.device)
        output = wp.zeros(4, dtype=wp.float32, device=self.device)

        cache.launch(_affine, dim=4, inputs=[source, 2.0], outputs=[output])
        cache.launch(_affine, dim=4, inputs=[source, 3.0], outputs=[output])

        self.assertFalse(cache._entries)
        np.testing.assert_array_equal(output.numpy(), np.full(4, 6.0, dtype=np.float32))

    def test_auto_mode_uses_eager_launch_on_cpu(self):
        """Auto mode should avoid retaining replay commands on CPU."""
        device = wp.get_device("cpu")
        cache = WarpLaunchCache(device=device, mode="auto")
        output = wp.zeros(4, dtype=wp.float32, device=device)

        cache.launch(_increment, dim=4, inputs=[output])

        self.assertFalse(cache._entries)
        np.testing.assert_array_equal(output.numpy(), np.ones(4, dtype=np.float32))

    def test_invalid_mode_is_rejected(self):
        """The constructor should reject unknown execution modes."""
        with self.assertRaisesRegex(ValueError, "Invalid Warp launch mode"):
            WarpLaunchCache(device=self.device, mode="capture")  # type: ignore[arg-type]
    ...
    def test_debug_mode_rejects_changed_static_array(self):
        ...
        cache.launch(_affine, dim=4, inputs=[source_a, 1.0], outputs=[output])
        with self.assertRaisesRegex(RuntimeError, "static argument 0 changed"):
            cache.launch(_affine, dim=4, inputs=[source_b, 1.0], outputs=[output])
    ...
    def test_debug_mode_rejects_changed_static_scalar(self):
        ...
        cache.launch(_affine, dim=4, inputs=[source, 1.0], outputs=[output])
    ...
    def test_debug_mode_accepts_new_proxy_for_same_array(self):
        ...
        cache.launch(_affine, dim=4, inputs=[ProxyArray(source), 2.0], outputs=[output])
        cache.launch(_affine, dim=4, inputs=[ProxyArray(source), 2.0], outputs=[output])
    ...
    def test_zero_dim_is_noop_and_does_not_populate_cache(self):
        ...
        self.cache.launch(_increment, dim=(4, 0), inputs=[output])
    ...
    def test_fixed_device_fast_path_skips_device_lookup(self):
        ...
        output = wp.zeros(4, dtype=wp.float32, device=self.device)
        self.cache.launch(_increment, dim=4, inputs=[output])
    ...
    def test_invalidate_site_and_reset(self):
        ...
        self.cache.launch(_increment, dim=1, inputs=[output_a], site="a")
        self.cache.launch(_increment, dim=1, inputs=[output_b], site="b")
    ...
    def test_invalidate_accepts_tensor_site_identity(self):
        ...
        self.cache.launch(_increment, dim=1, inputs=[output], site=site)
    ...
    def test_reset_synchronizes_only_nonempty_cache_device(self):
        ...
        empty_cache = WarpLaunchCache(device=self.device, mode="eager")
        with patch.object(wp, "synchronize_device", wraps=wp.synchronize_device) as synchronize:
            empty_cache.reset()
            synchronize.assert_not_called()

            output = wp.zeros(1, dtype=wp.float32, device=self.device)
            self.cache.launch(_increment, dim=1, inputs=[output])
            self.cache.reset()
    ...
    def test_cold_recording_composes_with_cuda_capture(self):
        """A first recorded launch should become a node in an outer capture."""
        output = wp.zeros(4, dtype=wp.float32, device=self.device)
    ...
    def test_replay_composes_with_capture_on_explicit_stream(self):
        """A recorded launch should use the stream selected by an outer capture."""
        output = wp.zeros(4, dtype=wp.float32, device=self.device)
        stream = wp.Stream(self.device)
        self.cache.launch(_increment, dim=4, inputs=[output], stream=stream)
    ...
    def test_captured_graph_survives_command_reset_while_arguments_live(self):
        """A graph should retain launch data while its argument storage remains alive."""
        output = wp.zeros(4, dtype=wp.float32, device=self.device)
        self.cache.launch(_increment, dim=4, inputs=[output])
        ...
        self.cache.reset()  # ``output`` remains the graph argument's strong owner.
        wp.capture_launch(capture.graph)
        np.testing.assert_array_equal(output.numpy(), np.full(4, 2.0, dtype=np.float32))
