...
def test_non_concatenated_obs_groups_contain_all_terms(device):
    ...
    expected_policy_terms = [
        "actions", "joint_pos", "joint_vel", "object", "cube_positions", 
    ...
    assert len(policy_space.spaces) == len(expected_policy_terms), \
        f"Policy group should have {len(expected_policy_terms)} terms, got {len(policy_space.spaces)}: {list(policy_space.spaces.keys())}"
    ...
    assert len(subtask_space.spaces) == len(expected_subtask_terms), \
        f"Subtask_terms group should have {len(expected_subtask_terms)} terms, got {len(subtask_space.spaces)}: {list(subtask_space.spaces.keys())}"
