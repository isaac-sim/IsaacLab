...
Tests:
    - uv run --frozen --extra isaacsim isaaclab train --rl_library rsl_rl
        --task Isaac-Cartpole-Direct --num_envs 16 --max_iterations 5 physics=physx
        -> verify training completes from the committed lockfile (PhysX via the isaacsim extra)
"""
...
# Canonical cartpole probe, with the physics selector swapped to PhysX. The isaacsim
# extra is conflict-forked from the base install, so uv syncs the environment with the
# Isaac Sim wheels before running.
_PHYSX_TRAIN_CMD = [
    "train",
    "--rl_library",
    "rsl_rl",
...
class Test_Uv_Run_Isaacsim_Trains_Cartpole:
    """``uv run --extra isaacsim`` from the committed lockfile trains Cartpole on PhysX.
    ...
    def test_uv_run_isaacsim_from_committed_lock_trains_cartpole(self, isaaclab_root, tmp_path):
        """Verify ``uv run --frozen --extra isaacsim isaaclab train`` completes on PhysX."""
        result = run_cmd(
            ["uv", "run", "--frozen", "--extra", "isaacsim", "isaaclab", *_PHYSX_TRAIN_CMD],
            cwd=isaaclab_root,
            env={
