...
def test_initialization_fixed_base(sim, num_articulations, device):
    ...
    for _ in range(10):
        ...
        assert torch.allclose(articulation.data.root_state_w, default_root_state)
...
def test_initialization_floating_base_made_fixed_base(sim, num_articulations):
    ...
    articulation_cfg.spawn.rigid_props = sim_utils.RigidBodyPropertiesCfg(kinematic_enabled=True)
...
def test_initialization_fixed_base_made_floating_base(sim, num_articulations):
    ...
    articulation_cfg.spawn.rigid_props = sim_utils.RigidBodyPropertiesCfg(kinematic_enabled=False)
    articulation, _ = generate_articulation(articulation_cfg, num_articulations, device=sim.device)
...
def test_out_of_range_default_joint_vel(sim):
...
@pytest.mark.parametrize("num_articulations", [1, 2])
def test_joint_limits(sim, num_articulations):
    ...
    limits = torch.zeros(num_articulations, articulation.num_joints, 2, device=sim.device)
    limits[..., 0] = (
        torch.rand(num_articulations, articulation.num_joints, device=sim.device) + 5.0
    ) * -1.0
    limits[..., 1] = torch.rand(num_articulations, articulation.num_joints, device=sim.device) + 5.0
    articulation.write_joint_limits_to_sim(limits)

    # Check new limits are in place
    assert torch.allclose(articulation._data.joint_limits, limits)
    assert torch.allclose(articulation._data.default_joint_pos, default_joint_pos)

    # Set new joint limits with indexing
    env_ids_tensor = torch.arange(1, device=sim.device)
    joint_ids_tensor = torch.arange(2, device=sim.device)
    limits = torch.zeros(len(env_ids_tensor), len(joint_ids_tensor), 2, device=sim.device)
    limits[..., 0] = (torch.rand(len(env_ids_tensor), len(joint_ids_tensor), device=sim.device) + 5.0) * -1.0
    limits[..., 1] = torch.rand(len(env_ids_tensor), len(joint_ids_tensor), device=sim.device) + 5.0
    articulation.write_joint_limits_to_sim(limits, env_ids=env_ids_tensor.tolist(), joint_ids=joint_ids_tensor.tolist())

    # Check new limits are in place
    assert torch.allclose(articulation._data.joint_limits[env_ids_tensor][:, joint_ids_tensor], limits)
    assert torch.allclose(articulation._data.default_joint_pos, default_joint_pos)

    # Set new joint limits that invalidate default joint pos with indexing
    limits = torch.zeros(len(env_ids_tensor), len(joint_ids_tensor), 2, device=sim.device)
    limits[..., 0] = torch.rand(len(env_ids_tensor), len(joint_ids_tensor), device=sim.device) * -0.1
    limits[..., 1] = torch.rand(len(env_ids_tensor), len(joint_ids_tensor), device=sim.device) * 0.1
    articulation.write_joint_limits_to_sim(limits, env_ids=env_ids_tensor.tolist(), joint_ids=joint_ids_tensor.tolist())

    # Check if all values are within the bounds
    within_bounds = (
        articulation._data.default_joint_pos[env_ids_tensor][:, joint_ids_tensor] >= limits[..., 0]
    ) & (articulation._data.default_joint_pos[env_ids_tensor][:, joint_ids_tensor] <= limits[..., 1])
...
def test_loading_gains_from_usd(sim, num_articulations):
    ...
    assert torch.allclose(articulation.actuators["body"].stiffness, expected_stiffness)
    assert torch.allclose(articulation.actuators["body"].damping, expected_damping)

@pytest.mark.parametrize("num_articulations", [1, 2])
def test_setting_gains_from_cfg(sim, num_articulations):
...
def test_setting_velocity_limit_implicit(sim, num_articulations, device, vel_limit_sim, vel_limit):
    ...
    # check data buffer
    assert torch.allclose(articulation.data.joint_velocity_limits, physx_vel_limit)
    ...
    if vel_limit_sim is None:
        ...
        assert torch.allclose(physx_vel_limit, expected_velocity_limit)
...
def test_body_root_state(sim, num_articulations, device, with_offset):
    ...
    articulation.root_physx_view.set_coms(com, env_idx.tolist())

    # check they are set
    assert torch.allclose(articulation.root_physx_view.get_coms(), com)
    ...
    for i in range(50):
        ...
        if with_offset:
            ...
            # LINK state
            # pose
            assert torch.allclose(root_state_w[..., :7], root_link_state_w[..., :7])
            assert torch.allclose(body_state_w[..., :7], body_link_state_w[..., :7])
            ...
            # ang_vel
            assert torch.allclose(root_state_w[..., 10:], root_link_state_w[..., 10:])
            assert torch.allclose(body_state_w[..., 10:], body_link_state_w[..., 10:])
            ...
            assert torch.allclose(pos_gt, body_com_state_w[..., :3], atol=1e-3, rtol=1e-1)

            # orientation
            com_quat_b = articulation.data.com_quat_b
            com_quat_w = math_utils.quat_mul(body_link_state_w[..., 3:7], com_quat_b)
            assert torch.allclose(com_quat_w, body_com_state_w[..., 3:7])
            assert torch.allclose(com_quat_w[:, 0, :], root_com_state_w[..., 3:7])

            # linear vel, and angular vel
            assert torch.allclose(root_state_w[..., 7:], root_com_state_w[..., 7:])
            assert torch.allclose(body_state_w[..., 7:], body_com_state_w[..., 7:])
        else:
            # single joint center of masses are at link frames so they will be the same
            assert torch.allclose(root_state_w, root_link_state_w)
            assert torch.allclose(root_state_w, root_com_state_w)
            assert torch.allclose(body_state_w, body_link_state_w)
            assert torch.allclose(body_state_w, body_com_state_w)

@pytest.mark.parametrize("num_articulations", [1, 2])
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
@pytest.mark.parametrize("with_offset", [True, False])
@pytest.mark.parametrize("state_location", ["com", "link"])
def test_write_root_state(sim, num_articulations, device, with_offset, state_location):
    ...
    for i in range(10):
        ...
        if state_location == "com":
            if i % 2 == 0:
                ...
                articulation.write_root_com_state_to_sim(rand_state, env_ids=env_idx.tolist())
        elif state_location == "link":
            if i % 2 == 0:
                articulation.write_root_link_state_to_sim(rand_state)
            else:
                articulation.write_root_link_state_to_sim(rand_state, env_ids=env_idx.tolist())

        if state_location == "com":
            assert torch.allclose(rand_state, articulation.data.root_com_state_w)
        elif state_location == "link":
            assert torch.allclose(rand_state, articulation.data.root_link_state_w)
