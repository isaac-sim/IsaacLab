                                    ...
                                    if sim_limit is not None and act_limit is not None:
                                        ...
                                        if actuator == "implicit":
                                            if sim_limit is not None and act_limit is None:
                                                expected_max_dof_vel = torch.tensor(sim_limit, device=device).repeat(
                                                    num_articulations
                                                )
                                            elif sim_limit is None and act_limit is not None:
                                                expected_max_dof_vel = torch.tensor(act_limit, device=device).repeat(
                                                    num_articulations
                                                )
                                            torch.testing.assert_close(
                                                expected_max_dof_vel,
                                                articulation.root_physx_view.get_dof_max_velocities(),
                                            )
