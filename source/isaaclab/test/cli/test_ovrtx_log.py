...
def test_replayed_log_is_attributed_per_test_and_saved_whole(tmp_path: Path) -> None:
    ...
    test's failure carries the renderer output produced while it ran. With the artifact directory named,
    each test's log is also saved under a directory of its own name -- the copy that survives the caps, so a
    run whose renderer log outgrows them is only diagnosable from the saved file. What is saved is that
    test's own range of a log the renderer keeps appending to for the lifetime of the process; saving the
    file whole instead would credit beta with alpha's output, and every test after them with both.
    A crashing renderer leaves a dump next to its log, which is why the log's directory is saved with it;
    subdirectories are left alone, since the log's directory is the system temp directory.
    """
    ...
    assert sorted(path.name for path in save_dir.iterdir()) == ["test_alpha.0", "test_beta.0"]
    # The saved copy is unbounded where the replay is capped.
    ...
    saved_beside_beta = {path.name for path in (save_dir / "test_beta.0").iterdir()}
    assert {"ovrtx_renderer.log", "ovrtx_crash.dmp"} <= saved_beside_beta
    assert "somebody_elses_tree" not in saved_beside_beta
...
def test_only_tests_that_write_renderer_output_are_saved(tmp_path: Path) -> None:
    ...
    rather than failing the test that ran without one. Once one test has built a renderer, the tests
    after it still save only what they wrote themselves: the log outlives the test that wrote it, so
    finding it there says nothing about the test now running. Saving on its existence alone gave a
    directory to every test that followed the first renderer -- skips included, which is most of a run
    parametrized over backends a task does not support.
    """
    save_dir = tmp_path / "ovrtx-logs"
    ...
    assert b"3 passed, 1 skipped" in output
...
def test_log_left_behind_by_a_previous_owner_is_discarded(tmp_path: Path) -> None:
    ...
    """
    # the renderer default under the redirected temp directory
    (tmp_path / "ovrtx_renderer.log").write_bytes(b"[omni.rtx] stale-line\n" * 10)
