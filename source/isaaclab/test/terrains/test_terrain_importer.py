...
def test_grid_clone_env_origins(device, env_spacing, num_envs):
    """Tests that TerrainImporter env origins form a valid grid with correct spacing."""
    with build_simulation_context(device=device, auto_add_lighting=True) as sim:
        ...
        env_origins = terrain_importer.env_origins

        # check correct number of origins
        assert env_origins.shape == (num_envs, 3)

        # check all z values are zero (flat ground)
        torch.testing.assert_close(env_origins[:, 2], torch.zeros(num_envs, device=device), rtol=1e-5, atol=1e-5)

        # check minimum spacing between any two origins is at least env_spacing (or close to it)
        if num_envs > 1:
            # for efficiency, just check that neighbors have correct spacing
            unique_x = torch.unique(env_origins[:, 0])
            unique_y = torch.unique(env_origins[:, 1])
            if len(unique_x) > 1:
                x_spacing = torch.diff(torch.sort(unique_x).values).min()
                assert x_spacing >= env_spacing - 1e-5, f"X spacing {x_spacing} is less than {env_spacing}"
            if len(unique_y) > 1:
                y_spacing = torch.diff(torch.sort(unique_y).values).min()
                assert y_spacing >= env_spacing - 1e-5, f"Y spacing {y_spacing} is less than {env_spacing}"
...
def _populate_scene(sim: SimulationContext, num_balls: int = 2048, geom_sphere: bool = False):
    ...
    # Clone the scene using grid_transforms and usd_replicate
    env_spacing = 2.0
    env_origins, _ = grid_transforms(num_balls, env_spacing, device=sim.device)
    env_indices = torch.arange(num_balls, dtype=torch.long, device=sim.device)
    usd_replicate(sim.stage, [env_fmt.format(0)], [env_fmt], env_indices, positions=env_origins)
