...
def test_terrain_importer_env_origins(device, env_spacing, num_envs):
    ...
    with build_simulation_context(device=device, auto_add_lighting=True) as sim:
        ...
        assert terrain_importer.env_origins_pa.warp.dtype == wp.vec3f
        assert terrain_importer.env_origins_pa.warp.shape == (num_envs,)
        assert terrain_importer_origins.shape == (num_envs, 3)
        assert terrain_importer.env_origins_pa.warp is terrain_importer.env_origins_pa.warp
        assert terrain_importer_origins.data_ptr() == terrain_importer.env_origins_pa.warp.ptr
