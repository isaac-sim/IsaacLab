...
* Changed the installation model of :meth:`~isaaclab.cli.commands.install.command_install`
  ...
  ``isaaclab_tasks_experimental``, ``isaaclab_visualizers``) are now always installed
  by ``./isaaclab.sh -i``. Optional submodules (``contrib``, ``mimic``, ``ov``,
  ``teleop``) and automatic extra feature sets (``newton``, ``rl[...]``,
  ``visualizer[...]``) are installed by ``./isaaclab.sh -i`` / ``./isaaclab.sh -i all``.
  Optional submodule dependency extras require selectors, so rlinf dependencies are
