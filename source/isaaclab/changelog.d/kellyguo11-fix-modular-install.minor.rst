...
* Changed the installation model of :meth:`~isaaclab.cli.commands.install.command_install`
  ...
  ``isaaclab_visualizers``) are now always installed by ``./isaaclab.sh -i``.
  Optional heavy submodules (``mimic``, ``teleop``) and extra feature sets
  (``newton``, ``rl[...]``, ``visualizer[...]``, ``contrib``, ``ov``) are installed on
  request. Old per-submodule tokens (e.g. ``assets``, ``tasks``, ``physx``) now emit a
  warning and are skipped gracefully. Migrate using the table below:
  ...
  +----------------------------------------------+-------------------------------------------+
  | ``./isaaclab.sh -i assets,tasks,ov,rl[...]`` | ``./isaaclab.sh -i ov,rl[...]``           |
  +----------------------------------------------+-------------------------------------------+
