...
* Added the ``--isaacsim_source`` CLI option, which incrementally builds Isaac Sim from a source checkout,
  ...
  points ``uv`` at that directory through ``find-links`` in ``pyproject.toml``, creates and
  pins the ``isaacsim-local`` extra to the version it built, and re-resolves Isaac Sim from those
