...
* Changed ``train_multigpu`` to restrict console output to rank 0 by default, since every rank
  otherwise repeats the same startup, warning, and model-summary output once per GPU. Pass
