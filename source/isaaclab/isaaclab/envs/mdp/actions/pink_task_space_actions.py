...
class PinkInverseKinematicsAction(ActionTerm):
    ...
    def _initialize_hand_pose_transforms(self) -> None:
        ...
        hand_rot_offset = torch.tensor(self.cfg.controller.hand_rotational_offset, device=self.device)
    ...
    def _extract_controlled_frame_poses(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        for task_index in range(num_tasks):
            ...
            # Create pose matrix
            pose = math_utils.make_pose(position, math_utils.matrix_from_quat(quaternion))
            ...
            if self._asset.is_fixed_base:
                gravity = torch.zeros_like(
                    self._asset.root_physx_view.get_gravity_compensation_forces()[
                        :, torch.tensor(self._controlled_joint_ids)
                    ]
                )
            else:
                ...
                gravity = self._asset.root_physx_view.get_gravity_compensation_forces()[
                    :, torch.tensor(self._controlled_joint_ids) + self._physx_floating_joint_indices_offset
                ]
