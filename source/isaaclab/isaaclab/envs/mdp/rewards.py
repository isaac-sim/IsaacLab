...
class is_terminated_term(ManagerTermBase):
    ...
    * attr:`term_keys`: The termination terms to penalize. This can be a string, a list of strings
    ...
    def __call__(self, env: ManagerBasedRLEnv, term_keys: str | list[str] = ".*") -> torch.Tensor:
        # sum the term values to account for multiple terminations in the same step
...
def base_height_l2(
    ...
    if sensor_cfg is not None:
        # raise the target by the mean terrain height under the sensor
        sensor: RayCaster = env.scene[sensor_cfg.name]
        target_height = target_height + torch.mean(sensor.data.ray_hits_w.torch[..., 2], dim=1)
    return torch.square(asset.data.root_pos_w.torch[:, 2] - target_height)
...
def joint_deviation_l1(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    return torch.sum(torch.abs(joint_pos - asset.data.default_joint_pos.torch[:, asset_cfg.joint_ids]), dim=1)
...
def joint_pos_limits(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    out_of_limits = -(joint_pos - limits[..., 0]).clip(max=0.0) + (joint_pos - limits[..., 1]).clip(min=0.0)
...
def undesired_contacts(env: ManagerBasedRLEnv, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    is_contact = _max_contact_force_norm(env, sensor_cfg) > threshold
...
def desired_contacts(env, sensor_cfg: SceneEntityCfg, threshold: float = 1.0) -> torch.Tensor:
    ...
    is_contact = _max_contact_force_norm(env, sensor_cfg) > threshold
    return (~is_contact).all(dim=1).float()


def contact_forces(env: ManagerBasedRLEnv, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize contact forces as the amount of violations of the net contact force."""
    violation = _max_contact_force_norm(env, sensor_cfg) - threshold
    return torch.sum(violation.clip(min=0.0), dim=1)


def _max_contact_force_norm(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    """Largest net normal contact force norm [N] over the sensor history, shape (num_envs, num_bodies)."""
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    net_contact_forces = contact_sensor.data.net_normal_forces_w_history.torch[:, :, sensor_cfg.body_ids]
    return torch.max(torch.linalg.norm(net_contact_forces, dim=-1), dim=1)[0]
