...
def joint_torques_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.sum(torch.square(asset.actuators.applied_torque.torch[:, asset_cfg.joint_ids]), dim=1)
...
def applied_torque_limits(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    out_of_limits = torch.abs(
        asset.actuators.applied_torque.torch[:, asset_cfg.joint_ids]
        - asset.actuators.computed_torque.torch[:, asset_cfg.joint_ids]
    )
    return torch.sum(out_of_limits, dim=1)
