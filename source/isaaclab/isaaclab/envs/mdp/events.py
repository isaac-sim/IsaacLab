...
class _RandomizeFixedTendonPropertiesPhysx(ManagerTermBase):
    def __init__(
            self, cfg: EventTermCfg, env: ManagerBasedEnv, asset: RigidObject | Articulation, asset_cfg: SceneEntityCfg
    ):
        self.asset_cfg = asset_cfg
...
class _RandomizeFixedTendonPropertiesNewton(ManagerTermBase):
    def __init__(
            self, cfg: EventTermCfg, env: ManagerBasedEnv, asset: RigidObject | Articulation, asset_cfg: SceneEntityCfg
    ):
        self.asset = asset
...
class randomize_fixed_tendon_parameters(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        manager_name = env.sim.physics_manager.__name__.lower()
        if "newton" in manager_name:
            self._impl = _RandomizeFixedTendonPropertiesNewton(cfg, env, self.asset, self.asset_cfg)
        else:
            self._impl = _RandomizeFixedTendonPropertiesPhysx(cfg, env, self.asset, self.asset_cfg)
