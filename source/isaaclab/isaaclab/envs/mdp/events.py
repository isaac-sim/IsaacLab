...
from ._randomization import _randomize_prop_by_op, _validate_scale_range
...
class randomize_rigid_body_material(ManagerTermBase):
    ...
    .. deprecated:: 3.0
        Select the backend term explicitly through ``EventTermCfg.func``.
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        warnings.warn(
            "isaaclab.envs.mdp.randomize_rigid_body_material is deprecated. "
            "Select the physics backend's event term explicitly with EventTermCfg.func.",
            DeprecationWarning,
            stacklevel=2,
        )
        self._impl = env.sim.physics_manager._create_legacy_material_randomizer(cfg, env)
...
class randomize_rigid_body_collider_offsets(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        warnings.warn(
            "isaaclab.envs.mdp.randomize_rigid_body_collider_offsets is deprecated. "
            "Select the physics backend's event term explicitly with EventTermCfg.func.",
            DeprecationWarning,
            stacklevel=2,
        )
        self._impl = env.sim.physics_manager._create_legacy_collider_randomizer(cfg, env)
...
class randomize_physics_scene_gravity(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        super().__init__(cfg, env)
        warnings.warn(
            "isaaclab.envs.mdp.randomize_physics_scene_gravity is deprecated. "
            "Select the physics backend's event term explicitly with EventTermCfg.func.",
            DeprecationWarning,
            stacklevel=2,
        )
        self._impl = env.sim.physics_manager._create_legacy_gravity_randomizer(cfg, env)
