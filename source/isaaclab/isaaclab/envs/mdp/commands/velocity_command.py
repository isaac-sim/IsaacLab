...
class UniformVelocityCommand(CommandTerm):
    ...
    def _update_metrics(self):
        ...
        self.metrics["error_vel_xy"] += (
            torch.linalg.norm(self.vel_command_b[:, :2] - wp.to_torch(self.robot.data.root_lin_vel_b)[s, :2], dim=-1)
            / max_command_step
        )
        self.metrics["error_vel_yaw"] += (
            torch.abs(self.vel_command_b[:, 2] - wp.to_torch(self.robot.data.root_ang_vel_b)[s, 2]) / max_command_step
        )
    ...
    def _update_command(self):
        ...
        if self.cfg.heading_command:
            ...
            heading_error = math_utils.wrap_to_pi(
                self.heading_target[env_ids] - wp.to_torch(self.robot.data.heading_w)[self._robot_data_slice][env_ids]
            )
    ...
    def _debug_vis_callback(self, event):
        ...
        s = self._robot_data_slice
        # -- base state
        base_pos_w = wp.to_torch(self.robot.data.root_pos_w)[s].clone()
        base_pos_w[:, 2] += 0.5
        # -- resolve the scales and quaternions
        vel_des_arrow_scale, vel_des_arrow_quat = self._resolve_xy_velocity_to_arrow(self.command[:, :2])
        vel_arrow_scale, vel_arrow_quat = self._resolve_xy_velocity_to_arrow(
            wp.to_torch(self.robot.data.root_lin_vel_b)[s, :2]
        )
        # display markers
    ...
    def _resolve_xy_velocity_to_arrow(self, xy_velocity: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        base_quat_w = wp.to_torch(self.robot.data.root_quat_w)[self._robot_data_slice]
