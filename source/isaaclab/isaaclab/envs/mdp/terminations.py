...
def joint_pos_out_of_manual_limit(
    ...
    if asset_cfg.joint_ids is None:
        asset_cfg.joint_ids = slice(None)
    joint_pos = asset.data.joint_pos.torch[:, asset_cfg.joint_ids]
    return torch.any((joint_pos < bounds[0]) | (joint_pos > bounds[1]), dim=1)
...
def illegal_contact(env: ManagerBasedRLEnv, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    max_force_norm = torch.max(torch.linalg.norm(net_contact_forces, dim=-1), dim=1)[0]
    return torch.any(max_force_norm > threshold, dim=1)
