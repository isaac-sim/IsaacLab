...
class SubTaskConfig:
    ...
    selection_strategy_kwargs: dict = field(default_factory=dict)
...
class SubTaskConstraintConfig:
    ...
    coordination_scheme: SubTaskConstraintCoordinationScheme = field(
        default_factory=lambda: deepcopy(SubTaskConstraintCoordinationScheme.REPLAY)
    )
...
class MimicEnvCfg:
    ...
    datagen_config: DataGenConfig = field(default_factory=DataGenConfig)

    # Dictionary of list of subtask configurations for each end-effector.
    # Keys are end-effector names.
    subtask_configs: dict[str, list[SubTaskConfig]] = field(default_factory=dict)

    # List of configurations for subtask constraints
    task_constraint_configs: list[SubTaskConstraintConfig] = field(default_factory=list)
