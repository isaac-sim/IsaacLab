...
"""XR configuration classes and utilities.

.. deprecated::
    This module has moved to :mod:`isaaclab_teleop.xr_cfg`.
    Importing from ``isaaclab.devices.openxr.xr_cfg`` is deprecated and will be
...
try:
    ...
    from isaaclab_teleop.xr_cfg import *  # noqa: F401,F403

    warnings.warn(
        "Importing from isaaclab.devices.openxr.xr_cfg is deprecated. "
        "Please import from isaaclab_teleop.xr_cfg instead.",
        DeprecationWarning,
        stacklevel=2,
    )
except ImportError:
    # isaaclab_teleop is not installed; provide original definitions as fallback
    # so that the base isaaclab package continues to work standalone.
    ...
    class XrAnchorRotationMode(enum.Enum):
        ...
        """Follow prim rotation mode with smooth interpolation via slerp."""
    ...
    class XrCfg:
        ...
        anchor_rotation_custom_func: Callable[[np.ndarray, np.ndarray], np.ndarray] = (
