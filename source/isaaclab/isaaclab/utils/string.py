...
def resolve_matching_names_values(
    ...
    Args:
        ...
        list_of_strings: A list of strings to match.
        preserve_order: Whether to preserve the order of the query keys in the returned values. Defaults to False.
        match_all: Whether to require that all keys in the dictionary get matched. Defaults to True.
    ...
    Raises:
        ...
        ValueError: When not all regular expressions in the data keys are matched (if match_all is True).
    """
    # check valid input
    ...
    # check that all regular expressions are matched (only if match_all is True)
    if match_all and not all(keys_match_found):
        # make this print nicely aligned for debugging
        msg = "\n"
