...
@wp.kernel(enable_backward=False)
def raycast_dynamic_meshes_masked_kernel(
    env_mask: wp.array(dtype=wp.bool),
    mesh: wp.array2d(dtype=wp.uint64),
    ray_starts: wp.array2d(dtype=wp.vec3),
    ray_directions: wp.array2d(dtype=wp.vec3),
    ...
    """Performs ray-casting against multiple dynamic meshes for masked environments.

    Extends :func:`raycast_dynamic_meshes_kernel` with environment masking.
