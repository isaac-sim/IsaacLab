...
"""Clone-plan construction from asset cfgs."""
...
def clone_plan_from_env_0(
    ...
    Auto-populates :attr:`~isaaclab.cloner.ClonePlan.cfg_rows` from
    :data:`~isaaclab.cloner.REPLICATION_QUEUE`, including only cfgs whose ``prim_path``
    falls under the env-root prefix of ``destination``. Must be called *after* all asset
    ...
    Returns:
        A :class:`~isaaclab.cloner.ClonePlan` with a single source row covering every env.
    """
