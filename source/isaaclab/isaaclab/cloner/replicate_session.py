...
def replicate(plan: ClonePlan, *, stage: Usd.Stage, replicate_physics: bool = True) -> None:
    ...
    spawner. It replicates through :class:`pxr.Sdf` alone, so it also applies to kit-less runs,
    where a USD-reading renderer such as OVRTX would otherwise see only ``env_0``.
    With ``replicate_physics=False`` physics contexts are dropped; USD replication still fires
    when the cfg has a spawner or explicitly requests it.
