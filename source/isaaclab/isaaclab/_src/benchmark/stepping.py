...
class EnvironmentStepTimingRecorder(AbstractContextManager):
    ...
    def __enter__(self) -> EnvironmentStepTimingRecorder:
        ...
        if self._measure_synchronized_step_breakdown:
            ...
            from isaaclab._src.utils.timer import Timer  # noqa: PLC0415
...
def run_play_loop(env, policy, num_steps: int) -> tuple[list[float], MeanStd | None, MeanStd | None, float | None]:
    ...
    Returns:
        ...
        ``ep_length`` are :class:`~isaaclab.benchmark.schema.MeanStd`
    ...
    from isaaclab._src.benchmark.metrics import mean_std_peak  # noqa: PLC0415
