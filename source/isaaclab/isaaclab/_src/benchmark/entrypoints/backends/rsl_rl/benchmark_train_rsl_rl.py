...
if TYPE_CHECKING:
    from isaaclab._src.benchmark import BenchmarkResult
...
def _parse_args(argv: list[str]):
    ...
    from isaaclab._src.app import add_launcher_args
    ...
    from isaaclab._src.benchmark.distributed import validate_distributed_args
    from isaaclab._src.benchmark.entrypoints.early_stop import add_success_cli_args
...
def run(argv: list[str]) -> BenchmarkResult | None:
    ...
    from isaaclab._src.app import launch_simulation
    ...
    from isaaclab._src.benchmark.distributed import DistributedContext
    from isaaclab._src.benchmark.metrics import RL_LIBRARY_DESCRIPTORS, parse_tf_logs
    from isaaclab._src.benchmark.schema import StartupTime
    ...
    from isaaclab._src.benchmark.entrypoints.early_stop import (
    ...
    with launch_simulation(env_cfg, args_cli):
        with contextlib.ExitStack() as cleanup:
            ...
            # lockstep, so rank 0's durations cover the whole world's work for that iteration.
            ...
            num_envs, steps_per_iteration = distributed.global_work(env.unwrapped.num_envs, agent_cfg.num_steps_per_env)
            collection_fps_series = [steps_per_iteration / c for c in coll if c > 0]
            total_fps_series = (
                # RSL-RL's own total_fps counts only this rank's environments.
                [steps_per_iteration / t for t in iteration_times_s if t > 0]
                if distributed.enabled
