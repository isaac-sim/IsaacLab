...
def lazy_export(
    ...
    * ``from isaaclab.envs.mdp import foo, bar`` — lazily re-exports
    ...
    * ``from isaaclab.envs.mdp import *`` — sets up a lazy fallback so that
    ...
    # -- Build lazy resolution for absolute named imports -----------------
    absolute_name_modules = {
        name: source_module for source_module, names in absolute_named.items() for name in names
    }
    for name in absolute_name_modules:
    ...
    def _named_getattr(name: str):
        source_module = absolute_name_modules.get(name)
