...
class BodyData:
    def __init__(self, root_newton_view: NewtonArticulationView, device: str) -> None:
        """Initializes the container for body data.

        Usually, bodies are all the rigid bodies belonging to an articulation.
    ...
    def _create_simulation_bindings(self):
        ...
        self._sim_bind_body_mass = self._root_newton_view.get_attribute("body_mass", NewtonManager.get_model())
        self._sim_bind_body_inertia = self._root_newton_view.get_attribute("body_inertia", NewtonManager.get_model())
