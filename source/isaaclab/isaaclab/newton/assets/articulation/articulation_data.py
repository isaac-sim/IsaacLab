...
from isaaclab.newton.assets.core.body_properties.body_data import BodyData
from isaaclab.newton.assets.core.joint_properties.joint_data import JointData
from isaaclab.newton.assets.core.kernels import vec13f, combine_pose_and_velocity_to_state
...
class ArticulationData(BaseArticulationData):
    def __init__(self, root_newton_view, device: str, frontend: Frontend = Frontend.TORCH):
        # Set the parameters
        self.device = device
        # Set the root articulation view
        # note: this is stored as a weak reference to avoid circular references between the asset class
        #  and the data container. This is important to avoid memory leaks.
        self._root_view = weakref.proxy(root_newton_view)
    ...
    def default_root_pose(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.default_root_pose)
    ...
    def default_root_vel(self) -> wp.array | torch.Tensor:
        ...
        This quantity is configured through the :attr:`isaaclab.assets.ArticulationCfg.init_state` parameter.
        """
        return self.maybe_to_torch(self._root_data.default_root_vel)
    ...
    def default_joint_pos(self) -> wp.array | torch.Tensor:
        ...
        This quantity is configured through the :attr:`isaaclab.assets.ArticulationCfg.init_state` parameter.
        """
        return self.maybe_to_torch(self._joint_data.default_joint_pos)
    ...
    def default_joint_vel(self) -> wp.array | torch.Tensor:
        ...
        This quantity is configured through the :attr:`isaaclab.assets.ArticulationCfg.init_state` parameter.
        """
        return self.maybe_to_torch(self._joint_data.default_joint_vel)
    ...
    def joint_target(self) -> wp.array | torch.Tensor:
        ...
        """
        return  self.maybe_to_torch(self._joint_data.joint_target)
    ...
    def joint_effort_target(self) -> wp.array | torch.Tensor:
        ...
        which are then set into the simulation.
        """
        return self.maybe_to_torch(self._joint_data.joint_effort_target)
    ...
    def computed_effort(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._joint_data.computed_effort)
    ...
    @property
    def joint_target_sim(self) -> wp.array | torch.Tensor:
        """Joint target. Shape is (num_instances, num_joints)."""
        return self.maybe_to_torch(self._joint_data.joint_target_sim)

    @property
    def joint_effort_sim(self) -> wp.array | torch.Tensor:
        """Joint effort. Shape is (num_instances, num_joints)."""
        return self.maybe_to_torch(self._joint_data.joint_effort_sim)
    ...
    @property
    def joint_control_mode_sim(self) -> wp.array | torch.Tensor:
        """Joint control mode. Shape is (num_instances, num_joints).
        ...
        * Velocity control: 2
        ...
        """
        return self.maybe_to_torch(self._joint_data.joint_control_mode_sim)

    @property
    def joint_stiffness_sim(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._joint_data.joint_stiffness_sim)

    @property
    def joint_damping_sim(self) -> wp.array | torch.Tensor:
        """Joint damping provided to the simulation. Shape is (num_instances, num_joints)

        In the case of explicit actuators, the value for the corresponding joints is zero.
        """
        return self.maybe_to_torch(self._joint_data.joint_damping_sim)

    @property
    def joint_armature(self) -> wp.array | torch.Tensor:
        """Joint armature provided to the simulation. Shape is (num_instances, num_joints)."""
        return self.maybe_to_torch(self._joint_data.joint_armature)
    ...
    def joint_friction_coeff(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_friction_coeff)
    ...
    def joint_pos_limits_lower(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_pos_limits_lower)
    ...
    def joint_pos_limits_upper(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_pos_limits_upper)
    ...
    def joint_vel_limits(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_vel_limits)
    ...
    def joint_effort_limits(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_effort_limits)
    ...
    def joint_pos(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_pos)

    @property
    def joint_vel(self) -> wp.array | torch.Tensor:
        """Joint velocities. Shape is (num_instances, num_joints)."""
        return self.maybe_to_torch(self._joint_data.joint_vel)
    ...
    def joint_dynamic_friction(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_dynamic_friction)

    @property
    def joint_viscous_friction(self) -> wp.array | torch.Tensor:
        """Joint viscous friction. Shape is (num_instances, num_joints)."""
        return self.maybe_to_torch(self._joint_data.joint_viscous_friction)
    ...
    def soft_joint_pos_limits(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._joint_data.soft_joint_pos_limits)
    ...
    @property
    def root_mass(self) -> wp.array | torch.Tensor:
        """Root mass ``wp.float32`` in the world frame. Shape is (num_instances,)."""
        return self.maybe_to_torch(self._root_data.root_mass)

    @property
    def root_inertia(self) -> wp.array | torch.Tensor:
        """Root inertia ``wp.mat33`` in the world frame. Shape is (num_instances, 9)."""
        return self.maybe_to_torch(self._root_data.root_inertia)
    ...
    def root_link_pose_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_link_pose_w)
    ...
    def root_link_vel_w(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._root_data.root_link_vel_w)
    ...
    def root_com_pose_w(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._root_data.root_com_pose_w)
    ...
    def root_com_vel_w(self) -> wp.array | torch.Tensor:
        ...
        relative to the world.
        """
        return self.maybe_to_torch(self._root_data.root_com_vel_w)
    ...
    def root_state_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_state_w)
    ...
    def root_link_state_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_link_state_w)
    ...
    def root_com_state_w(self) -> wp.array | torch.Tensor:
        ...
        The velocity is of the articulation root's center of mass frame.
        """
        return self.maybe_to_torch(self._root_data.root_com_state_w)
    
    @property
    def root_com_pose_b(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_com_pose_b)

    @property
    def root_com_pos_b(self) -> wp.array | torch.Tensor:
        ...
        rigid body's actor frame.
        """
        return self.maybe_to_torch(self._root_data.root_com_pos_b)
    
    @property
    def root_com_quat_b(self) -> wp.array | torch.Tensor:
        ...
        rigid body's actor frame.
        """
        return self.maybe_to_torch(self._root_data.root_com_quat_b)
    ...
    def body_link_pose_w(self) -> wp.array | torch.Tensor:
        ...
        The orientation is provided in (x, y, z, w) format.
        """
        return self.maybe_to_torch(self._body_data.body_link_pose_w)
    ...
    def body_link_vel_w(self) -> wp.array | torch.Tensor:
        ...
        relative to the world.
        """
        return self.maybe_to_torch(self._body_data.body_link_vel_w)
    ...
    def body_com_pose_w(self) -> wp.array | torch.Tensor:
        ...
        The orientation is provided in (x, y, z, w) format.
        """
        return self.maybe_to_torch(self._body_data.body_com_pose_w)
    ...
    def body_com_vel_w(self) -> wp.array | torch.Tensor:
        ...
        relative to the world.
        """
        return self.maybe_to_torch(self._body_data.body_com_vel_w)
    ...
    def body_state_w(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._body_data.body_state_w)
    ...
    def body_link_state_w(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._body_data.body_link_state_w)
    ...
    def body_com_state_w(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._body_data.body_com_state_w)
    ...
    def body_com_acc_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_acc_w)
    ...
    def body_com_pos_b(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_pos_b)
    ...
    def body_com_pose_b(self) -> wp.array | torch.Tensor:
        ...
        The orientation is provided in (x, y, z, w) format.
        """
        return self.maybe_to_torch(self._body_data.body_com_pose_b)
    ...
    def body_incoming_joint_wrench_b(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._joint_data.body_incoming_joint_wrench_b)
    ...
    def joint_acc(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._joint_data.joint_acc)
    ##
    # Derived Properties.
    ##
    ...
    def projected_gravity_b(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(self._root_data.projected_gravity_b)
    ...
    def heading_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.heading_w)
    ...
    def root_link_vel_b(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_link_vel_b)
    ...
    def root_com_vel_b(self) -> wp.array | torch.Tensor:
        ...
        Velocity is provided in the form of [wx, wy, wz, vx, vy, vz].
        """
        return self.maybe_to_torch(self._root_data.root_com_vel_b)
    ...
    def root_link_pos_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_link_pos_w)
    ...
    def root_link_quat_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_link_quat_w)
    ...
    def root_link_lin_vel_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_link_lin_vel_w)
    ...
    def root_link_ang_vel_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_link_ang_vel_w)
    ...
    def root_com_pos_w(self) -> wp.array | torch.Tensor:
        ...
        This quantity is the position of the actor frame of the root rigid body relative to the world.
        """
        return self.maybe_to_torch(self._root_data.root_com_pos_w)
    ...
    def root_com_quat_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_com_quat_w)
    ...
    def root_com_lin_vel_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_com_lin_vel_w)
    ...
    def root_com_ang_vel_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._root_data.root_com_ang_vel_w)
    ...
    def body_link_pos_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_link_pos_w)
    ...
    def body_link_quat_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_link_quat_w)
    ...
    def body_link_lin_vel_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_link_lin_vel_w)
    ...
    def body_link_ang_vel_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_link_ang_vel_w)
    ...
    def body_com_pos_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_pos_w)
    ...
    def body_com_quat_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_quat_w)
    ...
    def body_com_lin_vel_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_lin_vel_w)
    ...
    def body_com_ang_vel_w(self) -> wp.array | torch.Tensor:
        ...
        This quantity is the angular velocity of the articulation bodies' center of mass frame.
        """
        return self.maybe_to_torch(self._body_data.body_com_ang_vel_w)
    ...
    def body_com_lin_acc_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_lin_acc_w)
    ...
    def body_com_ang_acc_w(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_ang_acc_w)
    ...
    def body_com_quat_b(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._body_data.body_com_quat_b)

    @property
    def root_link_lin_vel_b(self) -> wp.array | torch.Tensor:
        ...
        its actor frame.
        """
        return self.maybe_to_torch(self._root_data.root_link_lin_vel_b)

    @property
    def root_link_ang_vel_b(self) -> wp.array | torch.Tensor:
        ...
        its actor frame.
        """
        return self.maybe_to_torch(self._root_data.root_link_ang_vel_b)

    @property
    def root_com_lin_vel_b(self) -> wp.array | torch.Tensor:
        ...
        its actor frame.
        """
        return self.maybe_to_torch(self._root_data.root_com_lin_vel_b)

    @property
    def root_com_ang_vel_b(self) -> wp.array | torch.Tensor:
        ...
        its actor frame.
        """
        return self.maybe_to_torch(self._root_data.root_com_ang_vel_b)
    ...
    def joint_pos_limits(self) -> wp.array | torch.Tensor:
        ...
        """
        return self.maybe_to_torch(self._joint_data.joint_pos_limits)
    ...
    def default_root_state(self) -> wp.array | torch.Tensor:
        ...
        return self.maybe_to_torch(state)
    ...
    def default_fixed_tendon_limit(self):
        return None
    def default_joint_friction(self):
        return None
    def default_joint_limits(self):
        return None
    def fixed_tendon_limit(self):
        return None
