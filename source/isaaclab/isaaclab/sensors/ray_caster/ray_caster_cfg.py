...
class RayCasterCfg(SensorBaseCfg):
    ...
    class_type: type[RayCaster] | str = "isaaclab.sensors.ray_caster.ray_caster:RayCaster"

    spawn: SensorFrameCfg | None = field(default_factory=SensorFrameCfg)
    ...
    **not** be a physics body.
    ...
    mesh_prim_paths: list[str] = REQUIRED
    ...
    * ``world`` if rays' starting positions and directions are always fixed. This is useful in combination
    ...
    pattern_cfg: PatternBaseCfg = REQUIRED
    ...
    **Newton backend only** (ignored by the PhysX and OvPhysX backends, which ray cast against the
    ...
    ray_cast_drift_range: dict[str, tuple[float, float]] = field(
        default_factory=lambda: {"x": (0.0, 0.0), "y": (0.0, 0.0), "z": (0.0, 0.0)}
    )
    ...
    visualizer_cfg: VisualizationMarkersCfg = field(
        default_factory=lambda: replace_config(RAY_CASTER_MARKER_CFG, prim_path="/Visuals/RayCaster")
    )
