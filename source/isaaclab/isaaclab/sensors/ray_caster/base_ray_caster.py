...
class BaseRayCaster(SensorBase):
    ...
    def _initialize_warp_meshes(self):
        ...
        if all(
            (mesh_prim_path, self._device) not in BaseRayCaster.meshes for mesh_prim_path in self.cfg.mesh_prim_paths
        ):
            raise RuntimeError(
    ...
    def _initialize_rays_impl(self):
        ...
        ray_starts_torch = ray_starts_torch.repeat(self._view_count, 1, 1)
        ray_directions_torch = ray_directions_torch.repeat(self._view_count, 1, 1)
