...
The optional PPISP processor resolves these sentinels before renderer setup.
The sensor owns the resulting processing state independently of the renderer.
"""
...
class CameraISPMode(StrEnum):
    ...
    Selects how the PPISP processor discovers the ISP configuration from the
    ...
    The processor walks the stage for a ``RenderProduct`` whose ``camera``
