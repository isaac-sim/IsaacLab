...
def _force_visualizer_cfg(marker_cfg: VisualizationMarkersCfg) -> VisualizationMarkersCfg:
    """Create a contact-force marker configuration with the expected arrow scale."""
    cfg = replace_config(marker_cfg, prim_path="/Visuals/ContactSensor")
    cast("UsdFileCfg", cfg.markers["arrow"]).scale = (0.04, 0.04, 0.2)
    return cfg
...
class ContactSensorCfg(SensorBaseCfg):
    ...
    If provided, a per-partner force matrix (:attr:`ContactSensorData.normal_force_matrix_w`) is
