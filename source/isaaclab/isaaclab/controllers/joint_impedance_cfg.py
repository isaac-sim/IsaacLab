...
class JointImpedanceControllerCfg:
    ...
    class_type: type | str = config_field("isaaclab.controllers.joint_impedance:JointImpedanceController")
    """The associated controller class."""

    command_type: str = config_field("p_abs")
    """Type of command: p_abs (absolute) or p_rel (relative)."""

    dof_pos_offset: Sequence[float] | None = config_field(None)
    """Offset to DOF position command given to controller. (default: None).

    If None then position offsets are set to zero.
    """

    impedance_mode: str = config_field(MISSING)
    """Type of gains: "fixed", "variable", "variable_kp"."""

    inertial_compensation: bool = config_field(False)
    ...
    stiffness: float | Sequence[float] = config_field(MISSING)
    """The positional gain for determining desired torques based on joint position error."""

    damping_ratio: float | Sequence[float] | None = config_field(None)
