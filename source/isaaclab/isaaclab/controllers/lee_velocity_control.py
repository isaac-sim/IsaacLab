...
class LeeVelController:
    ...
    def __init__(self, cfg: LeeVelControllerCfg, env):
        ...
        self.K_vel_range = torch.tensor(self.cfg.K_vel_range, device=env.device).repeat(env.num_envs, 1, 1)
    ...
    def __call__(self, command):
        ...
        forces = (acc - self.gravity) * self.mass
        self.wrench_command_b[:, 2] = torch.sum(
            math_utils.quat_apply(self.robot.data.root_quat_w, forces)[:, :, 2], dim=1
        )
