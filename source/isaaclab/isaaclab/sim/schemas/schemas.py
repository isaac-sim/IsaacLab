...
def _create_world_fixed_joint(articulation_prim: Usd.Prim, stage: Usd.Stage) -> None:
    ...
    used on kitless backends (e.g. Newton) where ``omni.physx`` is unavailable. Only the
    single-body (world-attached) case is handled, matching the fixed-root-link spawn path.
    ...
    if fix_root_link is not None:
        ...
        if existing_fixed_joint_prim is not None:
            ...
            try:
                ...
                # Kitless backends (e.g. Newton) do not ship ``omni.physx``; author the world
                # fixed joint directly with USD instead so fixed-base articulations can spawn.
