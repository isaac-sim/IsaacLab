...
from isaaclab.visualizers import NewtonVisualizerCfg, RerunVisualizerCfg, Visualizer
...
class SettingsHelper:
    """Helper for typed settings access."""
...
class SimulationContext:
    ...
    * Rendering and visualization
    ...
    def __init__(self, cfg: SimulationCfg | None = None):
        ...
        # SettingsManager handles carb integration internally
        self.settings = SettingsManager.instance()
        # Initialize carb settings if available (e.g., when SimulationApp is launched)
        self.settings.initialize_carb_settings()
    ...
    def clear_stage(cls) -> None:
        ...
        def _predicate(path: str) -> bool:
            if path == "/World":
                return False
            if prims_utils.from_prim_path_get_type_name(path) == "PhysicsScene":
                return False
            return True
