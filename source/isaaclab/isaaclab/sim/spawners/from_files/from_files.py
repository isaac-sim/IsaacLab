...
def _spawn_from_usd_file(
    ...
    **kwargs,
    ...
    if cfg.joint_drive_props is not None:
        # Transition shim, remove later: fragment lists dispatch through each fragment's callback;
        # legacy cfgs dispatch through class metadata so backend compatibility behavior stays in
        # the package that owns the cfg.
        joint_frags = (
        ...
        if joint_frags and all(isinstance(f, schemas.SchemaFragment) for f in joint_frags):
            ...
            modify_func = cfg.joint_drive_props._modify_func
            if not callable(modify_func):
                modify_func = string_to_callable(modify_func)
            modify_func(prim_path, cfg.joint_drive_props, stage)
...
def spawn_from_usd_with_compliant_contact_material(
    ...
    **kwargs,
    ...
    if stiff is not None or damp is not None:
        ...
        material_cfg_type = cfg._physics_material_cfg_type
        if not callable(material_cfg_type):
            material_cfg_type = string_to_callable(material_cfg_type)
        material_cfg = material_cfg_type(**material_kwargs)
