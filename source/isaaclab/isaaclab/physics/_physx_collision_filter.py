...
"""Compile backend-neutral collision policy into PhysX USD collision groups.
...
def apply_collision_filter(
    ...
    plan: ClonePlan,
    groups: dict[str, CollisionGroupCfg] | None,
) -> None:
    ...
    Args:
        ...
        groups: Optional backend-neutral manager policy.

    Raises:
        RuntimeError: If required scene/destination topology is missing or the generated namespace
            is already occupied.
    """
    isolate_environments = plan.isolate_environments
    has_policy = bool(groups)
    ...
    if isolate_environments and not has_policy:
        ...
        if not _authored_groups(stage) and not stage.GetPrimAtPath("/World/collisions").IsValid():
            _author_compact_environment_isolation(
                stage, physics_scene_path, "/World/collisions", env_paths, global_paths
            )
            return

    colliders = _discover_topology(stage, plan)
    if not colliders:
        return
    manager_policy = None if groups is None else CompiledCollisionFilter(groups, plan.env_template)
    scene_inverted = bool(scene_prim.GetAttribute("physxScene:invertCollisionGroupFilter").Get())
    authored_policy = _snapshot_authored_policy(stage, tuple(colliders), scene_inverted=scene_inverted)

    profiles: dict[_Profile, list[str]] = {}
    for path, world in colliders.items():
        profile = _Profile(
            world,
            () if manager_policy is None else manager_policy.memberships(path),
            authored_policy.memberships[path],
        )
...
def _author_compact_environment_isolation(
    stage: Usd.Stage,
    physics_scene_path: str,
    ...
    """Author the compact PhysX representation used when no other groups exist."""
    from pxr import Sdf, Usd, UsdGeom  # noqa: PLC0415

    scene_prim = stage.GetPrimAtPath(physics_scene_path)
    scene_prim.CreateAttribute("physxScene:invertCollisionGroupFilter", Sdf.ValueTypeNames.Bool).Set(True)
    root_layer = stage.GetRootLayer()
    with Usd.EditContext(stage, Usd.EditTarget(root_layer)):
        UsdGeom.Scope.Define(stage, collision_root_path)
    root_spec = root_layer.GetPrimAtPath(collision_root_path)

    def author_group(name: str, includes: list[str]) -> tuple[str, Sdf.RelationshipSpec]:
        path = f"{collision_root_path}/{name}"
        ...
        expansion = Sdf.AttributeSpec(
            group, "collection:colliders:expansionRule", Sdf.ValueTypeNames.Token, Sdf.VariabilityUniform
        )
        expansion.default = "expandPrims"
        members = Sdf.RelationshipSpec(group, "collection:colliders:includes", False)
        for member in includes:
            members.targetPathList.Append(member)
        return path, Sdf.RelationshipSpec(group, "physics:filteredGroups", False)

    with Sdf.ChangeBlock():
        global_group = author_group("global_group", global_paths) if global_paths else None
        if global_group is not None:
            global_group[1].targetPathList.Append(global_group[0])
        for index, env_path in enumerate(env_paths):
            env_group = author_group(f"group{index}", [env_path])
            env_group[1].targetPathList.Append(env_group[0])
            if global_group is not None:
                env_group[1].targetPathList.Append(global_group[0])
                global_group[1].targetPathList.Append(env_group[0])


def _discover_topology(stage: Usd.Stage, plan: ClonePlan) -> dict[str, int | None]:
    ...
    if plan.isolate_environments and len(env_ids) > 1:
        unresolved = sorted(
            path
    ...
    return {
        path: worlds[path]
        if plan.isolate_environments and not any(_is_at_or_below(path, root) for root in global_roots)
        else None
...
def _deinstance_groups(stage: Usd.Stage, group_paths: tuple[str, ...]) -> None:
    """Expand only instance roots containing collision groups that must be overridden."""
    from pxr import Usd  # noqa: PLC0415
