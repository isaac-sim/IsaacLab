...
* :func:`random_color_from_id` / :func:`color_hash` run on the host (Python), used to build color
  palettes and the ``"(r, g, b, a)"`` keys of ``camera.data.info`` id-to-label maps.
...
def color_hash(seed: int) -> int:
    """Host equivalent of :func:`color_hash_wp` (uint64 arithmetic, masked to uint32)."""
    mask64 = 0xFFFFFFFFFFFFFFFF
    h = seed & 0xFFFFFFFF
    h ^= h >> 16
    h = (h * 0x85EBCA6B) & mask64
    h ^= h >> 13
    h = (h * 0xC2B2AE35) & mask64
    h ^= h >> 16
    return h & 0xFFFFFFFF
...
def color_hash_wp(seed: wp.uint32) -> wp.uint32:
    """Simple hash for better color distribution. Used for colorization."""
