...
class IsaacRtxRenderer:
    ...
    def create_render_data(self, sensor: SensorBase) -> IsaacRtxRenderData:
        """Create render product and annotators for the tiled camera. See :meth:`~.base_renderer.BaseRenderer.create_render_data`."""
    ...
    def set_outputs(self, render_data: IsaacRtxRenderData, output_data: dict[str, torch.Tensor]):
        """Store reference to output buffers for writing during render. See :meth:`~.base_renderer.BaseRenderer.set_outputs`."""
        render_data.output_data = output_data

    def update_transforms(self) -> None:
        """No-op for Isaac RTX - uses USD scene directly. See :meth:`~.base_renderer.BaseRenderer.update_transforms`."""
        pass
    ...
    def update_camera(
        ...
        """No-op for Replicator - uses USD camera prims directly. See :meth:`~.base_renderer.BaseRenderer.update_camera`."""
        pass

    def render(self, render_data: IsaacRtxRenderData):
        """Extract data from annotators and write to output buffers. See :meth:`~.base_renderer.BaseRenderer.render`."""
    ...
    def write_output(
        ...
        """No-op for Isaac RTX - all outputs written in render(). See :meth:`~.base_renderer.BaseRenderer.write_output`."""
        pass

    def cleanup(self, render_data: IsaacRtxRenderData | None):
        """Detach annotators from render product. See :meth:`~.base_renderer.BaseRenderer.cleanup`."""
        if render_data:
