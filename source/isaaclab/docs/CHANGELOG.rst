...
* Documented :attr:`~isaaclab.sim.schemas.ArticulationRootPropertiesCfg.articulation_enabled`
  ...
  (Rule 3 -- single-namespace USD with verified Newton consumer);
  ``enabled_self_collisions`` moves to the PhysX subclass (Rule 2 -- dual-namespace USD,
  with a future Newton sibling cfg owning the ``newton:*`` namespace).
