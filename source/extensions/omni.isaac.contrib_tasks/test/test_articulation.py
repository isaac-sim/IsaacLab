...
# Pre-defined configs
##
from omni.isaac.orbit_assets import ANYMAL_C_CFG, FRANKA_PANDA_CFG, ANDROID_CFG  # isort:skip
...
class TestArticulation(unittest.TestCase):
    ...
    def test_initialization_floating_base(self):
        ...
        robot = Articulation(cfg=ANDROID_CFG.replace(prim_path="/World/Robot"))
    ...
    def test_external_force_on_single_body(self):
        ...
        # Robots
        robot_cfg = ANDROID_CFG # ANYMAL_C_CFG
    ...
    def test_external_force_on_multiple_bodies(self):
        ...
        # Robots
        robot_cfg = ANDROID_CFG # ANYMAL_C_CFG
        robot_cfg.spawn.func("/World/Android/Robot_1", robot_cfg.spawn, translation=(0.0, -0.5, 0.65))
        robot_cfg.spawn.func("/World/Android/Robot_2", robot_cfg.spawn, translation=(0.0, 0.5, 0.65))
        # create handles for the robots
        robot = Articulation(robot_cfg.replace(prim_path="/World/Android/Robot.*"))

        # Play the simulator
        self.sim.reset()

        # Find bodies to apply the force
        body_ids, _ = robot.find_bodies(".*_thigh_roll")
        joint_ids, _ = robot.find_joints(".*_hip_roll")
        # Sample a large force
        external_wrench_b = torch.zeros(robot.num_instances, len(body_ids), 6, device=self.sim.device)
