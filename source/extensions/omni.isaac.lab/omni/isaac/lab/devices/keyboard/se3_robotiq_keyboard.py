...
class Se3RobotiqKeyboard(DeviceBase):
    ...
    * gripper: a binary command to open or close the gripper.
    ...
    def __init__(self, pos_sensitivity: float = 0.4, rot_sensitivity: float = 0.8, gripper_sensitivity: float = 0.02):
        ...
        self._delta_gripper = np.zeros(2)  # (pinch, scissor)
    ...
    def __str__(self) -> str:
        ...
        msg += "\t----------------------------------------------\n"
        msg += "\tMove gripper (open/close): J/K\n"
        msg += "\tMove gripper scissor:      U/I\n"
    ...
    def _on_keyboard_event(self, event, *args, **kwargs):
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            if event.input.name == "L":
                self.reset()
            if event.input.name in ["J", "K", "U", "I"]:
        ...
        if event.type == carb.input.KeyboardEventType.KEY_RELEASE:
            if event.input.name in ["W", "S", "A", "D", "Q", "E"]:
                ...
                self._delta_rot -= self._INPUT_KEY_MAPPING[event.input.name]
            elif event.input.name in ["J", "K", "U", "I"]:
                self._delta_gripper -= self._INPUT_KEY_MAPPING[event.input.name]
    ...
    def _create_key_bindings(self):
        ...
        self._INPUT_KEY_MAPPING = {
            ...
            # scissor open/close
            "U": np.array([0, 1]) * self.gripper_sensitivity,
            "I": np.array([0, -1]) * self.gripper_sensitivity,
            # x-axis (forward)
