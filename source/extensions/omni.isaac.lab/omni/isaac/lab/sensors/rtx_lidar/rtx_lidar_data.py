...
class RtxLidarData:
    ...
    Each dictionary corresponds to an instance of a lidar. Each dictonary contains the following fields from the
    ...
    The product  of ticksPerScan, numChannels, and numEchos will be the same as the number of returns if you initialize the annotator with annotator.initialize(keepOnlyPositiveDistance=False) before attaching the render product.
    """
    ...
    The following keys will ALWAYS be returned:
        data: The position [x,y,z] of each return in meter expressed in the RtxLidarCfg.data_frame. If 'world' the  data will be returned realtive 
            to simulation world. If 'sensor' the data will be returned realative to sensor frame.
