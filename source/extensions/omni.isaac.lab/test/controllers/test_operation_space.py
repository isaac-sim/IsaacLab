...
from omni.isaac.lab.utils.math import compute_pose_error, subtract_frame_transforms, quat_rotate_inverse

##
# Pre-defined configs
##
from omni.isaac.lab_assets import FRANKA_PANDA_CFG, FRANKA_PANDA_HIGH_PD_CFG  # isort:skip
...
class TestOperationSpaceController(unittest.TestCase):
    ...
    def test_franka_pose_abs_fixed(self):
        ...
        opc_cfg = OperationSpaceControllerCfg(command_types=["pose_abs"], impedance_mode="fixed", stiffness=100.0, damping_ratio=1.0)
    ...
    def _run_op_space_controller(
        ...
        for count in range(1500):
            ...
            if count % 250 == 0:
                ...
                robot.set_joint_effort_target(joint_efforts, arm_joint_ids)
                robot.write_data_to_sim()
            ...
            pos_error_norm = torch.norm(pos_error, dim=-1)
            rot_error_norm = torch.norm(rot_error, dim=-1)
            print("pos_error_norm", pos_error_norm[0], "rot_error_norm", rot_error_norm[0])
