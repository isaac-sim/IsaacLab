...
def _domain_presets_for_docs(preset_map: dict[PresetTarget, list[str]]) -> list[str]:
    ...
    (for example observation presets named ``physx`` / ``newton_mjwarp``). Those
    are selected together via ``physics=`` and should not be repeated under
    ``presets=`` in the docs table.
    """
    typed_names = set(preset_map.get(PresetTarget.PHYSICS, [])) | set(preset_map.get(PresetTarget.RENDERER, []))
    return [name for name in preset_map.get(PresetTarget.DOMAIN, []) if name not in typed_names]
