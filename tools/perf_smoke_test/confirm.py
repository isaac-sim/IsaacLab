...
def make_docker_rerun(workspace: Path, ci_image_tag: str) -> RerunFn:
    ...
    def _rerun(bench_result: dict, attempt_dir: Path, attempt: int) -> float | None:
        ...
        docker_cmd = [
            ...
            "-v", f"{attempt_dir}:/tmp/bench_out",
            "-v", f"{workspace / 'jit-cache'}:/tmp/jit-cache",
            "-v", f"{workspace / 'kit-cache'}:/isaac-sim/kit/cache",
            "-v", f"{workspace}:/workspace/isaaclab",
            ci_image_tag, "-c", inner,
        ]
