...
def _resolve_matched(work: Path) -> bool:
    """Reproduce the gate's image-resolution step: is HEAD's era already recorded?"""
    manifest = image_era.load_manifest_from_git(remote="origin", repo_dir=work)
    _, matched = image_era.resolve_image(image_era.era_key_from_tree(work), manifest)
    return matched


def test_era_resolution_pins_recorded_eras_and_falls_back_otherwise(tmp_path) -> None:
    """Resolution across a full environment-change cycle: miss, then pin, then miss again.

    ``matched`` is what decides whether the gate pins an immutable image or falls
    back to the floating tag, so this asserts that signal end to end.
    """
    work = _init_repo_with_remote(tmp_path)
    (work / "docker").mkdir()
