...
def test_build_seed_plan_branches_ref_with_target_seeds_single_commit(repo) -> None:
    """A pinned invocation ('<sha>:develop', count 1) seeds exactly that commit.

    Seeding a fresh environment starts from one boundary commit via
    ``branches: "<sha>:develop"``; this locks that the branches path resolves a
    raw SHA and stamps the overridden target.
    """
    work = repo["work"]
...
def test_jit_cache_reuses_samples_and_isolates_backends(tmp_path: Path) -> None:
    ...
    newton_cache = seed_baselines._cache_bucket_path(
        cache_root, "release/2.0", "abcdef123456", "Isaac-Cartpole-Direct", "newton"
    )
    repeated_newton_cache = seed_baselines._cache_bucket_path(
        cache_root, "release/2.0", "abcdef123456", "Isaac-Cartpole-Direct", "newton"
    )
    physx_cache = seed_baselines._cache_bucket_path(
        cache_root, "release/2.0", "abcdef123456", "Isaac-Cartpole-Direct", "physx"
    )
...
def test_kit_cache_reuses_samples_and_isolates_backends(tmp_path: Path) -> None:
    ...
    newton_cache = seed_baselines._cache_bucket_path(
        cache_root, "release/2.0", "abcdef123456", "Isaac-Camera-Direct", "newton"
    )
    repeated_newton_cache = seed_baselines._cache_bucket_path(
        cache_root, "release/2.0", "abcdef123456", "Isaac-Camera-Direct", "newton"
    )
    physx_cache = seed_baselines._cache_bucket_path(
        cache_root, "release/2.0", "abcdef123456", "Isaac-Camera-Direct", "physx"
    )
