...
Thin gate wrapper over the Isaac Lab benchmark core that produces a schema-v1
:class:`~isaaclab.test.benchmark.schema.RuntimeBundle`. Warmup runs as a separate
excluded phase, and ``num_frames`` controls only measured steady-state steps,
matching the canonical runtime benchmark.

Usage example::

    ./isaaclab.sh -p tools/perf_smoke_test/perf_runtime.py \
        --task Isaac-Cartpole-Direct \
        --num_envs 4096 --num_frames 200 --warmup_frames 100 \
...
parser.add_argument(
    ...
    type=int,
    default=200,
    help="Number of measured environment steps to run after warmup.",
)
parser.add_argument(
    "--warmup_frames",
    type=int,
    default=0,
    help="Exact number of excluded warmup environment steps.",
)
...
def main(env_cfg, app_start_time_begin: int, app_start_time_end: int) -> None:
    ...
    with contextlib.closing(gym.make(args_cli.task, cfg=env_cfg)) as env:
        ...
        # Run warmup separately so ``num_frames`` always means measured steps,
        # matching the canonical runtime benchmark on current develop.
        warmup_step_times_s = stepping.run_runtime_warmup(env, args_cli.warmup_frames)
        environment_step_timer = stepping.EnvironmentStepTimingRecorder(env)
        with environment_step_timer, BenchmarkMonitor(benchmark, interval=1.0):
            measured_step_times = stepping.run_runtime_loop(env, args_cli.num_frames, reset=False)
        ...
        first_step_s = warmup_step_times_s[0] if warmup_step_times_s else measured_step_times[0]
        startup = StartupTime(
        ...
        bundle = builders.build_runtime_bundle(
            ...
            runtime=runtime,
            resources=resources,
            extra={"num_frames": args_cli.num_frames, "warmup_frames": args_cli.warmup_frames},
        )
