...
"""Selection expansion and filtering for registry-discovered task lists.
...
from tools.odin.discover import DiscoveredTask, DiscoveryError, filter_rows, rows_for_selection, write_task_list
...
_NO_PHYSICS = DiscoveredTask(task_id="Isaac-Intrinsic", scope="core", rl_libraries=("skrl",), physics=(), renderers=())
_TASKS = [_ANT, _CONTRIB, _NO_PHYSICS]


def test_standard_picks_one_library_per_task() -> None:
    rows = rows_for_selection(_TASKS, "standard")
...
def test_cross_backend_keeps_only_the_two_compared_backends() -> None:
    task = DiscoveredTask(
        ...
        scope="core",
        rl_libraries=("rsl_rl",),
        physics=("newton_mjwarp", "ovphysx", "newton_kamino"),
        renderers=(),
    )
    rows = rows_for_selection([task], "cross-backend")
...
def test_physx_is_dropped_when_ovphysx_is_also_declared() -> None:
    ...
    assert {row["physics"] for row in rows_for_selection([task], "standard")} == {"ovphysx"}


def test_physx_is_kept_when_ovphysx_is_absent() -> None:
    task = DiscoveredTask(task_id="Isaac-X", scope="core", rl_libraries=("rsl_rl",), physics=("physx",), renderers=())
    assert {row["physics"] for row in rows_for_selection([task], "standard")} == {"physx"}
...
def test_proxy_backend_is_never_dispatched() -> None:
    task = DiscoveredTask(
        ...
        scope="core",
        rl_libraries=("rsl_rl",),
        physics=("newton_mjwarp", "newton_mjwarp_vbd_proxy"),
        renderers=(),
    )
    assert {row["physics"] for row in rows_for_selection([task], "standard")} == {"newton_mjwarp"}


def test_task_without_physics_gets_exactly_one_row_with_no_token() -> None:
    rows = [row for row in rows_for_selection(_TASKS, "standard") if row["task_id"] == "Isaac-Intrinsic"]
    assert len(rows) == 1
...
def test_task_without_a_supported_library_is_dropped() -> None:
    ...
    assert rows_for_selection([task], "standard") == []


def test_rows_are_deterministic() -> None:
    assert rows_for_selection(_TASKS, "standard") == rows_for_selection(list(reversed(_TASKS)), "standard")


def test_unknown_selection_is_rejected() -> None:
    with pytest.raises(DiscoveryError, match="quantum"):
        rows_for_selection(_TASKS, "quantum")
...
def test_written_list_is_consumable_by_the_planner(tmp_path: Path) -> None:
    ...
    write_task_list(out, rows, meta={"selection": "standard", "row_count": len(rows)})
...
def test_provenance_header_is_ignored_by_the_planner(tmp_path: Path) -> None:
    out = tmp_path / "tasks.yaml"
    write_task_list(out, rows_for_selection(_TASKS, "standard"), meta={"selection": "standard"})
    assert yaml.safe_load(out.read_text())["discovery"]["selection"] == "standard"
