...
- **Do not trim the device axis where it is the sharding mechanism.** A module marked `device_split` is re-invoked once per device, and a session device lock skips the cases that do not match the running process. Narrowing those modules to one device deletes coverage instead of duplication. Check for `device_split` and for an autouse device-skip fixture before touching a device parametrize, and confirm the passed count does not drop.
