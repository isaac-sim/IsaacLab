...
These installation-integration tests have strict conventions — scope, directory
layout, filename/class/method naming, the `Setup:`/`Tests:` docstring format,
markers, isaacsim availability gating, the training assertion helper, and the
standard cartpole training command. They live in the co-located guide:
`source/isaaclab/test/install_ci/README.md`. Read it before adding or changing
