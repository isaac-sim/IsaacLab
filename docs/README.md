...
a temporary environment for the locked `test` extra, which combines test and
...
```batch
:: 1. Build the current documentation
uv run --isolated --locked --extra test -- cmd /c docs\make.bat current-docs
...
uv run --isolated --locked --extra test -- make -C docs multi-docs
...
```batch
:: 1. Build the multi-version documentation
uv run --isolated --locked --extra test -- cmd /c docs\make.bat multi-docs
