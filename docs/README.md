...
a temporary environment for the locked `docs` dependency group, leaving the
...
```batch
:: 1. Build the current documentation
uv run --isolated --locked --only-group docs -- cmd /c docs\make.bat current-docs
...
uv run --isolated --locked --only-group docs -- make -C docs multi-docs
...
```batch
:: 1. Build the multi-version documentation
uv run --isolated --locked --only-group docs -- cmd /c docs\make.bat multi-docs
