...
(``presets=isaac_rtx_renderer,...``) selects the Kit path. Newton physics (``presets=mjwarp,...``) or
...
.. list-table::
   ...
   * - ``mjwarp,...`` or ``newton_renderer,...`` (no Kit signals)
     - Newton GL (``"newton_gl"``)
     - ``newton.viewer.ViewerGL`` on the SDP Newton model
   * - ``mjwarp,...,ovrtx_renderer,...`` (OVRTX + Newton physics)
     - Newton GL (``"newton_gl"``)
     - ``newton.viewer.ViewerGL`` on the SDP Newton model
