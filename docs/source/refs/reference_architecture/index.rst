...
2. Design your assets or robot in any software of your choice and export it to USD using
   USD converters/importers such as the `CAD Converter`_, `URDF Importer`_, `MJCF Importer`_,
   `Onshape Importer`_, etc. More details are found in the `Importing Assets section`_ in the
   `Isaac Sim Reference Architecture`_. Isaac Lab's URDF and MJCF command-line converters can
   use either the Isaac Sim importer extensions or the standalone importer wheel.
