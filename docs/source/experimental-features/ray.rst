...
MLflow tracking is optional; commands that use its client add ``--with mlflow`` separately.
...
.. code-block:: bash
  ...
  uv run --extra ray --with mlflow python -m isaaclab_contrib.rl.ray.mlflow_to_local_tensorboard \
