...
.. code-block:: console
   ...
   usage: launch_app.py [-h] [--size SIZE] [--width WIDTH] [--height HEIGHT] [--visualizer none] [--livestream {0,1,2}]
   ...
   --visualizer none            Force display off at all times.
   --livestream {0,1,2}
...
As noted in the help message, the :class:`~app.AppLauncher` arguments (``--livestream``, ``--visualizer none``)
...
to if we had passed the ``--visualizer none`` flag because headlessness is implied by our ``LIVESTREAM``
