...
``Isaac-Ant``, ``Isaac-Pendulum-MARL-Direct``, and several locomotion tasks.
Passing ``physics=newton_kamino`` to another task does not automatically enable
Kamino; the task must define and validate its own ``newton_kamino`` preset.
