...
Isaac Lab provides multiple visualizer backends for debugging and monitoring simulations. Unlike renderers (which
handle sensor data for cameras), visualizers are designed for lightweight real-time monitoring, development,
...
.. list-table:: Visualizer Comparison
   ...
   * - Visualizer
     ...
     - Dependencies
   * - **Omniverse**
     - High-fidelity visualization, full feature support
   ...
   * - **Rerun**
     ...
     - Browser webviewer interface, time scrubbing, .rrd recording export
     - ``rerun-sdk``
...
Launch visualizers with ``--visualizer`` with default visualizer configurations.
...
Default visualizer configuration can be edited by changing values in the visualizer configuration classes which can be found in ``source/isaaclab/isaaclab/visualizers``.

You can also add and edit custom visualizers by adding new visualizer configuration to the simulation configuration:
...
The Omniverse visualizer provides high-fidelity visualization using Isaac Sim's viewport system. It supports
the full range of Isaac Lab features including visualization markers and live plots.
...
- Full Isaac Sim rendering capabilities
...
The Newton OpenGL visualizer provides a lightweight, high-performance OpenGL-based viewer with interactive
controls and minimal dependencies.

**Key Features:**

- Lightweight OpenGL rendering with low overhead
- Interactive camera controls (FPS-style navigation)
...
The Rerun visualizer provides a web-based interface with powerful features for recording, time scrubbing,
and analyzing simulation runs. Ideal for remote monitoring and post-training analysis.
...
To reduce any overhead, consider:
...
environments. For large-scale simulations, the Newton  visualizer is recommended as it can handle significantly higher environment counts than either Omniverse 
or Rerun. To still use Rerun, you can reduce the number of environments using ``--num_envs``:
...
.. note::
    ...
    A future feature may support visualizing only a subset of environments, which would improve 
    visualization performance and reduce resource usage while maintaining full-scale training.
