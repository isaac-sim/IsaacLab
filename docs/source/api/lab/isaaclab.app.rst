...
.. tab-set::
   ...
   .. tab-item:: :icon:`fa-brands fa-linux` Linux x86_64
      ...
      .. code-block:: bash
         ...
         uv run --extra isaacsim isaaclab demo quadrupeds

      Alternatively, set the environment variable inline for a single invocation:

      .. code-block:: bash

         LIVESTREAM=2 uv run --extra isaacsim isaaclab demo quadrupeds
   ...
   .. tab-item:: :icon:`fa-brands fa-linux` Linux aarch64 (DGX Spark)
      ...
      .. code-block:: bash
         ...
         export LIVESTREAM=2
         # run the python script
         LD_PRELOAD=/lib/aarch64-linux-gnu/libgomp.so.1 uv run --extra isaacsim isaaclab demo quadrupeds

      Alternatively, set the environment variable inline for a single invocation:

      .. code-block:: bash

         LIVESTREAM=2 LD_PRELOAD=/lib/aarch64-linux-gnu/libgomp.so.1 uv run --extra isaacsim isaaclab demo quadrupeds
