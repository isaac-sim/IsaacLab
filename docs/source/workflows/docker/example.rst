...
Example: running a script in the container
==========================================

This example walks through the full loop of building the container, running a script inside it, and
copying the results back to the host. It uses ``log_time.py``, a tutorial script that writes a log
file, so that there is a concrete artifact to retrieve.

For the setup and reference material behind each step, see :ref:`deployment-docker`. For Docker
itself, refer to `their official documentation <https://docs.docker.com/get-started/overview/>`_.

Building the container
----------------------

From the root of the Isaac Lab repository, run:
...
The example corresponds to the ``log_time.py`` script in the ``IsaacLab/scripts/tutorials/00_sim`` directory.
...
by typing ``exit``, we are back in the repository root on the host. We can then enter
...
We could return to the Isaac Lab Docker terminal environment by running ``./docker/container.py enter`` again,
...
the build process when we next run ``./docker/container.py start``, we may enter the following command to delete the **isaac-lab-base** image:
