...
.. code-block:: python
   ...
   benchmark.add_measurement(
       phase_name="simulation",
       metadata=StringMetadata(name="task", data="Isaac-Cartpole-Manager"),
   )
...
.. list-table::
   ...
   * - ``--task``
     - required
     - Environment task name (e.g., ``Isaac-Cartpole-Manager``)
   * - ``--num_envs``
     - ``4096``
     - Number of parallel environments
   * - ``--num_frames``
   ...
   [
     {
       ...
       "metadata": [
         {"name": "MyBenchmark simulation task", "data": "Isaac-Cartpole-Manager", "type": "string"}
       ]
     }
   ]
