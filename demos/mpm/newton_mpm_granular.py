...
A block of granular material is dropped onto static box colliders. The example
...
.. code-block:: bash
    ...
    uvx isaaclab example mpm-granular --visualizer newton_gl
"""
...
def keep_running(sim, count: int) -> bool:
    """Return whether the example loop should continue this frame."""
...
def main() -> None:
    """Launch and run the Isaac Lab MPM granular example."""
    ...
    with launch_simulation(sim_cfg, args_cli):
        ...
        sim.reset()
        print(
            f"[INFO]: Isaac Lab Newton granular MPM example ready. Spawned {particle_count(scene)} particles.",
            flush=True,
        )
