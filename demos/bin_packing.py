...
def spawn_grocery(
    ...
    **kwargs,
    ...
    for mesh in sim_utils.get_all_matching_child_prims(root_path, lambda child: child.GetTypeName() == "Mesh"):
        ...
        schemas.define_collision_properties(mesh_path, schemas.UsdPhysicsCollisionCfg(collision_enabled=True))
        schemas.define_mesh_collision_properties(mesh_path, PhysxConvexHullCfg())
    schemas.define_rigid_body_properties(root_path, PhysxRigidBodyCfg(solver_position_iteration_count=4))
    schemas.define_mass_properties(root_path, schemas.MassCfg(mass=GROCERY_MASS))
    return prim
