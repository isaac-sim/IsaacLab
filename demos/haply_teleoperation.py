...
"""Teleoperate a robot arm with Haply Inverse3 and VerseGrip devices.

The Inverse3 provides position tracking and force feedback. The VerseGrip provides orientation and button input.

.. code-block:: bash

    uvx isaaclab demo haply-teleoperation --websocket_uri ws://localhost:10001
...
def apply_haply_to_robot_mapping(
    ...
    Applies the transformed Haply displacement to the initial robot position.

    Args:
        haply_pos: Current Haply absolute position [m], shape [3].
        haply_initial_pos: Haply zero-reference position [m], shape [3].
        robot_initial_pos: Initial robot end-effector position [m], shape [3].

    Returns:
        Target end-effector position in the world frame [m], shape [3].
    """
    haply_delta = haply_pos - haply_initial_pos
    robot_offset = torch.stack((-haply_delta[1], haply_delta[0], haply_delta[2] - HAPLY_Z_OFFSET))
    robot_pos = robot_initial_pos + robot_offset
    robot_pos[0].clamp_(*WORKSPACE_LIMITS["x"])
    robot_pos[1].clamp_(*WORKSPACE_LIMITS["y"])
    robot_pos[2].clamp_(*WORKSPACE_LIMITS["z"])
    return robot_pos
...
class FrankaHaplySceneCfg(InteractiveSceneCfg):
    ...
    cube = RigidObjectCfg(
        ...
        spawn=sim_utils.CuboidCfg(
            ...
            physics_material=sim_utils.UsdPhysicsRigidBodyMaterialCfg(static_friction=0.5, dynamic_friction=0.5),
...
def run_simulator(
    ...
    robot_initial_pos = robot.data.body_pos_w.torch[0, ee_body_idx].clone()
    haply_initial_pos = torch.zeros(3, device=sim.device)
    ...
    while sim.is_headless_or_exist_active_visualizer() and (args_cli.max_steps < 0 or step_count < args_cli.max_steps):
        ...
        left_forces = left_finger_sensor.data.net_normal_forces_w
        right_forces = right_finger_sensor.data.net_normal_forces_w
        if left_forces is None or right_forces is None:
            raise RuntimeError("Haply force feedback requires contact-force data.")
        left_finger_forces = left_forces.torch[0, 0]
        right_finger_forces = right_forces.torch[0, 0]
        total_contact_force = (left_finger_forces + right_finger_forces) * 0.5
        haply_device.push_force(forces=total_contact_force.unsqueeze(0), position=env_ids)


def main() -> None:
    """Set up and run the Haply teleoperation demo."""
