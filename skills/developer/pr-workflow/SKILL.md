...
5. If the PR changes documentation, run `uv run --isolated --locked --extra test -- make -C docs current-docs` and require a successful build with no warnings or errors. This keeps validation in a temporary locked combined test and documentation environment, and the target treats Sphinx warnings as errors.
