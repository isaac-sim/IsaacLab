...
7. When the failure looks environmental rather than code-level, or when a report has to be reproduced on another machine, ask for a bundle from `python3 tools/capture_env.py capture`; it runs on an installation too broken to import Isaac Lab, and its `REPRODUCE.md` carries the steps to rebuild the environment locally. Tell the user to read `REPRODUCE.md` and `env/environment.txt` from the bundle before attaching it to a public issue: the capture records the hostname, the command they name, and allowlisted path variables. The bundle records the commit the checkout sits on, but no remotes and no uncommitted changes.
