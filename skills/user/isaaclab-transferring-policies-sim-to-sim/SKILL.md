...
3. **Articulation joint and body ordering.** The backends traverse the same USD articulation differently, so any branched robot's joint and body tensor axes are permutations of each other. A checkpoint stores column positions, not names, so an uncorrected cross-backend replay feeds each joint's observation to a different joint's action. On every PN and NP command, set both `env.scene.robot.joint_ordering` and `env.scene.robot.body_ordering` to the **source** checkpoint's convention (`physx` for PN, `mjwarp` for NP); `physics=` still selects the target backend. Serial chains such as `Isaac-Lift-Franka` agree natively and need no override; `Isaac-Velocity-Rough-G1` and `Isaac-Velocity-Rough-AnymalD` do need it. Verify by comparing `joint_names` with `backend_joint_names` (and the body equivalents); a scrambled axis collapses the policy immediately rather than degrading gracefully.
