...
1. **Task readiness and checkpoint compatibility.** Keep one MDP for the same registered task. Resolve each backend's `PresetCfg`, using it for intentional physics, asset, and control differences without silently changing policy-facing MDP terms. If an MDP-term preset differs, restore one checkpoint contract or treat it as a different task and retrain. Require successful training in both engines and exact action, observation, policy-state, timing, mechanism, and episode contracts.
