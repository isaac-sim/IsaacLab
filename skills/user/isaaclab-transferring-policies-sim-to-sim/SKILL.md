...
1. **Task readiness and checkpoint compatibility.** First ensure the same task can be trained successfully in both physics engines. Resolve both task variants and require exact equality for action, observation, policy-state, timing, mechanism, and episode contracts.
