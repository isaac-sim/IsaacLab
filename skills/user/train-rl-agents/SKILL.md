...
6. From the Isaac Lab checkout, use `uv run train` and `uv run play` with `--rl_library` rather than the deprecated per-library scripts under `scripts/reinforcement_learning/`.
