...
```bash
./isaaclab.sh -p scripts/environments/random_agent.py --task Isaac-Ant --num_envs 8
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py --task Isaac-Ant --num_envs 64 --max_iterations 5 --headless
```

Keep the direct baseline available for comparison:

```bash
./isaaclab.sh -p scripts/environments/random_agent.py --task Isaac-Ant-Direct --num_envs 8
```
