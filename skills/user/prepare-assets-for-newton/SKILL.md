...
5. **Velocity limits distinction.** `velocity_limit` is a rated speed and `velocity_limit_sim` is a requested solver clamp. Isaac Lab writes the latter to Newton's model, but MJWarp does not consume or enforce it; Kamino does. Add task or control checks for required speed bounds under MJWarp and use per-joint `effort_limit_sim`.
