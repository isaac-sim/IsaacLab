...
4. **Match contact and friction behavior.** Expect more default slip in MJWarp. Validate colliders, material bindings, contact locations and gripper force; inspect resolved `condim`; tune material friction; then compare the global cone and `impratio`. Use fixed-grasp displacement, contact count, effort, penetration, and success metrics.
