...
2. **Import a multi-physics asset.** Use provided Isaac Lab assets directly where supported. For a new URDF or MJCF, retain `run_asset_transformer=True` and `run_multi_physics_conversion=True` so the importer creates neutral physics, PhysX, and MuJoCo payloads. Account for the nested rigid-body structure.
3. **Separate common and solver-specific properties.** Keep common USD Physics fields in solver-common base cfgs, MJWarp fields in `Mujoco*Cfg`, Newton-native fields in `Newton*Cfg`, and PhysX-only fields in `Physx*Cfg`. Confirm support in the generated schema APIs.
4. **Audit the mechanical model.** Verify intentional positive mass, COM, inertia and frames, collision geometry, approximation and scale, material binding, topology, articulation root, fixed-base and fixed-joint representation, joint axes and limits, self-collision, and gravity overrides.
5. **Match collision, contact, and friction behavior.** Verify colliders, bindings, contact locations/counts, and normal force before changing friction. Inspect per-shape `condim`, then tune material friction. Reserve `priority`, `solmix`, `solref`, and `solimp` for measured per-collider needs; use the Tune MJWarp how-to for global tuning.
6. **Validate actuators and limits.** Audit per-joint effort, gains, friction, armature, action scale, and control period. Armature is for articulated coordinates only; retune damping after changing it. `actuator_velocity_limit` is rated speed and `joint_velocity_limit` requests a solver clamp, but MJWarp enforces neither while stepping; Kamino honors the solver clamp. Enforce required speed bounds in task or control logic. Use `joint_effort_limit` for solver limits and `actuator_effort_limit` to clip explicit actuator models.
