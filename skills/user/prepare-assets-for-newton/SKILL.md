...
8. **Diagnose Newton-only failures.** Reproduce the first bad step with one environment, fixed state, no randomization, and identical actions. Classify initialization/model, contact/capacity, control, or dense-scene failures. Raise overflowing capacity before changing convergence settings.
...
Keep this skill synchronized with the authoritative how-to. Use the MJWarp solver page for current solver defaults and tuning values.
