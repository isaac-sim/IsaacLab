...
| Parent joint frame | Reaction-force fixtures; Kamino frame conversion; Isaac Lab `FIX-JOINT-FRAME` | Physical world-space motion on MJWarp and Kamino | Retain USD coverage; no independent Isaac Lab Python override exists |
| Child joint frame | Reaction-force fixtures; Kamino frame conversion; Isaac Lab `FIX-JOINT-FRAME` | Physical link pose and motion on MJWarp and Kamino | Retain USD coverage; no independent Isaac Lab Python override exists |
...
| Velocity limit | MuJoCo property tests plus Isaac Lab `FIX-LIMIT-VEL` | Strict expected failures detect missing enforcement on MJWarp and Kamino | MJWarp `JOINT-05` remains an accepted Isaac Lab out-of-scope gap; add sustained-drive coverage for PhysX |
| Effort limit | One-step clamped drive response plus Isaac Lab `FIX-LIMIT-EFFORT` | MJWarp USD/config/runtime paths are analytical; Kamino remains a strict expected failure | Extend the fixture to PhysX and retain Kamino issue coverage |
...
| Joint friction coefficient | MuJoCo property/notify tests plus Isaac Lab `FIX-PASSIVE` | MJWarp USD/config/runtime paths physically match absolute dry-friction effort; Kamino revolute coverage is a strict expected failure | Extend the fixture to PhysX and retain Kamino issue coverage |
...
- **Kamino inertial runtime notifications do not refresh every derived quantity.** `set_masses_index` and
  ...
  `set_coms_index` updates Kamino's COM-dependent model data and Isaac Lab invalidates derived world COM data.
...
- **Common Python mass schemas remain incomplete for Phase 1.** `MassCfg` and `MassPropertiesCfg` expose mass
  ...
  Python override paths are unavailable.
