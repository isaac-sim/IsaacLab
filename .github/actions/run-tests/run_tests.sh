...
run_tests() {
  ...
  docker run -d --name $container_name \
    ...
    -v "$PWD/.github/actions/_lib/retry-command.sh:/retry-command.sh:ro" \
    ...
    -c "
      ...
      bash /retry-command.sh ./isaaclab.sh -p -m pip install pytest pytest-mock junitparser flaky \"coverage>=7.6.1\"
      ...
      if [ -n \"\${TEST_EXTRA_PIP_PACKAGES:-}\" ]; then
        ...
        bash /retry-command.sh ./isaaclab.sh -p -m pip install \${TEST_EXTRA_PIP_PACKAGES}
      ...
      if [ -n \"\${TEST_EXTRA_UV_PACKAGES:-}\" ]; then
        ...
        if [ ! -x \"\${uv_executable}\" ]; then
          bash /retry-command.sh ./isaaclab.sh -p -m pip install uv
        fi
        bash /retry-command.sh \"\${uv_executable}\" pip install --python \"\${isaac_python}\" --target \"\${isaac_user_site}\" \${TEST_EXTRA_UV_PACKAGES}
        ...
        bash /retry-command.sh \"\${uv_executable}\" pip install --python \"\${isaac_python}\" --target \"\${isaac_uv_overlay}\" --no-deps \${TEST_EXTRA_UV_PACKAGES}
