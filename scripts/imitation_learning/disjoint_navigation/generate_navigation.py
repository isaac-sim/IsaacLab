...
from isaaclab_mimic.disjoint_nav_envs.g1_disjoint_nav_env import G1DisjointNavEnv, G1DisjointNavEnvCfg
...
class DisjointNavDataGenerationState(enum.IntEnum):
    ...
    GRASP_OBJECT = 0  # Robot grasps object at start position
    LIFT_OBJECT = 1  # Robot lifts object while stationary
    NAVIGATE = 2  # Robot navigates to approach position with object
    APPROACH = 3  # Robot approaches final goal position
    DROP_OFF_OBJECT = 4  # Robot places object at end position
    DONE = 5  # Task completed
...
class NavigationControlConfig:
    ...
    angular_gain: float = 2.0  # Proportional gain for angular velocity control
    linear_gain: float = 1.0  # Proportional gain for linear velocity control
    linear_max: float = 1.0  # Maximum allowed linear velocity (m/s)
    distance_threshold: float = 0.1  # Distance threshold for state transitions (m)
    following_offset: float = 0.6  # Look-ahead distance for path following (m)
    angle_threshold: float = 0.2  # Angular threshold for orientation control (rad)
    approach_distance: float = 1.0  # Buffer distance from final goal (m)


def compute_navigation_velocity(
    current_pose: torch.Tensor, target_xy: torch.Tensor, config: NavigationControlConfig
...
def setup_navigation_scene(
    env: G1DisjointNavEnv, input_episode_data: EpisodeData, approach_distance: float, randomize_placement: bool = True
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        env = G1DisjointNavEnv(cfg=env_cfg)
