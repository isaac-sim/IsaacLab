...
for TASK in $TASKS; do
    for NUM_ENV in $NUM_ENVS; do
        if [[ $task == *"RGB-Camera"* ]] || [[ $task == *"Depth-Camera"* ]]; then
            ./isaaclab.sh -p source/isaaclab/isaaclab/test/benchmark/benchmark_core.py --backend omniperf --output_dir "$OUTPUT_DIR" --task "$TASK" --num_envs "$NUM_ENV" --headless --num_frames "$NUM_FRAMES" --enable_cameras
        else
            ./isaaclab.sh -p source/isaaclab/isaaclab/test/benchmark/benchmark_core.py --backend omniperf --output_dir "$OUTPUT_DIR" --task "$TASK" --num_envs "$NUM_ENV" --headless --num_frames "$NUM_FRAMES"
        fi
    done
done