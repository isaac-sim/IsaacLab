...
"""Smoke test for scripts/benchmarks/play.py with --rl_library sb3.

Trains a short cartpole run to produce a checkpoint, then plays that checkpoint
through the benchmark and asserts the emitted ``PlayBundle``. SB3's
``CheckpointCallback`` saves every 16000 timesteps, so the training run needs
more iterations than the other libraries to write a checkpoint at all.
"""
