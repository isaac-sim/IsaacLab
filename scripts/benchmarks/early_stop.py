...
DEFAULT_SUCCESS_TAG = "Metrics/success_rate"
...
class SuccessRateTracker:
    """Accumulates a per-iteration metric and checks trailing-window convergence.

    Args:
        metric_key: Key in ``extras["log"]`` to track.
    ...
    def __init__(self, metric_key: str, threshold: float, window: int, num_steps_per_env: int):
    ...
    def record_step(self, extras: dict) -> None:
        """Record one env step."""
        log = extras.get("log", {})
        if self.metric_key in log:
            val = log[self.metric_key]
...
class RslRlEarlyStopWrapper:
    ...
    def __init__(
        ...
        self.env = env
        self.runner = runner
        self.tracker = SuccessRateTracker(metric_key, threshold, window, num_steps_per_env)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is EarlyStopConverged:
            ...
            print(
                f"[INFO] Early stop: {self.tracker.metric_key} converged at iteration "
...
class RlGamesEarlyStopObserver:
    ...
    Args:
        ...
        metric_key: Key in ``infos["log"]`` to track.
        threshold: Minimum metric value to pass.
        window: Consecutive iterations above threshold to trigger stop.
    ...
    def after_init(self, algo):
        ...
        self.tracker = SuccessRateTracker(self.metric_key, self.threshold, self.window, num_steps)
...
def add_success_cli_args(parser: argparse.ArgumentParser) -> None:
    ...
    Adds ``--check_success``, ``--success_tag``, ``--success_threshold``, and
    ``--success_window``. Use :func:`build_success_kwargs` to resolve the parsed
    values into a kwargs dict for the wrapper constructors.
    """
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"Override the success metric tag (default: {DEFAULT_SUCCESS_TAG}).",
    )
    parser.add_argument(
        "--success_threshold",
        type=float,
...
def build_success_kwargs(args_cli: argparse.Namespace) -> dict:
    ...
    Returns a dict with ``metric_key``, ``threshold``, ``window``, and
    ``stop_on_convergence``, suitable to splat into :class:`RslRlEarlyStopWrapper`
    or :class:`RlGamesEarlyStopObserver`.
    """
    return {
        "metric_key": args_cli.success_tag if args_cli.success_tag is not None else DEFAULT_SUCCESS_TAG,
        "threshold": (
...
def get_success_tracker(
    ...
    history = log_data.get(kwargs["metric_key"])
    if not history:
        return None
    tracker = SuccessRateTracker(kwargs["metric_key"], kwargs["threshold"], kwargs["window"], num_steps_per_env=0)
