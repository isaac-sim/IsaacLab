...
spawns only the sampled subset of objects into each environment. The simulation
...
class BinPackingSceneCfg(InteractiveSceneCfg):
    ...
    The static fields declare the world-shared ground plane and light plus the
    storage bin. :meth:`__post_init__` then declares one
    :class:`~isaaclab.assets.RigidObjectCfg` field per grocery slot, cycling
    through the grocery variants.
    """
    ...
    def __post_init__(self):
        """Declare one grocery slot per possible object, cycling through the grocery variants."""
        ...
        for slot, name in enumerate(GROCERY_NAMES):
            setattr(
                ...
                RigidObjectCfg(
                    ...
                    spawn=sim_utils.UsdFileCfg(
                        usd_path=GROCERY_USDS[slot % len(GROCERY_USDS)], rigid_props=rigid_props
                    ),
                ),
            )
...
def make_bin_packing_scene_cfg(
    ...
    """Instantiate :class:`BinPackingSceneCfg` with clone combinations encoding the sampled layouts.
...
def main():
    ...
    with launch_simulation(cfg=PhysicsCfg(), launcher_args=args_cli) as physics_cfg:
        # Sample a random number of objects per environment
