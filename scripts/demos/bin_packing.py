...
BIN_XY_BOUND = ((-0.2, -0.3), (0.2, 0.3))  # ((x_min, y_min), (x_max, y_max))
...
def build_grocery_defaults(
    ...
    - Active poses: stacked 3D grid over the bin with ``z_spacing`` per layer.
    ...
    Args:
        ...
        z_spacing: Vertical spacing between active layers.
        cache_spacing: XY spacing for the cached grid.
        num_objects_per_layer: Objects per layer in the active stack.
        max_objects_per_bin: Upper bound of active objects per environment.
        max_num_objects: Cap for objects considered per environment.
        cache_height: Height for cached objects (meters).

    Returns:
        Tuple of two tensors (active_spawn_poses, cached_spawn_poses), each of
        shape (num_envs, M, 7) with [x, y, z, qx, qy, qz, qw], where M = ``max_num_objects``.
    """
