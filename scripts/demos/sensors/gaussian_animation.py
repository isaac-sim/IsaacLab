...
``positions``/``orientations``/``scales`` arrays are themselves time sampled (deformable motion). Both
spellings of the per-particle arrays exist in the wild: the half-precision ``positionsh``,
``orientationsh``, and ``scalesh`` that NuRec exports, and the full-float ``positions``,
...
SCALES_ATTR_NAMES = ("scalesh", "scales")
"""Candidate per-particle scale attribute names, most specific first."""
...
def _write_fabric_particle_array(
    ...
    Warp specializes this generic kernel for each supported vector element type, avoiding duplicate
    kernels for positions, orientations, and scales.
    """
    prim_index, particle_index = wp.tid()
    output[prim_index, particle_index] = values[particle_index]
...
class AnimatedGaussianTrack:
    ...
    scales_attr_name: str | None
    """Name of the time-sampled per-particle scale attribute, or ``None`` if scales are static."""
    ...
    def is_deformable(self) -> bool:
        ...
        return any((self.positions_attr_name, self.orientations_attr_name, self.scales_attr_name))
    ...
    for prim in Usd.PrimRange(default_prim):
        ...
        tracks.append(
            AnimatedGaussianTrack(
                ...
                scales_attr_name=None if scales_attr is None else scales_attr.GetName(),
...
def collect_authored_times(source_stage: Usd.Stage, tracks: list[AnimatedGaussianTrack]) -> list[float]:
    ...
    for track in tracks:
        ...
        for attr_name in (track.positions_attr_name, track.orientations_attr_name, track.scales_attr_name):
...
class IsaacRtxFabricTrackPlayback:
    ...
    def __init__(
        ...
        for track in tracks:
            ...
            for attr_name, half_dtype, float_dtype in (
                ...
                (track.scales_attr_name, wp.vec3h, wp.vec3f),
            ):
    ...
    def write_array_attribute(self, frame_index: int) -> None:
        ...
        for binding in self._bindings:
            ...
            inputs = [values, binding.output] if binding.is_array else [values, binding.rows, binding.output]
            wp.launch(binding.kernel, dim=dim, inputs=inputs, device=self._device)
...
class OVRTXGaussianTrackPlayback:
    ...
    def __init__(
        ...
        # Completion handles for the renderer reads issued from each slot. They are waited only
        # when that slot is recycled, allowing every other slot to overlap with rendering.
        self._slot_syncs: list[list[RendererSync]] = [[] for _ in range(num_slots)]
        self._submitted: list[int | None] = [None] * num_slots
        self._last_played = 0
        ...
        for track in tracks:
            ...
            for kind, attr_name, components in (
                ...
                ("scales", track.scales_attr_name, 3),
            ):
                if attr_name is None:
                    continue
                attribute = gaussian_prim.GetAttribute(attr_name)
                # Vt converts quaternion arrays to (imaginary, real) components, which is already the
                # (x, y, z, w) order the renderers expect. All columns are then a plain vectorized cast.
    ...
    def _wait_for_slot_reuse(self, slot: int) -> None:
        """Wait until the renderer has released every input buffer in ``slot``."""
        for sync in self._slot_syncs[slot]:
            sync.wait()
        self._slot_syncs[slot].clear()

    def close(self) -> None:
        """Drain renderer reads before releasing the ring buffers."""
        for slot in range(self._num_slots):
            self._wait_for_slot_reuse(slot)

    def play(self, renderer: BaseRenderer, frame_index: int) -> None:
        ...
        asynchronous, so this returns without waiting on the GPU. A slot's completion handles are
        waited only immediately before that slot is overwritten. Replaying the current frame is
        free, which is what lets the caller seed a pose before its warmup steps.

        Args:
            renderer: Renderer exposing the ``update_particle_field_*`` hooks.
        ...
        for index in range(frame_index, min(frame_index + self._prefetch_depth, self._num_frames - 1) + 1):
            ...
            if self._resident[upload_slot] == index:
                continue
            self._wait_for_slot_reuse(upload_slot)
            for column in self._columns:
                wp.copy(column.ring[upload_slot], column.host[index])
            self._resident[upload_slot] = index
            self._submitted[upload_slot] = None
        self._last_played = frame_index

        slot = frame_index % self._num_slots
        if self._submitted[slot] == frame_index:
            return
        for column in self._columns:
            values = column.ring[slot]
            if column.kind == "xform":
                sync = renderer.update_particle_field_transforms(column.prim_paths, values)
            else:
                # Every env shares the sample: the arrays are in the Gaussian prim's local frame.
                shared = [values] * len(column.prim_paths)
                sync = renderer.update_particle_field_particles(column.prim_paths, **{column.kind: shared})
            self._slot_syncs[slot].append(sync)
        self._submitted[slot] = frame_index
...
class _AnimationColumn:
    ...
    kind: str
    """Which renderer hook the column feeds: ``xform``, ``positions``, ``orientations`` or ``scales``."""
