...
parser.add_argument(
    ...
    type=int,
    default=None,
    help=("Render passes to run before saving images. Defaults to 32 for Isaac RTX and OVRTX, and 0 for Newton."),
)
parser.add_argument(
    "--write_fps",
    type=float,
...
if args_cli.rtx_render_mode == "Minimal":
    parser.error("--rtx_render_mode Minimal is incompatible with this demo's RGB camera output.")
...
def force_gaussian_sorting_mode_hint(stage: Usd.Stage, sorting_mode: str = "cameraDistance") -> int:
    """Force a Gaussian sort mode on the demo stage's root layer.

    This is a temporary PPISP-demo workaround for `OMPE-108417`: RTX drops all Gaussian
    contribution from a multi-camera OVRTX render product when the asset-authored sort mode is
    ``zDepth``. ``cameraDistance`` renders correctly, but changes the asset's requested blending
    order, so this helper must remain opt-in at the demo call site and be removed when the issue is
    fixed.

    Args:
        stage: USD stage to modify.
        sorting_mode: Sort mode token to author on each Gaussian-splat prim.

    Returns:
        Number of Gaussian-splat prims overridden.
    """
    attr_paths = [
        prim.GetPath().AppendProperty("sortingModeHint")
...
def _requested_isaacrtx_settings() -> list[tuple[str, bool | float | int | str, str]]:
    ...
    if args_cli.rtx_accumulation_limit is not None:
        settings.append(("/rtx/raytracing/accumulationLimit", args_cli.rtx_accumulation_limit, "accumulationLimit"))
...
def get_playback_time_samples(
    ...
    time_samples.update(gaussian_anim.collect_authored_times(source_stage, gaussian_tracks))
    ...
    # Keep the cadence strictly uniform and include a final sample at or beyond the last authored
    # time. USD holds the last authored value if the final uniform sample falls just past it.
    time_step = time_codes_per_second / args_cli.render_fps
    sample_count = int(np.ceil((end_time - start_time) / time_step)) + 1
...
def run_simulator(
    ...
    write_dt = 1.0 / args_cli.write_fps
    next_write_time = 0.0
    ...
    print(
        ...
        f"writing at up to {args_cli.write_fps:g} FPS.",
        flush=True,
    )
    if args_cli.warmup_steps > 0:
    ...
    for frame_index, time_code in enumerate(frame_time_codes):
        ...
        frame_time = frame_index * render_dt
        if frame_time + 1.0e-12 < next_write_time:
            continue
        while next_write_time <= frame_time + 1.0e-12:
...
def main() -> None:
    ...
    try:
        ...
        if playback_times:
            print(
                f"[INFO] Resampled {len(playback_times)} camera/Gaussian animation time sample(s) at "
                f"{args_cli.render_fps:g} FPS: "
                f"{playback_times[0]:g}..{playback_times[-1]:g}.",
                flush=True,
            )
        else:
            playback_times = [args_cli.camera_time_code]
            print(
                f"[INFO] No camera or Gaussian animation samples found; playing the static scene at USD time "
                f"{args_cli.camera_time_code:g}.",
                flush=True,
            )
        frame_time_codes = playback_times
        if args_cli.num_frames is not None:
            # Hold the last animation state when more frames are requested than the timeline provides.
        ...
        if args_cli.renderer == "ovrtx":
            ...
            if overridden_count:
                print(
                    f"[INFO] Forced sortingModeHint=cameraDistance on {overridden_count} Gaussian splat prim(s) "
                    "for OVRTX (temporary workaround for OMPE-108417).",
                    flush=True,
                )
