...
.. code-block:: bash
    ...
    ./env_isaaclab/bin/python scripts/demos/sensors/ppisp_camera_ovrtx.py \
        --input_scene /path/to/scene.usdz \
        --max_steps 60 \
        --num_envs 8
...
parser.add_argument(
    ...
    type=float,
    default=0.0,
    help="USD time code used to bake the selected camera pose into duplicated env cameras.",
)
parser.add_argument(
    "--num_envs",
    type=int,
...
def run_simulator(sim: sim_utils.SimulationContext, baseline_camera: Camera, ppisp_camera: Camera) -> None:
    ...
    for count in range(1, args_cli.max_steps + 1):
        sim.step()
        baseline_camera.update(sim_dt, force_recompute=True)
        ppisp_camera.update(sim_dt, force_recompute=True)
        ...
        if count % args_cli.save_interval == 0:
            ...
            save_images_grid(
                ...
                filename=os.path.join(output_dir, f"ppisp_camera_ovrtx_{count:04d}.png"),
            )
            save_tensor_image(
                make_tiled_image(baseline.float() / 255.0),
                os.path.join(output_dir, f"ppisp_camera_ovrtx_{count:04d}_baseline_tiled.png"),
            )
            save_tensor_image(
                make_tiled_image(ppisp.float() / 255.0),
                os.path.join(output_dir, f"ppisp_camera_ovrtx_{count:04d}_ppisp_tiled.png"),
            )
            save_tensor_image(
                make_tiled_image(diff),
                os.path.join(output_dir, f"ppisp_camera_ovrtx_{count:04d}_diff_tiled.png"),
            )
