...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to YAML file containing joint order to treat as schema order for importing (uses key 'target_joint_names' by default).",
)
parser.add_argument(
    "--import_schema_joint_order_key",
    type=str,
    default="target_joint_names",
    help="Key inside YAML to read schema joint order from for importing (default: target_joint_names).",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to YAML file containing joint order to treat as schema order (uses key 'source_joint_names' by default).",
)
parser.add_argument(
    "--export_schema_joint_order_key",
    type=str,
    default="source_joint_names",
    help="Key inside YAML to read schema joint order from (default: source_joint_names).",
)
...
class _SchemaPermutationHelper:
    ...
    def compute(self) -> bool:
        ...
        if schema_to_sim is None:
            return False
        print(schema_to_sim, sim_to_schema)
        ...
        # Required terms (names per velocity task config). If any missing, we skip reordering.
        try:
            idx_joint_pos = term_names.index("joint_pos")
            idx_joint_vel = term_names.index("joint_vel")
            idx_actions_obs = term_names.index("actions")
            print("idx_joint_pos", idx_joint_pos)
            print("idx_joint_vel", idx_joint_vel)
            print("idx_actions_obs", idx_actions_obs)
        except ValueError:
            return False

        # Validate sizes match number of joints
        num_joints = len(sim_joint_names)
        if term_dims[idx_joint_pos] != num_joints or term_dims[idx_joint_vel] != num_joints or term_dims[idx_actions_obs] != num_joints:
            return False
...
def main():
    ...
    if args_cli.export_schema_joint_order:
        try:
            ...
            if args_cli.export_schema_joint_order_file:
                ...
                key = args_cli.export_schema_joint_order_key or "target_joint_names"
                if key not in cfg_yaml:
                    raise KeyError(f"Key '{key}' not found in YAML {args_cli.export_schema_joint_order_file}")
    ...
    try:
        ...
        print(f"[DEBUG] Policy device before: {next(policy_nn.parameters()).device if hasattr(policy_nn, 'parameters') else 'No parameters'}")
        
        if hasattr(ppo_runner, "alg") and hasattr(ppo_runner.alg, "to"):
            ppo_runner.alg.to(target_device)
            print(f"[DEBUG] Moved ppo_runner.alg to {target_device}")
        ...
        if hasattr(policy, "to"):
            ...
            print(f"[DEBUG] Moved policy function to {target_device}")
        
        print(f"[DEBUG] Policy device after: {next(policy_nn.parameters()).device if hasattr(policy_nn, 'parameters') else 'No parameters'}")
        print(f"[DEBUG] Policy function device: {next(policy.parameters()).device if hasattr(policy, 'parameters') else 'No parameters'}")
        ...
        print(f"[DEBUG] Exception in device alignment: {e}")
        pass
    timestep = 0
