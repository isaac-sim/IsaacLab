...
def main() -> None:
    ...
    # Deactivating a link's only collision scope also deletes the inertia PhysX would have
    # computed from it. Every shipped link carries an explicit mass except left_hand_palm_link,
    # which is left with a zero inertia tensor: MuJoCo tolerates that, PhysX rejects the whole
    # articulation ("setMassSpaceInertiaTensor(): components must be > 0") and the observations
    # come back NaN on the first step. Copy the value off the mirror twin.
    mass_by_name: dict[str, tuple] = {}
    for prim in stage.Traverse():
        if prim.HasAPI(UsdPhysics.MassAPI):
            api = UsdPhysics.MassAPI(prim)
            if api.GetMassAttr().Get() is not None:
                mass_by_name[prim.GetName()] = (
                    api.GetMassAttr().Get(),
                    api.GetDiagonalInertiaAttr().Get(),
                    api.GetCenterOfMassAttr().Get(),
                )

    def _mass_fix(prim, indent: str) -> tuple[str, str]:
        """Return (api_schema_clause, body_lines) restoring an implied inertia, or ("", "")."""
        ...
        # The twin is the mirror image, so its inertia diagonal carries over and y flips on the
        # centre of mass.
        body = (
            f"{indent}float physics:mass = {mass!r}\n"
    ...
    prim_by_name = {c.GetName(): c for c in root.GetChildren()}
    for parent_name, children in nested_scopes.items():
        for child in prim_by_name[parent_name].GetChildren():
            prim_by_name.setdefault(f"{parent_name}/{child.GetName()}", child)

    for link in sorted(set(top_scopes) | set(by_link)):
        clause, body = ("", "")
        if link in top_scopes and link not in by_link:
            clause, body = _mass_fix(prim_by_name[link], "        ")
        lines.append(f'    over "{link}"{clause}\n    {{\n')
    ...
    for parent, children in sorted(nested_scopes.items()):
        ...
        for child in sorted(children):
            clause, body = _mass_fix(prim_by_name[f"{parent}/{child}"], "            ")
            lines.append(
                f'        over "{child}"{clause}\n        {{\n'
                '            over "collisions" (\n                active = false\n            )\n'
                "            {\n            }\n"
