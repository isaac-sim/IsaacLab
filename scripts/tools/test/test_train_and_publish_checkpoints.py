...
@pytest.mark.parametrize(
    "task_name",
    ["Isaac-Reorient-Cube-Shadow-Camera", "Isaac-Reorient-Cube-Shadow-Camera-Direct"],
)
def test_collect_shadow_hand_feature_extractor(
    task_name: str,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Shadow Hand camera collection must include the CNN saved by its feature extractor."""
    run_path = tmp_path / "run"
    run_path.mkdir()
    policy_path = run_path / "model_100.pt"
    policy_path.write_bytes(b"policy")
    feature_extractor_path = run_path / "cnn_50000_0.1.pth"
    feature_extractor_path.write_bytes(b"feature extractor")
    monkeypatch.setattr(
        "scripts.tools.train_and_publish_checkpoints.get_pretrained_checkpoint_path",
        lambda *args, **kwargs: str(policy_path),
    )
    job = CheckpointJob(
        workflow="rsl_rl",
        task_name=task_name,
    ...
    collected_path = collect_pretrained_checkpoint(job, str(tmp_path / "collected"))

    expected_stem = f"{task_name}_physx_rtx_rsl_rl"
    assert collected_path == str(tmp_path / "collected" / "rsl_rl" / f"{expected_stem}.pt")
    assert Path(collected_path).read_bytes() == b"policy"
    assert Path(collected_path).with_name(f"{expected_stem}_feature_extractor.pth").read_bytes() == (
        b"feature extractor"
    )


def test_collect_shadow_hand_requires_feature_extractor(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Shadow Hand camera collection must reject a policy without its required CNN."""
    run_path = tmp_path / "run"
    run_path.mkdir()
    policy_path = run_path / "model_100.pt"
    policy_path.touch()
    ...
    job = CheckpointJob(
        workflow="rsl_rl",
        task_name="Isaac-Reorient-Cube-Shadow-Camera",
        physics_backend="physx",
        render_backend="rtx",
    )
    output_dir = tmp_path / "collected"

    assert collect_pretrained_checkpoint(job, str(output_dir)) is None
    assert not output_dir.exists()


def test_publish_shadow_hand_feature_extractor(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Shadow Hand camera publication must upload the policy and its feature extractor."""
    task_name = "Isaac-Reorient-Cube-Shadow-Camera-Direct"
    stem = f"{task_name}_newtonmjwarp_newton_rsl_rl"
    collected_dir = tmp_path / "rsl_rl"
    collected_dir.mkdir()
    policy_path = collected_dir / f"{stem}.pt"
    policy_path.touch()
    feature_extractor_path = collected_dir / f"{stem}_feature_extractor.pth"
    feature_extractor_path.touch()
    job = CheckpointJob(
        workflow="rsl_rl",
        task_name=task_name,
        physics_backend="newtonmjwarp",
        render_backend="newton",
    )
    args = Namespace(
        dry_run=True,
        force_publish=True,
    ...
    assert f"Publishing {policy_path} -> omniverse://checkpoints/rsl_rl/{stem}.pt" in output
    assert (
        f"Publishing {feature_extractor_path} -> omniverse://checkpoints/rsl_rl/{stem}_feature_extractor.pth" in output
    )


def test_publish_shadow_hand_requires_collected_feature_extractor(tmp_path: Path) -> None:
    """Shadow Hand camera publication must not advertise an incomplete checkpoint pair."""
    task_name = "Isaac-Reorient-Cube-Shadow-Camera"
    stem = f"{task_name}_physx_rtx_rsl_rl"
    collected_dir = tmp_path / "rsl_rl"
    collected_dir.mkdir()
    (collected_dir / f"{stem}.pt").touch()
    job = CheckpointJob(
        workflow="rsl_rl",
        task_name=task_name,
        physics_backend="physx",
