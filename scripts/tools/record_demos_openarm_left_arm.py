...
"""
Left-arm-only recording script for OpenArm tasks.

The environment may expose a 14D dual-arm action space, but keyboard teleoperation
is routed exclusively to the left arm. The right arm receives zero IK motion and
its gripper remains open.
...
parser = argparse.ArgumentParser(description="Record OpenArm demonstrations using left-arm-only teleoperation.")
...
class OpenArmKeyboard:
    ...
    Gripper:      K (toggle)
    Save/Reset:   N / R
    Left arm only: all teleoperation commands are routed to the left arm.
    """
...
class JointMirrorBroadcaster:
    ...
    # Mirror only the left arm and left gripper to external hardware.
    JOINT_NAME_PATTERNS = [
...
def build_left_arm_action(
    teleop_7d: torch.Tensor,
    right_gripper_state: float,
    device: str,
) -> tuple[torch.Tensor, float, float]:
    """Route all teleoperation commands to the left arm.

    The dual-arm environment still expects a 14D action vector:
        [0:6]   left-arm IK command
        [6]     left gripper command
        [7:13]  right-arm IK command (always zero)
        [13]    right gripper command (kept at its stored state)

    Args:
        teleop_7d: Shape (7,) tensor [dx, dy, dz, drx, dry, drz, gripper].
        right_gripper_state: Stored right-gripper command, normally +1.0 (open).
        device: Torch device string.

    Returns:
        Tuple containing the 14D action, updated left-gripper state, and unchanged
        right-gripper state.
    """
    full = torch.zeros(TOTAL_ACTION_DIM, dtype=torch.float32, device=device)
    left_gripper_cmd = float(teleop_7d[6].item())

    full[LEFT_IK_SLICE] = teleop_7d[:6]
    full[LEFT_GRP_IDX] = left_gripper_cmd

    # The right-arm IK entries remain zero, so keyboard input never moves it.
    full[RIGHT_GRP_IDX] = right_gripper_state

    return full, left_gripper_cmd, right_gripper_state
...
def main():
    ...
    def _refresh_label():
        label_text = f"Active arm: LEFT  |  Demos: {demo_count}"
        try:
    ...
    mode_str = "dual-arm action space" if is_dual_arm else "single-arm action space"
    print(f"\n=== OpenArm Left-Arm-Only Recording ({mode_str}) ===")
    ...
    with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
        while simulation_app.is_running() and not stop_requested["flag"]:
            ...
            if is_dual_arm:
                full_action, left_gripper_state, right_gripper_state = build_left_arm_action(
                    teleop_7d=teleop_7d,
            ...
            if should_reset:
                ...
                right_gripper_state = 1.0
                _refresh_label()
                print("Ready. Active arm: LEFT only")
